...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\APC40\APC40.py
...
class APC40(APC, OptimizedControlSurface):
    ...
    def _create_controls(self):
        ...
        self._shift_button = make_button(0, 98, resource_type=PrioritizedResource, name=b'Shift_Button')
        ...
        self._session_matrix = ButtonMatrixElement(name=b'Button_Matrix')
        self._scene_launch_buttons_raw = [ make_color_button(0, index + 82, name=b'Scene_%d_Launch_Button' % index) for index in xrange(SESSION_HEIGHT)
                                         ]
        self._track_stop_buttons = [ make_color_button(index, 52, name=b'Track_%d_Stop_Button' % index) for index in xrange(SESSION_WIDTH)
                                   ]
        self._stop_all_button = make_color_button(0, 81, name=b'Stop_All_Clips_Button')
        self._matrix_rows_raw = [ [ make_color_button(track_index, scene_index + 53, name=b'%d_Clip_%d_Button' % (track_index, scene_index)) for track_index in xrange(SESSION_WIDTH) ] for scene_index in xrange(SESSION_HEIGHT)
                                ]
        for row in self._matrix_rows_raw:
            self._session_matrix.add_row(row)

        self._selected_slot_launch_button = make_pedal_button(67, name=b'Selected_Slot_Launch_Button')
        self._selected_scene_launch_button = make_pedal_button(64, name=b'Selected_Scene_Launch_Button')
        ...
        for index in xrange(MIXER_SIZE):
            self._volume_controls.append(make_slider(index, 7, name=b'%d_Volume_Control' % index))
            self._arm_buttons.append(make_on_off_button(index, 48, name=b'%d_Arm_Button' % index))
            self._solo_buttons.append(make_on_off_button(index, 49, name=b'%d_Solo_Button' % index))
            self._mute_buttons.append(make_on_off_button(index, 50, name=b'%d_Mute_Button' % index))
            self._select_buttons.append(make_on_off_button(index, 51, name=b'%d_Select_Button' % index))
        ...
        self._master_select_button = make_on_off_button(0, 80, name=b'Master_Select_Button')
        ...
        for index in range(8):
            ...
            encoder_name = b'Device_Control_%d' % index
        ...
        self._play_button = make_button(0, 91, name=b'Play_Button')
        self._stop_button = make_button(0, 92, name=b'Stop_Button')
        self._record_button = make_button(0, 93, name=b'Record_Button')
        self._nudge_up_button = make_button(0, 100, name=b'Nudge_Up_Button')
        self._nudge_down_button = make_button(0, 101, name=b'Nudge_Down_Button')
        self._tap_tempo_button = make_button(0, 99, name=b'Tap_Tempo_Button')
        self._global_bank_buttons = []
        self._global_param_controls = []
        for index in range(8):
            encoder_name = b'Track_Control_%d' % index
    ...
    def _create_device(self):
        self._device = DeviceComponent(name=b'Device_Component', is_enabled=False, layer=Layer(bank_buttons=self._device_bank_buttons, on_off_button=self._device_on_off_button), use_fake_banks=True, device_selection_follows_track_selection=True)
    ...
    def _create_transport(self):
        ...
        self._bank_button_translator = ChannelTranslationSelector(name=b'Bank_Button_Translations', is_enabled=False)
    ...
    def _create_global_control(self):
        ...
        encoder_modes = ModesComponent(name=b'Track_Control_Modes', is_enabled=False)
        encoder_modes.add_mode(b'pan', [set_pan_controls])
        encoder_modes.add_mode(b'send_a', [partial(set_send_controls, 0)])
        encoder_modes.add_mode(b'send_b', [partial(set_send_controls, 1)])
        encoder_modes.add_mode(b'send_c', [partial(set_send_controls, 2)])
        encoder_modes.selected_mode = b'pan'
        ...
        self._translation_selector = ChannelTranslationSelector(name=b'Global_Translations')
    ...
    def _product_model_id_byte(self):
        return 115
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/APC40/APC40.pyc
