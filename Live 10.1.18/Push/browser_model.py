...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push\browser_model.py
...
class VirtualBrowserItem(object):
    ...
    def __init__(self, name=b'', children_query=nop, is_folder=False):
        self.name = name
...
class BrowserListItem(ActionListItem):
    ...
    """
    URI_TO_NAME_FALLBACK = {b'query:Synths': b'Instruments', 
       b'query:Drums': b'Drums', 
       b'query:UserLibrary': b'User Library', 
       b'query:Plugins': b'Plug-Ins'}
    ...
    @lazy_attribute
    def _item_name(self):
        item_name = os.path.splitext(self.content.name)[0] if self.content else b''
        ...
        if not can_be_displayed:
            uri = getattr(self.content, b'uri', b'')
...
class FullBrowserModel(BrowserModel):
    ...
    """
    empty_list_messages = [
     b'<no tags>', b'<no devices>', b'<no presets>', b'<no presets>']
    ...
    def _push_content_list(self):
        if self._num_contents < len(self._contents):
            ...
            slot = self.register_slot(content, partial(self._on_item_activated, level), b'item_activated')
...
class QueryingBrowserModel(FullBrowserModel):
    ...
    """
    empty_list_messages = [
     b'<no devices>', b'<no presets>', b'<no presets>', b'<no presets>']
    ...
    def exchange_model(self, model):
        if super(QueryingBrowserModel, self).exchange_model(model):
            ...
            return True
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push/browser_model.pyc
