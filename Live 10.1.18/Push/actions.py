...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push\actions.py
...
class CreateDefaultTrackComponent(Component, Messenger):
    ...
    def __init__(self, selection=None, *a, **k):
        ...
        self.options.selected_color = b'Browser.Load'
        self.options.unselected_color = b'Browser.Load'
        self._on_option_selected.subject = self.options
        self._selection = selection
        return

    @listens(b'selected_option')
...
class CreateInstrumentTrackComponent(Component, Messenger):
    ...
    def __init__(self, selection=None, browser_mode=None, browser_component=None, browser_hotswap_mode=None, *a, **k):
        ...
        self._with_browser_modes.add_mode(b'create', [
         self._prepare_browser,
         SetAttributeMode(self.application.browser, b'filter_type', Live.Browser.FilterType.instrument_hotswap),
         SetAttributeMode(browser_component, b'do_load_item', self._do_browser_load_item),
         browser_mode,
         browser_component.reset_load_memory])
        self._with_browser_modes.add_mode(b'hotswap', [browser_hotswap_mode, browser_mode])
    ...
    def on_enabled_changed(self):
        self._with_browser_modes.selected_mode = b'create' if self.is_enabled() else None
        self._go_to_hotswap_task.kill()
        return
    ...
    def _go_to_hotswap(self):
        self._with_browser_modes.selected_mode = b'hotswap'
...
class CreateDeviceComponent(Component):
    ...
    def __init__(self, selection=None, browser_component=None, browser_mode=None, browser_hotswap_mode=None, insert_left=False, *a, **k):
        ...
        self._create_device_modes.add_mode(b'create', [
         SetAttributeMode(browser_component, b'do_load_item', self._do_browser_load_item),
         self._add_effect_mode,
         browser_mode,
         browser_component.reset_load_memory])
        self._create_device_modes.add_mode(b'hotswap', [browser_hotswap_mode, browser_mode])
        self._go_to_hotswap_task = self._tasks.add(task.sequence(task.delay(1), task.run(self._go_to_hotswap)))
        self._go_to_hotswap_task.kill()
    ...
    def on_enabled_changed(self):
        ...
        if self.is_enabled():
            if is_empty_drum_pad(self._selection.selected_object):
                self._create_device_modes.selected_mode = b'hotswap'
            else:
                self._create_device_modes.selected_mode = b'create'
    ...
    def _do_browser_load_item(self, item):
        ...
        item.action()
        self._go_to_hotswap_task.restart()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push/actions.pyc
