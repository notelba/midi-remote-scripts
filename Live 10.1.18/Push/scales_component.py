...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push\scales_component.py
    ...
    def _update_data_sources(self, selected):
        if self.is_enabled():
            for name, (source, string) in self._mode_data_sources.iteritems():
                source.set_display_string(b'*' + string if name == selected else string)
...
class InstrumentPresetsComponent(DisplayingModesComponent):
    ...
    def __init__(self, note_layout=None, *a, **k):
        ...
        self._line_names = recursive_map(DisplayDataSource, (
         ('Scale layout:', ),
         ('4th ^', '4th >', '3rd ^', '3rd >', 'Sequent ^', 'Sequent >', '', '')))
        self.add_mode(b'scale_p4_vertical', partial(self._set_scale_mode, True, 3), self._line_names[1][0])
        self.add_mode(b'scale_p4_horizontal', partial(self._set_scale_mode, False, 3), self._line_names[1][1])
        self.add_mode(b'scale_m3_vertical', partial(self._set_scale_mode, True, 2), self._line_names[1][2])
        self.add_mode(b'scale_m3_horizontal', partial(self._set_scale_mode, False, 2), self._line_names[1][3])
        self.add_mode(b'scale_m6_vertical', partial(self._set_scale_mode, True, None), self._line_names[1][4])
        self.add_mode(b'scale_m6_horizontal', partial(self._set_scale_mode, False, None), self._line_names[1][5])
        return
...
class InstrumentScalesComponent(Component):
    presets_toggle_button = ButtonControl(color=b'DefaultButton.Off', pressed_color=b'DefaultButton.On')
    ...
    def __init__(self, note_layout=None, *a, **k):
        ...
        self._presets.selected_mode = b'scale_p4_vertical'
    ...
    presets_layer = forward_property(b'_presets')(b'layer')
    ...
    def _update_key_center_buttons(self):
        if self.is_enabled():
            for index, button in enumerate(self._key_center_buttons):
                if button:
                    color = b'Scales.Selected' if self._note_layout.root_note == ROOT_NOTES[index] else b'Scales.Unselected'
    ...
    def _update_absolute_relative_button(self):
        if self.is_enabled() and self._absolute_relative_button != None:
            color = b'Scales.FixedOn' if self._note_layout.is_fixed else b'Scales.FixedOff'
    ...
    def _update_diatonic_chromatic_button(self):
        if self.is_enabled() and self._diatonic_chromatic_button != None:
            color = b'Scales.Diatonic' if self._note_layout.is_in_key else b'Scales.Chromatic'
    ...
    def _update_data_sources(self):
        ...
        for idx, (source, orig) in enumerate(zip(key_sources, key_names)):
            source.set_display_string(b'   ' + consts.CHAR_SELECT + orig if idx == key_index else b'    ' + orig)

        self._line_sources[0][6].set_display_string(b'Fixed: Y' if self._note_layout.is_fixed else b'Fixed: N')
        self._line_sources[1][6].set_display_string(b'In Key' if self._note_layout.is_in_key else b'Chromatc')
        self._info_sources[1].set_display_string(str(self._scale_list.scrollable_list.selected_item))
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push/scales_component.pyc
