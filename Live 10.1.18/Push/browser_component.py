...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push\browser_component.py
...
def _memoized_stem_cleaner(stem):
    ...
    stem = re.escape(stem)
    rule1 = re.compile(b'([a-z])' + stem + b's?([ A-Z])')
    rule2 = re.compile(b'[' + ellipsis + b' \\-]' + stem + b's?([\\-' + ellipsis + b' A-Z])')
    rule3 = re.compile(b'' + stem + b's?$')

    def cleaner(short_name):
        short_name = b' ' + short_name
        short_name = rule1.sub(b'\\1' + ellipsis + b'\\2', short_name)
        short_name = rule2.sub(ellipsis + b'\\1', short_name)
        short_name = rule3.sub(ellipsis, short_name)
        return short_name.strip(b' ')
...
def split_stem(sentence):
    ...
    sentence = re.sub(b'([a-z])([A-Z])', b'\\1 \\2', sentence)
    return sentence.split()


_stripper_double_spaces = re.compile(b' [\\- ]*')
_stripper_double_ellipsis = re.compile(consts.CHAR_ELLIPSIS + b'+')
_stripper_space_ellipsis = re.compile(b'[\\- ]?' + consts.CHAR_ELLIPSIS + b'[\\- ]?')

def full_strip(string):
    """ Strip string for double spaces and dashes """
    string = _stripper_double_spaces.sub(b' ', string)
...
class BrowserComponent(Component):
    ...
    prehear_button = ToggleButtonControl(toggled_color=b'Browser.Prehear', untoggled_color=b'Browser.PrehearOff')
    ...
    def set_select_buttons(self, buttons):
        ...
        for component, button in izip(self._list_components, buttons[1::2]):
            self._set_button_if_enabled(component, b'action_button', button)

        for component, button in izip(self._list_components, buttons[::2]):
            if self.shift_button.is_pressed:
                self._set_button_if_enabled(component, b'prev_page_button', button)
                self._set_button_if_enabled(component, b'select_prev_button', None)
            else:
                self._set_button_if_enabled(component, b'prev_page_button', None)
                self._set_button_if_enabled(component, b'select_prev_button', button)
    ...
    def set_state_buttons(self, buttons):
        ...
        for component, button in izip(self._list_components, buttons[::2]):
            if self.shift_button.is_pressed:
                self._set_button_if_enabled(component, b'next_page_button', button)
                self._set_button_if_enabled(component, b'select_next_button', None)
            else:
                self._set_button_if_enabled(component, b'next_page_button', None)
                self._set_button_if_enabled(component, b'select_next_button', button)

        for button in buttons[1::2]:
            if button and self.is_enabled():
                button.set_light(b'DefaultButton.Disabled')
    ...
    def _item_formatter(self, depth, index, item, action_in_progress):
        ...
        if item:
            item_name = b'Loading...' if action_in_progress else self._shorten_item_name(shortening_limit, depth + self._scroll_offset, unicode(item))
            display_string = consts.CHAR_SELECT if item and item.is_selected else b' '
            ...
            if depth == 0 and self._scroll_offset > 0:
                prefix = consts.CHAR_ARROW_LEFT if index == 0 else b' '
    ...
    @listens(b'filter_type')
    def _on_filter_type_changed(self):
        self._update_browser_model()

    @listens(b'full_refresh')
    ...
    @listens_group(b'item_action')
    def _on_list_item_action(self, item, _):
        self.notify_load_item(item.content)

    @listens_group(b'selected_item')
    ...
    def _on_content_lists_changed(self):
        ...
        for idx, component in enumerate(head):
            ...
            set_data_sources_with_separator(component, sources, b'|')
        ...
        if last:
            ...
            set_data_sources_with_separator(last[0], sources, b'')
    ...
    def _on_encoder_matrix_value(self, value, *_):
        pass
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push/browser_component.pyc
