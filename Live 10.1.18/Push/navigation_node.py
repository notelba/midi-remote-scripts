...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push\navigation_node.py
...
def make_navigation_node(model_object, is_entering=True, session_ring=None, device_bank_registry=None, banking_info=None):
    ...
    if model_object == None:
        ...
        if model_object.can_have_chains:
            ...
            assert device_bank_registry, b'Navigating a device needs a bank registry'
...
class ChainNode(ModelNode):
    ...
    def _get_children_from_model(self):
        ...
        def expand_device(d):
            ...
            if drum_pad:
                drum_pad_name = drum_pad.name if len(drum_pad.chains) > 0 else b'EmptyPad'
                return [
                 (
    ...
    def _update_child_slots(self):
        ...
        for device in map(second, self.children):
            self._child_name_slots.register_slot(device, self._update_children, b'name')
            if isinstance(device, Live.DrumPad.DrumPad):
                self._child_state_slots.register_slot(device, partial(self._update_state, device), b'mute')
            elif isinstance(device, Live.Device.Device):
                if device.can_have_drum_pads:
                    self._selected_drum_pad_slots.register_slot(device.view, self._on_selected_drum_pad, b'selected_drum_pad')
                if device.parameters:
                    self._child_state_slots.register_slot(device.parameters[0], partial(self._update_state, device), b'value')
...
class SongNode(ModelNode):
    ...
    def __init__(self, session_ring=None, *a, **k):
        ...
        self.register_slot(self._object, self._update_children, b'visible_tracks')
        self.register_slot(self._object, self._update_children, b'return_tracks')
        self.register_slot(self._object.view, self._update_selected_child, b'selected_track')
        self._update_children()
        self._update_selected_child()
        return
...
class SimpleDeviceNode(ModelNode):
    ...
    def __init__(self, device_bank_registry=None, banking_info=None, *a, **k):
        ...
        assert device_bank_registry, b'Need a device bank registry.'
...
class RackNode(ModelNode):
    ...
    def __init__(self, *a, **k):
        ...
        self.register_slot(self._object, self._update_children, b'chains')
        self.register_slot(self._object.view, self._update_selected_child, b'selected_chain')
        self._update_children()
        self._update_selected_child()
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push/navigation_node.pyc
