...
BLANK_BLOCK = b' ' * DISPLAY_BLOCK_LENGTH
...
def adjust_arguments(format_string, original_arguments):
    ...
    for index, match in enumerate(matches):
        ...
        if has_markup is not None:
            ...
            if original_format_specifier != b's':
                if original_format_specifier.find(b'*') != -1:
                    raise ValueError(b'Format specifiers using * for field width/precision are not supported')
                original_argument = (b'%' + original_format_specifier) % original_argument
...
def apply_formatting(text_spec):
    ...
    if isinstance(text_spec, tuple):
        ...
        format_string = re.sub(FORMAT_SPECIFIER_WITH_MARKUP_PATTERN, b'%s', format_string)
...
class NotificationComponent(Component):
    ...
    def __init__(self, default_notification_time=2.5, blinking_time=0.3, display_lines=[], *a, **k):
        ...
        self._blink_text_task = self._tasks.add(task.loop(task.sequence(task.run(lambda : self._message_box.__setattr__(b'text', self._original_text)), task.wait(self._blinking_time), task.run(lambda : self._message_box.__setattr__(b'text', self._blink_text)), task.wait(self._blinking_time)))).kill()
        return

    message_box_layer = forward_property(b'_message_box')(b'layer')
    ...
    def use_full_display(self, message_line_index=2):
        ...
        if it is grabbed.
        """
        layer = Layer(priority=MESSAGE_BOX_PRIORITY, **dict([ (b'display_line1' if i == message_line_index else b'bg%d' % i, line) for i, line in enumerate(self._display_lines)
                                                            ]))
    ...
    def _create_tasks(self, notification_time):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push/notification_component.pyc
