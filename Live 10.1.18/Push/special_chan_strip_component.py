...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push\special_chan_strip_component.py
...
class SpecialChanStripComponent(components.ChannelStripComponent, Messenger):
    ...
    def __init__(self, *a, **k):
        ...
        self.empty_color = b'Option.Unused'
        self._invert_mute_feedback = True
        self._duplicate_button = None
        self._selector_button = None
        self._delete_handler = None
        self._track_parameter_name_sources = [ DisplayDataSource(b' ') for _ in xrange(14) ]
        self._track_parameter_data_sources = [ DisplayDataSource(b' ') for _ in xrange(14) ]
        self._track_parameter_graphic_sources = [ DisplayDataSource(b' ') for _ in xrange(14) ]
    ...
    def _update_control_sensitivities(self, control):
        if control:
            if hasattr(control, b'set_sensitivities'):
        ...
        if self.is_enabled():
            if self._track == None:
                ...
                if self._track == self.song.view.selected_track:
                    self.select_button.color = b'Mixer.ArmSelected'
                else:
                    self.select_button.color = b'Mixer.ArmUnselected'
            elif self._track == self.song.view.selected_track:
                self.select_button.color = b'Option.Selected'
            else:
                self.select_button.color = b'Option.Unselected'
        return
    ...
    def _update_track_name_data_source(self):
        if self._track_name_data_source:
            if liveobj_valid(self._track):
                ...
                prefix = consts.CHAR_SELECT if selected else b''
    ...
    def _disconnect_parameters(self):
        ...
        self._on_pan_control_value_changed.subject = None
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push/special_chan_strip_component.pyc
