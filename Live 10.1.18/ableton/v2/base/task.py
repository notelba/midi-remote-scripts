...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\base\task.py
...
class Task(object):
    ...
    def _set_parent(self, manager):
        if self._task_manager and manager:
            raise TaskError(b'Already attached to: ' + str(self._task_manager))
        ...
        for task in self._tasks:
            if not task.is_killed:
                try:
                    ...
                    task.kill()
                    logger.error(b'Error when executing task')
                    traceback.print_exc()
...
def totask(task):
    if not isinstance(task, Task):
        if not callable(task):
            raise TaskError(b'You can add either tasks or callables. ' + str(task))
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/base/task.pyc
