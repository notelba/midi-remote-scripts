...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\base\isclose.py
...
def isclose(a, b, rel_tol=1e-09, abs_tol=0.0):
    ...
    if rel_tol < 0.0 or abs_tol < 0.0:
        raise ValueError(b'error tolerances must be non-negative')
    if math.isinf(abs(a)) or math.isinf(abs(b)):
        return False
    diff = abs(b - a)
    return diff <= abs(rel_tol * b) or diff <= abs(rel_tol * a) or diff <= abs_tol
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/base/isclose.pyc
