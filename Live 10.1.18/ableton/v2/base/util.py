...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\base\util.py
...
def mixin(*args):
    ...
    if len(args) == 1:
        return args[0]
    name = b'Mixin_%s' % (b'_').join(cls.__name__ for cls in args)
...
def monkeypatch(target, name=None, override=False, doc=None):
    ...
    def patcher(func):
        ...
        if not override and hasattr(target, patchname):
            raise TypeError(b'Class %s already has method %s' % (target.__name__, patchname))
    ...
    def patcher(func):
        ...
        if hasattr(target, patchname):
            ...
            if not callable(oldfunc):
                raise TypeError(b'Can not extend non callable attribute')
            ...
            assert False, b'Must have something to extend'
...
def instance_decorator(decorator):
    ...
    class Decorator(object):
        ...
        def __init__(self, func=nop, *args, **kws):
            ...
            self._data_name = b'%s_%d_decorated_instance' % (func.__name__, id(self))
            self._func = func
            self._args = args
            self._kws = kws
...
def is_contextmanager(value):
    return callable(getattr(value, b'__enter__')) and callable(getattr(value, b'__exit__'))
...
class NamedTuple(object):
    ...
    def __init__(self, *others, **k):
        ...
        if b'_eq_dict' in self.__dict__:
            self._eq_dict.update(k)

    def __setattr__(self, name, value):
        raise AttributeError(b'Named tuple is constant')
    ...
    def _eq_dict(self):
        ...
        def public(objdict):
            return dict(filter(lambda (k, _): not k.startswith(b'_'), objdict.iteritems()))
...
class Slicer(object):
    ...
    def __getitem__(self, key):
        ...
        keys = self._keys + new
        assert len(keys) <= self._dimensions, b'Too many dimensions'
...
def trace_value(value, msg=b'Value: '):
    """
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/base/util.pyc
