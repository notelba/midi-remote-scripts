...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\message_scheduler.py
...
class MessageScheduler(object):
    ...
    def _process_request(self, request):
        if not (self._owner == None or self._owner == request.owner):
            ...
            if request.action == b'send' and (self._state == b'idle' or self._state == b'grabbed' and self._owner == request.owner):
    ...
    def disconnect(self, owner):
        if self._state != b'idle':
            ...
            self._process_queue()
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/message_scheduler.pyc
