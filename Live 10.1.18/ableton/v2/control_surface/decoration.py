...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\decoration.py
...
def get_parameter_automation_state(parameter):
    return getattr(parameter, b'automation_state', AutomationState.none)
...
class LiveObjectDict(dict):
    ...
    def _transform_key(self, key):
        assert hasattr(key, b'_live_ptr')
...
class PitchParameter(InternalParameter):
    ...
    def __init__(self, integer_value_host=None, decimal_value_host=None, *a, **k):
        ...
        self._integer_value = getattr(integer_value_host, b'value', 0)
        self._decimal_value = getattr(decimal_value_host, b'value', 0.0)
    ...
    def _combined_value(self):
        return getattr(self._integer_value_host, b'value', 0) + (getattr(self._decimal_value_host, b'value', 0.0) - 0.5)
    ...
    def _set_finegrain(self, new_value):
        if new_value < 0 or new_value > 1:
            ...
            new_value += offset
            self._set_coarse(getattr(self._integer_value_host, b'value', 0) - offset)
    ...
    @property
    def min(self):
        return getattr(self._integer_value_host, b'min', 0) - getattr(self._decimal_value_host, b'min', 0.0)

    @property
    def max(self):
        return getattr(self._integer_value_host, b'max', 1) + getattr(self._decimal_value_host, b'max', 1.0)

    @property
    def display_value(self):
        return (b'{0:.2f}st').format(self._combined_value)
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/decoration.pyc
