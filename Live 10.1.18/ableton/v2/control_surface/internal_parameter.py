...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\internal_parameter.py
...
class InternalParameter(InternalParameterBase):
    ...
    def _set_value(self, new_value):
        assert self.min <= new_value <= self.max, b'Invalid value %f' % new_value
...
class WrappingParameter(InternalParameter, PropertyHostMixin):
    ...
    def display_value(self):
        try:
            ...
            return unicode(self._display_value_conversion(value) if liveobj_valid(self._property_host) else b'')
        except RuntimeError:
            return unicode()
...
class IntegerParameter(InternalParameter):
    ...
    def _set_integer_value(self, new_value):
        assert self._min_value <= new_value <= self._max_value, (b'Invalid value {}').format(new_value)
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/internal_parameter.pyc
