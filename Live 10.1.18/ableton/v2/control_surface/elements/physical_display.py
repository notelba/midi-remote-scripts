...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\elements\physical_display.py
...
class DisplayElement(ControlElement):
    ...
    def __repr__(self):
        return b'<%s %r>' % (self.__class__.__name__, self.display_string)
...
class PhysicalDisplayElement(DisplayElement, NotifyingControlElement):
    ...
    def reset(self):
        if not (self._message_clear_all is not None or self._message_header is not None):
            ...
            if self._block_messages or self._message_clear_all != None:
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/elements/physical_display.pyc
