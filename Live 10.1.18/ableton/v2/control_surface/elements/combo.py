...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\elements\combo.py
...
class WrapperElement(CompoundElement, ProxyBase):
    ...
    class ProxiedInterface(CompoundElement.ProxiedInterface):
        ...
        def __getattr__(self, name):
            wrapped = self.outer.__dict__[b'_wrapped_control']
    ...
    def _is_initialized(self):
        return b'_wrapped_control' in self.__dict__
...
class ComboElement(WrapperElement):
    ...
    def get_control_element_priority(self, element, priority):
        if element == self._wrapped_control:
            assert priority is None or 1 - priority + int(priority) > self.priority_increment, b'Attempting to increase the priority over a whole unit. ' + b'Make sure the combo element is not inside another combo element'
...
class EventElement(NotifyingControlElement, ProxyBase, ButtonElementMixin):
    ...
    def proxied_interface(self):
        return getattr(self._subject, b'proxied_interface', self._subject)
    ...
    def reset(self):
        getattr(self._subject, b'reset', nop)()

    def send_value(self, *a, **k):
        try:
            send_value = super(EventElement, self).__getattr__(b'send_value')
    ...
    def set_light(self, *a, **k):
        try:
            set_light = super(EventElement, self).__getattr__(b'set_light')
        except AttributeError:
            set_light = nop
...
class DoublePressElement(WrapperElement):
    ...
    def single_press(self):
        return EventElement(self, b'single_press')

    @lazy_attribute
    def double_press(self):
        return EventElement(self, b'double_press')
...
class MultiElement(CompoundElement, ButtonElementMixin):
    ...
    def is_pressed(self):
        return find_if(lambda c: getattr(c, b'is_pressed', const(False))(), self.owned_control_elements()) != None

    def is_momentary(self):
        return find_if(lambda c: getattr(c, b'is_momentary', const(False))(), self.nested_control_elements()) != None
...
class ToggleElement(WrapperElement):
    ...
    def _update_toggled(self):
        ...
        if self._wrapped_control != None:
            self.register_control_element(self._wrapped_control)
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/elements/combo.pyc
