...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\control_surface.py
...
__all__ = (
 b'SimpleControlSurface', b'ControlSurface')
...
class SimpleControlSurface(EventObject):
    ...
    def process_midi_bytes(self, midi_bytes, midi_processor):
        ...
        if midi.is_sysex(midi_bytes):
            ...
            if result is not None:
                ...
                logger.warning(b'Got unknown sysex message: ' + midi.pretty_print_bytes(midi_bytes))
        else:
            ...
            if recipient is not None:
                ...
                logger.warning(b'Got unknown message: ' + midi.pretty_print_bytes(midi_bytes))
        return
    ...
    def _do_send_midi(self, midi_event_bytes):
        try:
            ...
            logger.error(b'Error while sending midi message %s', str(midi_event_bytes))
            traceback.print_exc()
            return False
    ...
    def preferences(self):
        ...
        if self.preferences_key is None:
            raise RuntimeError(b'Trying to access preferences without providing a preference_key')
    ...
    def _component_guard(self):
        with super(ControlSurface, self)._component_guard():
            with self._device_support_injector:
                yield
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/control_surface.pyc
