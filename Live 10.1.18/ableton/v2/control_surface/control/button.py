...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\control\button.py
...
__all__ = (
 b'ButtonControl', b'PlayableControl', b'ButtonControlBase', b'DoubleClickContext')
...
class ButtonControlBase(InputControl):
    ...
    double_clicked = control_event(b'double_clicked')

    class State(InputControl.State):
        """
        State-full representation of the Control.
        """
        disabled_color = control_color(b'DefaultButton.Disabled')
        ...
        def _on_released(self):
            self._call_listener(b'released')
            if self._repeat:
                self._repeat_task.kill()
            if self._has_delayed_event():
                if self._delay_task.is_running:
                    self._call_listener(b'released_immediately')
                    self._delay_task.kill()
                else:
                    self._call_listener(b'released_delayed')
            self._check_double_click_release()

        def _check_double_click_press(self):
            if self._has_listener(b'double_clicked') and not self._double_click_task.is_running:
        ...
        def _check_double_click_release(self):
            if self._has_listener(b'double_clicked') and self._double_click_task.is_running and self._double_click_context.control_state == self:
                self._double_click_context.click_count += 1
                if self._double_click_context.click_count == 2:
                    self._call_listener(b'double_clicked')
        ...
        def _repeat_task(self):
            notify_pressed = partial(self._call_listener, b'pressed')
        ...
        def _has_delayed_event(self):
            return self._has_listener(b'pressed_delayed') or self._has_listener(b'released_delayed') or self._has_listener(b'released_immediately')

        def _on_pressed_delayed(self):
            if self._is_pressed:
                self._call_listener(b'pressed_delayed')
...
class ButtonControl(ButtonControlBase):
    ...
    class State(ButtonControlBase.State):
        ...
        State-full representation of the Control.
        """
        color = control_color(b'DefaultButton.On')
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/control/button.pyc
