...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\resource.py
...
class ExclusiveResource(Resource):
    ...
    def grab(self, client, *a, **k):
        assert client is not None, b'Someone has to acquire resource'
        if self._owner == None:
    ...
    def on_received(self, client, *a, **k):
        raise NotImplementedError(b'Override or pass callback')
...
class SharedResource(Resource):
    ...
    def get_owner(self):
        assert False, b'Shared resource has no owner'
...
class ProxyResource(Proxy):
    ...
    def grab(self, client, *a, **k):
        self.__getattr__(b'grab')(self._client_wrapper.wrap(client), *a, **k)

    def release(self, client, *a, **k):
        self.__getattr__(b'release')(self._client_wrapper.wrap(client), *a, **k)

    @property
    def owner(self):
        return self._client_wrapper.unwrap(self.__getattr__(b'owner'))

    @property
    def owners(self):
        return map(self._client_wrapper.unwrap, self.__getattr__(b'owners'))
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/resource.pyc
