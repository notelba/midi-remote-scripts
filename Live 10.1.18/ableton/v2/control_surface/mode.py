...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\mode.py
...
def tomode(thing):
    if thing is None:
        ...
        if isinstance(thing, Mode):
            return thing
        if hasattr(thing, b'set_enabled'):
...
def to_camel_case_name(mode_name, separator=b''):
    return separator.join(map(lambda s: s.capitalize(), mode_name.split(b'_')))
...
class ModeButtonControl(ButtonControlBase):
    ...
    class State(ButtonControlBase.State):
        mode_selected_color = control_color(b'DefaultButton.On')
        mode_unselected_color = control_color(b'DefaultButton.Off')
        mode_group_active_color = control_color(b'DefaultButton.On')
...
class ModesComponent(Component):
    ...
    def add_mode_button_control(self, mode_name, behaviour):
        ...
        if self._enable_skinning:
            mode_color_basebame = b'Mode.' + to_camel_case_name(mode_name)
            colors = {b'mode_selected_color': mode_color_basebame + b'.On', 
               b'mode_unselected_color': mode_color_basebame + b'.Off', 
               b'mode_group_active_color': mode_color_basebame + b'.On'}
        button_control = make_mode_button_control(self, mode_name, behaviour, **colors)
        self.add_control(b'%s_button' % mode_name, button_control)
    ...
    def get_mode_button(self, name):
        return getattr(self, b'%s_button' % name)
...
class EnablingModesComponent(ModesComponent):
    ...
    def __init__(self, component=None, enabled_color=b'DefaultButton.On', disabled_color=b'DefaultButton.Off', *a, **k):
        super(EnablingModesComponent, self).__init__(*a, **k)
        component.set_enabled(False)
        self.add_mode(b'disabled', None, disabled_color)
        self.add_mode(b'enabled', component, enabled_color)
        self.selected_mode = b'disabled'
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/mode.pyc
