...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\components\playable.py
...
class PlayableComponent(Component):
    ...
    def _on_matrix_pressed(self, button):
        self.pressed_pads = apply_to_list(self.pressed_pads, b'append', button)
        if len(self.pressed_pads) == 1:
            self._update_control_from_script()

    def _on_matrix_released(self, button):
        if button in self.pressed_pads:
            self.pressed_pads = apply_to_list(self.pressed_pads, b'remove', button)
    ...
    def _update_button_color(self, button):
        button.color = b'DefaultButton.Off'
    ...
    def height(self):
        ...
        return 4
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/components/playable.pyc
