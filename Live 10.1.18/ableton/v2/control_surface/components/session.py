...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\components\session.py
...
class SessionComponent(Component):
    ...
    managed_select_button = ButtonControl(color=b'Session.Select', pressed_color=b'Session.SelectPressed')
    managed_delete_button = ButtonControl(color=b'Session.Delete', pressed_color=b'Session.DeletePressed')
    managed_duplicate_button = ButtonControl(color=b'Session.Duplicate', pressed_color=b'Session.DuplicatePressed')
    ...
    def __init__(self, session_ring=None, auto_name=False, *a, **k):
        ...
        self._stop_clip_triggered_value = b'Session.StopClipTriggered'
        self._stop_clip_value = b'Session.StopClip'
        self._stop_clip_disabled_value = b'Session.StopClipDisabled'
    ...
    def set_modifier_button(self, button, name, clip_slots_only=False):
        for y in xrange(self._session_ring.num_scenes):
            ...
            if not clip_slots_only:
                getattr(scene, (b'set_{}_button').format(name))(button)
            for x in xrange(self._session_ring.num_tracks):
                getattr(scene.clip_slot(x), (b'set_{}_button').format(name))(button)
    ...
    def _update_stop_clips_led(self, index):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/components/session.pyc
