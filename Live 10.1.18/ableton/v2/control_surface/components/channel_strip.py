...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\components\channel_strip.py
...
class ChannelStripComponent(Component):
    ...
    def __init__(self, *a, **k):
        ...
        def make_property_slot(name, alias=None):
            alias = alias or name
            return self.register_slot(None, getattr(self, b'_on_%s_changed' % alias), name)

        self._track_property_slots = [
         make_property_slot(b'mute'),
         make_property_slot(b'solo'),
         make_property_slot(b'arm'),
         make_property_slot(b'input_routing_type', b'input_routing'),
         make_property_slot(b'name', b'track_name')]
        self._mixer_device_property_slots = [
         make_property_slot(b'crossfade_assign', b'cf_assign'),
         make_property_slot(b'sends')]

        def make_button_slot(name):
            return self.register_slot(None, getattr(self, b'_%s_value' % name), b'value')

        self._mute_button_slot = make_button_slot(b'mute')
        self._solo_button_slot = make_button_slot(b'solo')
        self._arm_button_slot = make_button_slot(b'arm')
        self._shift_button_slot = make_button_slot(b'shift')
        self._crossfade_toggle_slot = make_button_slot(b'crossfade_toggle')
        return
    ...
    def disconnect(self):
        ...
        self._track_name_data_source.set_display_string(b'')
        self._mute_button = None
        self._solo_button = None
        self._arm_button = None
        self._shift_button = None
    ...
    def _update_track_name_data_source(self):
        self._track_name_data_source.set_display_string(self._track.name if liveobj_valid(self._track) else b' - ')
    ...
    def _update_select_button(self):
        if liveobj_valid(self._track) or self.empty_color == None:
            if self.song.view.selected_track == self._track:
                self.select_button.color = b'DefaultButton.On'
            else:
            ...
            for send_control in self._send_controls:
                if send_control != None:
                    if index < len(self._track.mixer_device.sends):
                        ...
                        self._empty_control_slots.register_slot(send_control, nop, b'value')
                index += 1
    ...
    def _disconnect_parameters(self):
        for control in self._all_controls():
            release_control(control)
            self._empty_control_slots.register_slot(control, nop, b'value')
        ...
        if self.is_enabled() and self._mute_button != None:
            if liveobj_valid(self._track) or self.empty_color == None:
                if self._track in chain(self.song.tracks, self.song.return_tracks) and self._track.mute != self._invert_mute_feedback:
                    self._mute_button.set_light(b'Mixer.MuteOff')
                else:
                    self._mute_button.set_light(b'Mixer.MuteOn')
            else:
        ...
        if self.is_enabled() and self._solo_button != None:
            if liveobj_valid(self._track) or self.empty_color == None:
                if self._track in chain(self.song.tracks, self.song.return_tracks) and self._track.solo:
                    self._solo_button.set_light(b'Mixer.SoloOn')
                else:
                    self._solo_button.set_light(b'Mixer.SoloOff')
            else:
        ...
        if self.is_enabled() and self._arm_button != None:
            if liveobj_valid(self._track) or self.empty_color == None:
                if self._track in self.song.tracks and self._track.can_be_armed and self._track.arm:
                    self._arm_button.set_light(b'Mixer.ArmOn')
                else:
                    self._arm_button.set_light(b'Mixer.ArmOff')
            else:
    ...
    def _on_input_routing_changed(self):
        if self.is_enabled():
            self._on_arm_changed()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/components/channel_strip.pyc
