...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\components\session_recording.py
...
class SessionRecordingComponent(Component):
    ...
    """
    automation_button = ToggleButtonControl(toggled_color=b'Automation.On', untoggled_color=b'Automation.Off')
    re_enable_automation_button = ButtonControl(color=b'Automation.On')
    delete_automation_button = ButtonControl(color=b'Automation.Off')
    record_button = ButtonControl(color=b'Recording.Off')
    ...
    def __init__(self, view_controller=None, *a, **k):
        ...
        self.register_slot(song, self.update, b'overdub')
        self.register_slot(song, self.update, b'session_record_status')
        self.register_slot(song, self._update_record_button, b'session_record')
        self.register_slot(song.view, self.update, b'selected_track')
        self.register_slot(song.view, self.update, b'selected_scene')
        self.register_slot(song.view, self.update, b'detail_clip')
    ...
    @listens(b're_enable_automation_enabled')
    ...
    @listens(b'session_automation_record')
    ...
    def _update_generic_new_button(self, new_button):
        if new_button and self.is_enabled():
            ...
            new_button.set_light(b'DefaultButton.On' if can_new else b'DefaultButton.Disabled')
    ...
    def _update_new_scene_button(self):
        if self._new_scene_button and self.is_enabled():
            ...
            self._new_scene_button.set_light(b'DefaultButton.On' if can_new else b'DefaultButton.Disabled')
    ...
    def _update_record_button(self):
        if self.is_enabled():
            ...
            if status == Live.Song.SessionRecordStatus.transition:
                self.record_button.color = b'Recording.Transition'
            elif status == Live.Song.SessionRecordStatus.on or song.session_record:
                self.record_button.color = b'Recording.On'
            else:
                self.record_button.color = b'Recording.Off'

    @listens(b'has_envelopes')
    ...
    def _update_delete_automation_button_color(self):
        if self._on_playing_clip_has_envelopes_changed.subject:
            self.delete_automation_button.color = b'Automation.On' if self._on_playing_clip_has_envelopes_changed.subject.has_envelopes else b'Automation.Off'
    ...
    def _view_selected_clip_detail(self):
        ...
        if self._view_controller is not None:
            self._view_controller.show_view(b'Detail/Clip')
        return
    ...
    def _reconnect_track_listeners(self):
        ...
        for track in self.song.tracks:
            if track.can_be_armed:
                manager.register_slot(track, self.update, b'arm')
                manager.register_slot(track, self.update, b'playing_slot_index')
                manager.register_slot(track, self.update, b'fired_slot_index')
    ...
    def scene_list_mode(self, scene_list_mode):
        ...
        self._update_new_button()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ableton/v2/control_surface/components/session_recording.pyc
