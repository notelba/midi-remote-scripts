...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\KeyPad\KeyPad.py
...
class KeyPad(ControlSurface):
    ...
    def _create_controls(self):
        ...
        self._encoders = make_controls(make_encoder, b'Encoder_%d', 57)
        self._rotaries_a = make_controls(make_slider, b'Rotary_A%d', 89)
        self._rotaries_b = make_controls(make_slider, b'Rotary_B%d', 97)
        self._faders = make_controls(make_slider, b'Fader_%d', 0)
        self._mute_buttons = make_controls(make_non_momentary_button, b'Mute_%d_Button', 8)
        self._solo_buttons = make_controls(make_button, b'Solo_%d_Button', 24)
        self._arm_buttons = make_controls(make_button, b'Arm_%d_Button', 40)
        self._play_button = make_button(1, 105, b'Play_Button')
        self._stop_button = make_button(1, 106, b'Stop_Button')
        self._record_button = make_button(1, 107, b'Record_Button')
        self._encoder_pushes = make_controls_range(partial(make_button, is_momentary=False), b'Encoder_%d_Button', self._encoder_range)
        self._shifted_mute_buttons = make_controls(make_non_momentary_button, b'Shifted_Mute_%d_Button', 16)
        self._shifted_solo_buttons = make_controls(make_button, b'Shifted_Solo_%d_Button', 32)
        self._all_shifted_arm_buttons = make_controls(make_button, b'Shifted_Arm_%d_Button', 49)
        ...
        self._shifted_play_button = make_button(1, 108, b'Shifted_Play_Button')
        self._shifted_stop_button = make_button(1, 109, b'Shifted_Stop_Button')
        self._shifted_record_button = make_button(1, 110, b'Shifted_Record_Button')
        self._shifted_octave_down_button = make_button(1, 111, b'Shifted_Octave_Down_Button')
        self._shifted_octave_up_button = make_button(1, 112, b'Shifted_Octave_Up_Button')
    ...
    def _setup_mixer(self):
        ...
        for index in xrange(NUM_CHANNEL_STRIPS):
            ...
            sends = ButtonMatrixElement(name=b'%d_Send_Controls' % (index + 1), rows=[
             (
    ...
    def _setup_session(self):
        self._session = SessionComponent(NUM_CHANNEL_STRIPS, name=b'Session_Control')
        self._session.set_enabled(False)
        stop_buttons = ButtonMatrixElement(name=b'Track_Stop_Buttons', rows=[self._shifted_solo_buttons])
        ...
        self._session.selected_scene().name = b'Selected_Scene_Control'
    ...
    def _setup_cue_control(self):
        self._cue_control = CuePointControlComponent(name=b'Cue_Point_Control')
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/KeyPad/KeyPad.pyc
