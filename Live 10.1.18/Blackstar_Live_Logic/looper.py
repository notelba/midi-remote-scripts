...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Blackstar_Live_Logic\looper.py
...
class LooperComponent(Component):
    ...
    def __init__(self, session_ring=None, *a, **k):
        ...
        self._is_clip_recording_slots = [ self.register_slot(None, partial(self._clip_state_changed, i), b'is_recording') for i in range(NUM_LOOPER_SWITCHES)
                                        ]
    ...
    def foot_switches(self, switch):
        if switch._double_click_context.click_count > 0:
            ...
            if not is_group_track(track):
                self._delete_target_for_track[track] = (fired_clip_slot(track) or self._get_controlled_clip_slot)(track) if 1 else None
    ...
    def _get_last_stopped_slot_with_clip(self, track):
        ...
        if has_clip(last_stopped):
            ...
            return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Blackstar_Live_Logic/looper.pyc
