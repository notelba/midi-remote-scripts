...
class NoteSetting(NoteSettingBase):
    ...
    def set_min_max(self, min_max_value):
        self.value_source.set_display_string(self.attribute_min_max_to_string(min_max_value[0], min_max_value[1]) if min_max_value else b'-')


RANGE_STRING_FLOAT = b'%.1f' + CHAR_ELLIPSIS + b'%.1f'
RANGE_STRING_INT = b'%d' + CHAR_ELLIPSIS + b'%d'
...
def step_offset_min_max_to_string(step_length, min_value, max_value):
    ...
    if min_value == max_value:
        return b'%d%%' % min_value
    return (RANGE_STRING_INT + b'%%') % (min_value, max_value)
...
class NoteLengthCoarseSetting(NoteSetting):
    ...
    def attribute_min_max_to_string(self, min_value, max_value):
        ...
        def format_string(value):
            ...
            return b'%%.%dg' % (num_non_decimal_figures + 2,)

        if min_value == max_value:
            return (format_string(min_value) + b' stp') % min_value
...
class NoteSettingsComponentBase(Component):
    ...
    def _add_setting(self, setting):
        assert len(self._settings) < 8, b'Cannot show more than 8 settings'
        self._settings.append(setting)
        self._update_encoders()
        self.register_disconnectable(setting)
        self.register_slot(setting, self.notify_setting_changed, b'setting_changed')
...
class DetailViewRestorerMode(Mode):
    ...
    def enter_mode(self):
        clip_view_visible = self._app.view.is_view_visible(b'Detail/Clip', False)
        device_chain_visible = self._app.view.is_view_visible(b'Detail/DeviceChain', False)
        if clip_view_visible != device_chain_visible:
            self._view_to_restore = b'Detail/Clip' if clip_view_visible else b'Detail/DeviceChain'
...
def show_clip_view(application):
    try:
        ...
        if view.is_view_visible(b'Detail/DeviceChain', False) and not view.is_view_visible(b'Detail/Clip', False):
            application.view.show_view(b'Detail/Clip')
    except RuntimeError:
        pass


class ModeSelector(Component):
    select_buttons = control_list(ButtonControl, color=b'DefaultButton.Disabled')
    state_buttons = control_list(ButtonControl, color=b'DefaultButton.Disabled')
...
class NoteEditorSettingsComponent(ModesComponent):
    ...
    automation_layer = forward_property(b'automation')(b'layer')
    mode_selector_layer = forward_property(b'_mode_selector')(b'layer')
    selected_setting = forward_property(b'_settings_modes')(b'selected_mode')
    ...
    def __on_selected_setting_mode_changed(self, mode):
        if mode == b'automation':
            self.automation.selected_time = self._active_note_regions()

    def update_view_state_based_on_selected_setting(self, setting):
        if self.selected_mode == b'enabled' and self.is_touched or setting is None:
            self._set_settings_view_enabled(False)
        elif self._is_step_held():
            if self.selected_setting == b'automation' and self.automation.can_automate_parameters or self.selected_setting == b'note_settings':
                self._show_settings()
        return

    @listens(b'full_velocity')
    def __on_full_velocity_changed(self):
        for editor in self._editors:
            editor.set_full_velocity()

    @listens(b'setting_changed')
    ...
    def _set_settings_view_enabled(self, should_show_view):
        really_show_view = should_show_view and self.automation.can_automate_parameters if self.selected_setting == b'automation' else should_show_view
        if really_show_view:
            if self.selected_mode == b'disabled':
                self.selected_mode = b'about_to_show'
        else:
            self._hide_settings()
    ...
    @listens_group(b'active_note_regions')
    ...
    def __on_modify_all_notes_changed(self, editor):
        self.selected_mode = b'about_to_show' if editor.modify_all_notes_enabled else b'disabled'

    @listens_group(b'notes_changed')
    ...
    def initial_encoders(self, encoder):
        if self.selected_mode == b'about_to_show':
            self._show_settings()
    ...
    def _can_notify_is_touched(self, encoder):
        if self.is_enabled():
            return self._settings_modes.selected_mode != b'note_settings' or encoder.index >= self.encoders.control_count - self.settings.number_of_settings
        return False
    ...
    def _update_note_infos(self):
        if self.settings.is_enabled():
            ...
            self.settings.set_info_message(b'Tweak to add note' if not self._is_edit_all_notes_active() and not min_max_values else b'')
        return

    def _show_settings(self):
        if self.selected_mode != b'enabled':
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/note_settings_component.pyc
