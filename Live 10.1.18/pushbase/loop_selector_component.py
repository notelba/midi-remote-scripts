...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\loop_selector_component.py
...
class LoopSelectorComponent(Component, Messenger):
    ...
    loop_selector_matrix = control_matrix(PadControl, sensitivity_profile=b'loop', mode=PlayableControl.Mode.listenable)
    ...
    def _update_page_and_playhead_leds(self):
        ...
        def replace_and_restore_tail_colors(page_colors, page):
            if clip_is_new_recording(self._sequencer_clip):
                ...
                page_colors[(page + 1):] = [b'LoopSelector.OutsideLoop'] * len(old_tail_values)
            yield
            if clip_is_new_recording(self._sequencer_clip):
            ...
            if 0 <= visible_page < len(page_colors):
                with save_page_color(page_colors, visible_page):
                    if self.song.is_playing:
                        page_colors[visible_page] = b'LoopSelector.PlayheadRecord' if self.song.session_record else b'LoopSelector.Playhead'
    ...
    def _update_page_colors(self):
        ...
        def mark_selected_pages(page_colors):
            for page_index in xrange(*self._selected_pages_range()):
                ...
                if page_colors[button_index].startswith(b'LoopSelector.InsideLoop'):
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/loop_selector_component.pyc
