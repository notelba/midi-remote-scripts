...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\scrollable_list.py
...
class DefaultItemFormatter(object):
    ...
    """
    action_message = b'Loading...'

    def __call__(self, index, item, action_in_progress):
        display_string = b''
        if item:
            display_string += consts.CHAR_SELECT if item.is_selected else b' '
...
class ListComponent(Component):
    ...
    def update(self):
        ...
        if self.is_enabled():
            self._update_action_feedback()
            self._update_display()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/scrollable_list.pyc
