...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\scrollable_list_component.py
...
class ScrollableListComponent(Component):
    ...
    def set_select_buttons(self, buttons):
        ...
        for b in self._select_buttons:
            self._select_button_slots.register_slot(b, self._on_select_value, b'value', extra_kws=dict(identify_sender=True))
    ...
    def _get_display_string(self, option_index):
        if option_index < len(self._option_names):
            decorator = consts.CHAR_SELECT if option_index == self.selected_option else b''
    ...
    def update(self):
        ...
        if self._offset_index > 0:
            ...
            if self._has_select_button(0):
                self._select_buttons[0].set_light(b'List.ScrollerOn')
            first_segment = 1
        if self._offset_index < self._maximal_offset():
            ...
            if self._has_select_button(-1):
                self._select_buttons[(-1)].set_light(b'List.ScrollerOn')
            max_segment -= 1
            ...
            if self._has_select_button(i):
                if i < len(self.option_names):
                    if j == self.selected_option:
                        self._select_buttons[i].set_light(b'Option.Selected')
                    else:
                        self._select_buttons[i].set_light(b'Option.Unselected')
                else:
                    self._select_buttons[i].set_light(b'Option.Unused')
...
class ScrollableListWithTogglesComponent(ScrollableListComponent):
    ...
    def __init__(self, *a, **k):
        ...
        def create_state_slot(idx):
            return self.register_slot(None, partial(self._on_state_button_value, idx), b'value')
    ...
    def _update_state_buttons(self):
        if not self.is_enabled():
            ...
            if buttons[0]:
                ...
                if self._offset_index > 0:
                    buttons[0].set_light(b'Option.Off')
                    first_button = 1
                if self._offset_index < self._maximal_offset():
                    buttons[(-1)].set_light(b'Option.Off')
                    max_button -= 1
                for state, button in zip(self._option_states[self._offset_index:], buttons[first_button:max_button]):
                    if button != None:
                        if state:
                            button.set_light(b'Option.On')
                        else:
                            button.set_light(b'Option.Off')
            ...
            return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/scrollable_list_component.pyc
