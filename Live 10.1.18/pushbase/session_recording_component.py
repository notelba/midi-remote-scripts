...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\session_recording_component.py
...
def have_other_recording_clips(tracks, recording_clip):
    for track in ifilter(lambda t: t.can_be_armed and (t.arm or t.implicit_arm), tracks):
        ...
        clip = getattr(slot, b'clip', None)
        if getattr(clip, b'is_recording', False) and clip is not recording_clip:
            return True
    ...
    def _update_record_button(self):
        if self.is_enabled():
            ...
            if liveobj_valid(clip_slot) and clip_slot.is_triggered and song.overdub and not clip_slot.is_recording:
                ...
                self.record_button.color = b'Recording.ArrangementRecordingOn'
            else:
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/session_recording_component.pyc
