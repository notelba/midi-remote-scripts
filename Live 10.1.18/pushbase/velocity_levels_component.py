...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\velocity_levels_component.py
...
class VelocityLevelsComponent(PlayableComponent):
    ...
    def __init__(self, velocity_levels=None, target_note_provider=None, skin_base_key=None, *a, **k):
        ...
        self.set_skin_base_key(skin_base_key or b'VelocityLevels')
    ...
    def _update_sensitivity_profile(self):
        profile = b'default' if self._takeover_pads else b'drums'
    ...
    @listens(b'selected_target_note')
    def __on_selected_target_note_changed(self):
        self.update()

    @listens(b'last_played_level')
    ...
    def _update_button_color(self, button):
        ...
        if index < len(levels) and self._played_level == levels[index]:
            ...
            if y == 0:
                ...
                color = b'LowLevel'
        button.color = self._skin_base_key + b'.' + color
    ...
    def update(self):
        ...
        if not self.is_enabled():
            self._notification_task.kill()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/velocity_levels_component.pyc
