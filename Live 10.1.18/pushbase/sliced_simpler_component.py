...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\sliced_simpler_component.py
...
class SlicedSimplerComponent(PlayableComponent, SlideableTouchStripComponent, SlideComponent, Slideable, Messenger):
    ...
    @listens(b'pad_slicing')
    def __on_pad_slicing_changed(self):
        self._update_led_feedback()

    @listens(b'slicing_style')
    ...
    def _update_button_color(self, button):
        ...
        if index < length_of_slices:
            button.color = b'SlicedSimpler.SliceSelected' if slices[index] == self._selected_slice() else b'SlicedSimpler.SliceUnselected'
        elif self._should_show_next_slice(index, length_of_slices):
            button.color = self._next_slice_color()
        else:
            button.color = b'SlicedSimpler.NoSlice'
    ...
    def _next_slice_color(self):
        return b'SlicedSimpler.NextSlice'
    ...
    def _try_quantize_notes_for_slice(self, index):
        self._quantizer.quantize_pitch(BASE_SLICING_NOTE + index, b'slice')
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/sliced_simpler_component.pyc
