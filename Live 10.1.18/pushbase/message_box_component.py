...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\message_box_component.py
...
FORMAT_SPECIFIER_WITH_MARKUP_PATTERN = re.compile(b'[%](len=([0-9]+),)?([^%]*?[diouxXeEfFgGcrs])')
...
def strip_restriction_markup_and_format(text_or_text_spec):
    if isinstance(text_or_text_spec, tuple):
        ...
        stripped_format_string = re.sub(FORMAT_SPECIFIER_WITH_MARKUP_PATTERN, b'%\\g<3>', format_string)
    ...
    def _update_cancel_button(self):
        if self.is_enabled():
            ...
            if self._can_cancel and button:
                button.set_light(b'MessageBox.Cancel')
        return

    def _update_display(self):
        if self._current_text != None:
            lines = self._current_text.split(b'\n')
            for source_line, line in izip_longest(self.data_sources, lines):
                if source_line:
                    source_line.set_display_string(line or b'')

            if self._can_cancel:
                self.data_sources[(-1)].set_display_string((b'[  Ok  ]').rjust(DISPLAY_LENGTH - 1))
        return
...
class InfoComponent(BackgroundComponent):
    ...
    def __init__(self, info_text=b'', *a, **k):
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/message_box_component.pyc
