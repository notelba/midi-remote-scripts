...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\action_with_options_component.py
...
class OptionsComponent(Component):
    ...
    unselected_color = b'Option.Unselected'
    selected_color = b'Option.Selected'
    ...
    def _update_data_sources(self):
        for index, (source, name) in enumerate(izip_longest(self._data_sources, self.option_names)):
            if name:
                source.set_display_string((consts.CHAR_SELECT if index == self._selected_option else b' ') + name)
            else:
...
class ToggleWithOptionsComponent(ActionWithOptionsComponent):
    ...
    def __init__(self, *a, **k):
        ...
        self.action_button.color = b'DefaultButton.Off'
    ...
    def _set_is_active(self, value):
        if value != self._is_active:
            ...
            self.action_button.color = b'DefaultButton.On' if value else b'DefaultButton.Off'
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/action_with_options_component.pyc
