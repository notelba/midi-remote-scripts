...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\push_base.py
...
class PushBase(ControlSurface):
    ...
    def _init_background(self):
        ...
        self._param_control_background = BackgroundComponent(is_enabled=False, add_nop_listeners=True, layer=Layer(global_param_controls=b'global_param_controls', priority=consts.BACKGROUND_PRIORITY))
        self._param_control_background.set_enabled(True)

    def _create_background_layer(self):
        return Layer(top_buttons=b'select_buttons', bottom_buttons=b'track_state_buttons', scales_button=b'scale_presets_button', octave_up=b'octave_up_button', octave_down=b'octave_down_button', side_buttons=b'side_buttons', repeat_button=b'repeat_button', accent_button=b'accent_button', double_button=b'double_button', param_touch=b'global_param_touch_buttons', touch_strip=b'touch_strip_control', nav_up_button=b'nav_up_button', nav_down_button=b'nav_down_button', nav_left_button=b'nav_left_button', nav_right_button=b'nav_right_button', new_button=b'new_button', aftertouch=b'aftertouch_control', _notification=self._notification.use_single_line(2), priority=consts.BACKGROUND_PRIORITY)
    ...
    def _create_alternating_layout_modes_for_levels_and_loop(self, mode=b'drum', add_touch_strip=False, default_mode=None, alternative_mode=None):
        ...
        base_layer = Layer(matrix=self.elements.matrix.submatrix[:4, 4:8], page_strip=b'touch_strip_control', scroll_strip=self._with_shift(b'touch_strip_control'), accent_button=b'accent_button', full_velocity=b'full_velocity_element') if add_touch_strip else Layer(matrix=self.elements.matrix.submatrix[:4, 4:8])
        ...
        alternating_layouts.add_mode(b'sequencer_loop', [
         sequencer,
         ...
         AddLayerMode(sequencer, Layer(loop_selector_matrix=self.elements.double_press_matrix.submatrix[
          4:8, 4:8], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[
          4:8, 4:8])),
        ...
        alternating_layouts.add_mode(b'sequencer_velocity_levels', [
         sequencer,
         self._note_editor_settings_component,
         velocity_levels,
    ...
    def _init_matrix_modes(self):
        ...
        self._note_modes.add_mode(b'drums', [self._drum_component, self._note_repeat_enabler, self._drum_modes])
        ...
        self._register_matrix_mode(b'disabled', parent_path=[b'matrix_modes', b'note'])
        ...
        self._register_matrix_mode(b'note', modes_component=self._note_modes, parent_path=[b'matrix_modes'])
    ...
    def _create_sequence_instrument_layer_with_loop(self):
        return Layer(playhead=b'playhead_element', mute_button=b'global_mute_button', quantization_buttons=b'side_buttons', loop_selector_matrix=self.elements.double_press_matrix.submatrix[:, 0], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:,
         0], note_editor_matrices=ButtonMatrixElement([[ self.elements.matrix.submatrix[:, 7 - row] for row in xrange(7) ]]), duplicate_button=b'duplicate_button', delete_button=b'delete_button')

    def _create_play_instrument_with_loop_layer(self):
        return Layer(playhead=b'playhead_element', mute_button=b'global_mute_button', quantization_buttons=b'side_buttons', loop_selector_matrix=self.elements.double_press_matrix.submatrix[:, 0], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:,
         0], duplicate_button=b'duplicate_button', delete_button=b'delete_button')
    ...
    def _init_split_melodic_sequencer(self):
        ...
        self._split_sequencer_mode.add_mode(b'alternative', [
         ...
         AddLayerMode(self._split_melodic_sequencer, Layer(loop_selector_matrix=self.elements.double_press_matrix.submatrix[:,
          4], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:,
          4]))], message=consts.MessageBoxText.LAYOUT_MELODIC_32_PADS_LOOP_SELECTOR, default_mode=b'default', alternative_mode=b'alternative')
    ...
    def process_midi_bytes(self, midi_bytes, midi_processor):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/push_base.pyc
