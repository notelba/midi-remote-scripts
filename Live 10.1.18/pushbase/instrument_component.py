...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\instrument_component.py
...
class NoteLayout(EventObject):
    ...
    def __init__(self, song=None, preferences=dict(), *a, **k):
        ...
        self._is_in_key = self._preferences.setdefault(b'is_in_key', True)
        self._is_fixed = self._preferences.setdefault(b'is_fixed', False)
        self._interval = self._song.get_data(b'push-note-layout-interval', 3)
        self._is_horizontal = self._song.get_data(b'push-note-layout-horizontal', True)
    ...
    def is_in_key(self, is_in_key):
        ...
        self._preferences[b'is_in_key'] = self._is_in_key
    ...
    def is_fixed(self, is_fixed):
        ...
        self._preferences[b'is_fixed'] = self._is_fixed
    ...
    def interval(self, interval):
        if interval != self._interval:
            ...
            self._song.set_data(b'push-note-layout-interval', interval)
    ...
    def is_horizontal(self, is_horizontal):
        if is_horizontal != self._is_horizontal:
            ...
            self._song.set_data(b'push-note-layout-horizontal', is_horizontal)
...
class InstrumentComponent(PlayableComponent, Slideable, Messenger):
    ...
    def show_pitch_range_notification(self):
        if self.is_enabled() and self.show_notifications:
            self.show_notification((b'Play {start_note} to {end_note}').format(start_note=pitch_index_to_string(self.pattern.note(0, 0).index), end_note=pitch_index_to_string(self.pattern.note(self.width - 1, self.height - 1).index)))
    ...
    def _update_button_color(self, button):
        ...
        button.color = b'Instrument.' + note_info.color
    ...
    def _set_button_control_properties(self, button):
        ...
        button.sensitivity_profile = b'default' if self._takeover_pads else b'instrument'
    ...
    def _update_aftertouch(self):
        if self.is_enabled() and self._aftertouch_control != None:
            self._aftertouch_control.send_value(b'mono')
        return
...
class SelectedNotesInstrumentComponent(InstrumentComponent):
    ...
    def __init__(self, note_editor_component=None, *a, **k):
        ...
        with self._updating_selected_notes_model():
            self.selected_notes_provider.selected_notes = self.song.view.selected_track.get_data(b'push-instrument-selected-notes', [DEFAULT_START_NOTE])
    ...
    def _get_color_for_button_in_selected_step(self, button):
        if button.identifier in self._note_editor_component.notes_in_selected_step:
            return b'Instrument.SelectedNote'
        return b'Instrument.' + self._get_note_info_for_coordinate(button.coordinate).color
    ...
    def _update_button_color(self, button):
        if self._is_note_editor_step_active():
            ...
            note_info = self._get_note_info_for_coordinate(button.coordinate)
            button.color = b'Instrument.SelectedNote' if note_info.index in self.selected_notes_provider.selected_notes else b'Instrument.' + note_info.color
    ...
    def _updating_selected_notes_model(self):
        yield
        self.song.view.selected_track.set_data(b'push-instrument-selected-notes', self.selected_notes_provider.selected_notes)
        self._update_led_feedback()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/instrument_component.pyc
