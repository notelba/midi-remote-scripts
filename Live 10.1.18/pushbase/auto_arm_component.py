...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\auto_arm_component.py
...
class AutoArmRestoreBehaviour(ModeButtonBehaviour):
    ...
    def update_button(self, component, mode, selected_mode):
        ...
        button.mode_selected_color = b'DefaultButton.Alert' if self._auto_arm.needs_restore_auto_arm else b'DefaultButton.On'
...
class RestoringAutoArmComponent(AutoArmComponent, Messenger):
    ...
    def _update_notification(self):
        if self.needs_restore_auto_arm:
            self._notification_reference = self.show_notification(b'  Press [Note] to arm the track:    ' + self.song.view.selected_track.name, blink_text=b'  Press        to arm the track:    ' + self.song.view.selected_track.name, notification_time=10.0)
        else:
    ...
    def _hide_notification(self):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/pushbase/auto_arm_component.pyc
