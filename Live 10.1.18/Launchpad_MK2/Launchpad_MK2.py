...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_MK2\Launchpad_MK2.py
...
class Launchpad_MK2(IdentifiableControlSurface, OptimizedControlSurface):
    ...
    def _create_controls(self):
        ...
        self._session_button_single = make_button(108, 0, b'Session_Mode_Button', msg_type=MIDI_CC_TYPE, is_modifier=True)
        self._session_button = FilteringMultiElement([
         self._session_button_single] + [ make_button(108, channel, msg_type=MIDI_CC_TYPE, name=b'Session_Mode_Button_ch_%d' % (channel,)) for channel in multi_button_channels
                                        ], feedback_channels=[
         0])
        self._user_1_button = FilteringMultiElement([ make_button(109, channel, msg_type=MIDI_CC_TYPE, name=b'User_1_Mode_Button_ch_%d' % (channel,)) for channel in (0, ) + multi_button_channels
                                                    ], feedback_channels=[
         0, 5, 6, 7])
        self._user_2_button = FilteringMultiElement([ make_button(110, channel, msg_type=MIDI_CC_TYPE, name=b'User_2_Mode_Button_ch_%d' % (channel,)) for channel in (0, ) + multi_button_channels
                                                    ], feedback_channels=[
         0, 13, 14, 15])
        self._mixer_button = FilteringMultiElement([ make_button(111, channel, msg_type=MIDI_CC_TYPE, name=b'Mixer_Mode_Button_ch_%d' % (channel,)) for channel in (0, ) + multi_button_channels
                                                   ], feedback_channels=[
         0])
        self._up_button = make_button(104, 0, msg_type=MIDI_CC_TYPE, name=b'Up_Button')
        self._down_button = make_button(105, 0, msg_type=MIDI_CC_TYPE, name=b'Down_Button')
        self._left_button = make_button(106, 0, msg_type=MIDI_CC_TYPE, name=b'Left_Button')
        self._right_button = make_button(107, 0, msg_type=MIDI_CC_TYPE, name=b'Right_Button')
        self._session_matrix_raw = [ [ make_button(col + offset, 0, name=b'Session_Matrix_Button_%d_%d' % (col, row)) for col in xrange(consts.SESSION_WIDTH) ] for row, offset in enumerate(xrange(81, 10, -10))
                                   ]
        self._session_matrix = ButtonMatrixElement(rows=self._session_matrix_raw, name=b'Session_Matrix')
        self._scene_launch_matrix_raw = [ make_button(identifier, 0, name=b'Scene_Launch_Button_%d' % (index,)) for index, identifier in enumerate(xrange(89, 18, -10))
                                        ]
        self._scene_launch_matrix = ButtonMatrixElement(rows=[
         self._scene_launch_matrix_raw], name=b'Scene_Launch_Buttons')
    ...
    def _create_user_controls(self):
        ...
        Max for Live.
        """
        self._user_1_matrix = ButtonMatrixElement(rows=[ [ make_button(identifier, USER_1_CHANNEL, name=b'User_1_Button_%d_%d' % (row_index, col_index)) for col_index, identifier in enumerate(row) ] for row_index, row in enumerate(USER_1_MATRIX_IDENTIFIERS)
                                                       ], name=b'User_1_Matrix')
        self._user_1_arrow_buttons = ButtonMatrixElement(rows=[
         [ make_button(identifier, USER_1_CHANNEL, msg_type=MIDI_CC_TYPE, name=b'User_1_Arrow_Button_%d' % (index,)) for index, identifier in enumerate(xrange(104, 108))
         ]], name=b'User_1_Arrow_Buttons')
        self._user_1_side_buttons = ButtonMatrixElement(rows=[ [make_button(identifier, USER_1_CHANNEL, name=b'User_1_Side_Button_%d' % (index,))] for index, identifier in enumerate(xrange(100, 108))
                                                             ], name=b'User_1_Side_Buttons')
        self._user_2_matrix = ButtonMatrixElement(rows=[ [ make_button(offset + col_index, USER_2_CHANNEL, name=b'User_2_Button_%d_%d' % (col_index, row_index)) for col_index in xrange(8) ] for row_index, offset in enumerate(xrange(81, 10, -10))
                                                       ], name=b'User_2_Matrix')
        self._user_2_arrow_buttons = ButtonMatrixElement(rows=[
         [ make_button(identifier, USER_2_CHANNEL, msg_type=MIDI_CC_TYPE, name=b'User_2_Arrow_Button_%d' % (index,)) for index, identifier in enumerate(xrange(104, 108))
         ]], name=b'User_2_Arrow_Buttons')
        self._user_2_side_buttons = ButtonMatrixElement(rows=[ [make_button(identifier, USER_2_CHANNEL, name=b'User_2_Side_Button_%d' % (index,))] for index, identifier in enumerate(xrange(89, 18, -10))
                                                             ], name=b'User_2_Side_Buttons')

    def _create_session(self):
        self._session = SessionComponent(is_enabled=False, num_tracks=self._session_matrix.width(), num_scenes=self._session_matrix.height(), enable_skinning=True, name=b'Session', is_root=True, layer=Layer(track_bank_left_button=self._left_button, track_bank_right_button=self._right_button, scene_bank_up_button=self._up_button, scene_bank_down_button=self._down_button))
    ...
    def _create_mixer(self):
        self._mixer = MixerComponent(is_enabled=False, num_tracks=consts.SESSION_WIDTH, invert_mute_feedback=True, enable_skinning=True, name=b'Mixer', is_root=True)
    ...
    def _create_translating_mixer_background(self, translation_channel):
        return TranslatingBackgroundComponent(translation_channel=translation_channel, is_enabled=False, name=b'Background', layer=Layer(stop_button=self._stop_button, mute_buttons=self._mute_button, solo_button=self._solo_button, record_arm_button=self._record_arm_button))
    ...
    def _create_modes(self):
        ...
        self._modes.add_mode(b'session_mode', [
         self._session, self._session_layer_mode], layout_byte=0, behaviour=EnablingReenterBehaviour(self._session_zoom))
        self._modes.add_mode(b'user_1_mode', [], layout_byte=1)
        self._modes.add_mode(b'user_2_mode', [], layout_byte=2)
        self._modes.add_mode(b'volume_mode', [
         self._session,
         self._create_translating_mixer_background(consts.VOLUME_MODE_CHANNEL),
         self._mixer_volume_layer], layout_byte=4, groups=set(b'mixer'))
        self._modes.add_mode(b'pan_mode', [
         self._session,
         self._create_translating_mixer_background(consts.PAN_MODE_CHANNEL),
         self._mixer_pan_layer], layout_byte=5, groups=set(b'mixer'))
        self._modes.add_mode(b'send_a_mode', [
         self._session,
         self._create_translating_mixer_background(consts.SEND_A_MODE_CHANNEL),
         self._mixer_send_a_layer], layout_byte=6, groups=set(b'mixer'))
        self._modes.add_mode(b'send_b_mode', [
         self._session,
         self._create_translating_mixer_background(consts.SEND_B_MODE_CHANNEL),
         self._mixer_send_b_layer], layout_byte=7, groups=set(b'mixer'))
        self._modes.add_mode(b'mixer_mode', [
         self._session, self._stop_clip_layer_mode, self._mixer_home_page_layer], layout_byte=3, groups=set(b'mixer'))
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Launchpad_MK2/Launchpad_MK2.pyc
