...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launch_Control\LaunchControl.py
...
def make_all_encoders(name_prefix=b'', make_encoder=make_launch_control_encoder):
    return (
     [ make_encoder(41 + index, name_prefix + b'_Bottom_Encoder_' + str(index)) for index in xrange(8)
     ],
     [ make_encoder(21 + index, name_prefix + b'_Top_Encoder_' + str(index)) for index in xrange(8)
     ])


pad_identifiers = [ 9 + i if i < 4 else 21 + i for i in xrange(8) ]
CC_STATUS = 176
MODE_SYSEX_MAP = {b'mixer': Sysex.MIXER_MODE, 
   b'session': Sysex.SESSION_MODE, 
   b'device': Sysex.DEVICE_MODE}
...
class LaunchControl(ControlSurface):
    ...
    def __init__(self, c_instance):
        ...
        self.log_message(b'Launch Control script loaded')
    ...
    def _init_mixer(self):
        ...
        bottom_encoders, top_encoders = make_all_encoders(b'Mixer', make_encoder)
        ...
        modes_layer = Layer(pan_volume_button=make_button(114, b'Pan_Volume_Mode_Button'), sends_button=make_button(115, b'Sends_Mode_Button'))
        ...
        self._mixer_track_nav_layer = Layer(track_bank_left_button=make_button(116, b'Mixer_Track_Left_Button'), track_bank_right_button=make_button(117, b'Mixer_Track_Right_Button'))
        for index in xrange(8):
            strip = self._mixer.channel_strip(index)
            strip.name = b'Channel_Strip_' + str(index)
            strip.empty_color = Colors.LED_OFF
            strip.set_invert_mute_feedback(True)
            mute_button = make_button(pad_identifiers[index], b'Track_Mute_Button_' + str(index), is_pad=True)
    ...
    def _init_session(self):
        ...
        bottom_encoders, top_encoders = make_all_encoders(b'Session_Mixer', make_encoder)
        pan_volume_layer = Layer(volume_controls=ButtonMatrixElement(rows=[bottom_encoders]), pan_controls=ButtonMatrixElement(rows=[top_encoders]))
        self._session_mixer = SpecialMixerComponent(8, Layer(), pan_volume_layer, Layer())
        self._session_mixer.set_enabled(False)
        self._session_mixer.name = b'Session_Mixer'
        clip_launch_buttons = [ make_button(identifier, b'Clip_Launch_Button_' + str(i), is_pad=True) for i, identifier in enumerate(pad_identifiers)
                              ]
        self._session = SpecialSessionComponent(num_tracks=8, num_scenes=0, name=b'Session')
        self._session.set_enabled(False)
        self._session.set_mixer(self._session_mixer)
        self._session_layer = Layer(track_bank_left_button=make_button(116, b'Track_Bank_Left_Button'), track_bank_right_button=make_button(117, b'Track_Bank_Right_Button'), select_prev_button=make_button(114, b'Scene_Bank_Up_Button'), select_next_button=make_button(115, b'Scene_Bank_Down_Button'), clip_launch_buttons=ButtonMatrixElement(rows=[clip_launch_buttons]))
    ...
    def _init_device(self):
        ...
        bottom_encoders, top_encoders = make_all_encoders(b'Device', make_encoder)
        parameter_controls = top_encoders[:4] + bottom_encoders[:4]
        bank_buttons = [ make_button(identifier, b'Device_Bank_Button_' + str(i), is_pad=True) for i, identifier in enumerate(pad_identifiers)
                       ]
        for button in bank_buttons:
            button.set_on_off_values(Colors.LED_ON, Colors.LED_OFF)

        self._device_bank_registry = DeviceBankRegistry()
        self._device = DeviceComponent(device_bank_registry=self._device_bank_registry, name=b'Device', device_selection_follows_track_selection=True)
        ...
        self._device_navigation.name = b'Device_Navigation'
        self._device_navigation.layer = Layer(next_device_button=make_button(115, b'Next_Device_Button'), previous_device_button=make_button(114, b'Prev_Device_Button'))
        self._view_control = ViewControlComponent()
        self._view_control.set_enabled(False)
        self._view_control.name = b'View_Control'
        self._view_control.layer = Layer(next_track_button=make_button(117, b'Device_Next_Track_Button'), prev_track_button=make_button(116, b'Device_Prev_Track_Button'))

    def _init_modes(self):
        self._modes = ModesComponent(is_root=True)
        self._modes.add_mode(b'mixer', [
        ...
        self._modes.add_mode(b'session', [
         ...
         self._session,
         self._show_controlled_tracks_message])
        self._modes.add_mode(b'device', [self._device, self._device_navigation, self._view_control])
        self._modes.add_mode(b'user', None)
        self._modes.selected_mode = b'mixer'
    ...
    @subject_slot(b'selected_send_index')
    def _on_selected_send_index(self, index):
        self._show_controlled_sends_message()

    @subject_slot(b'selected_mixer_mode')
    def _on_selected_mixer_mode(self, mode):
        if mode == b'sends':
            self._show_controlled_sends_message()
        else:
            self.show_message(b'Controlling Pan and Volume')
    ...
    def handle_sysex(self, midi_bytes):
        ...
        if self._is_user_mode_message(midi_bytes):
            self._modes.selected_mode = b'user'
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Launch_Control/LaunchControl.pyc
