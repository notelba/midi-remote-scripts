...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\APC40_MkII\MixerComponent.py
...
class ChannelStripComponent(ChannelStripComponentBase):
    ...
    def _on_cf_assign_changed(self):
        if self.is_enabled() and self._crossfade_toggle:
            ...
            value_to_send = None
            if state == 0:
                value_to_send = b'Mixer.Crossfade.A'
            elif state == 1:
                value_to_send = b'Mixer.Crossfade.Off'
            elif state == 2:
                value_to_send = b'Mixer.Crossfade.B'
...
class MixerComponent(MixerComponentBase):
    ...
    def _show_controlled_sends_message(self):
        if self._send_index is not None:
            send_name = chr(ord(b'A') + self._send_index)
            self._show_message(b'Controlling Send %s' % send_name)
        return
    ...
    def set_pan_controls(self, controls):
        ...
        if self.is_enabled() and controls:
            self._show_message(b'Controlling Pans')
    ...
    def set_user_controls(self, controls):
        ...
        if self.is_enabled() and controls:
            self._show_message(b'Controlling User Mappings')
    ...
    def update(self):
        ...
        if self.is_enabled():
            ...
            self._update_send_controls()
            self._update_user_controls()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/APC40_MkII/MixerComponent.pyc
