...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\APC40_MkII\APC40_MkII.py
...
class APC40_MkII(APC, OptimizedControlSurface):
    ...
    def _create_controls(self):
        ...
        self._mixer_encoders = ButtonMatrixElement(rows=[
         [ make_ring_encoder(48 + track, 56 + track, name=b'Track_Control_%d' % track) for track in xrange(NUM_TRACKS)
         ]])
        self._volume_controls = ButtonMatrixElement(rows=[
         [ make_slider(track, 7, name=b'%d_Volume_Control' % track) for track in xrange(NUM_TRACKS)
         ]])
        ...
        self._arm_buttons = ButtonMatrixElement(rows=[
         [ make_on_off_button(channel, 48, name=b'%d_Arm_Button' % channel) for channel in xrange(NUM_TRACKS)
         ]])
        self._solo_buttons = ButtonMatrixElement(rows=[
         [ make_on_off_button(channel, 49, name=b'%d_Solo_Button' % channel) for channel in xrange(NUM_TRACKS)
         ]])
        self._mute_buttons = ButtonMatrixElement(rows=[
         [ make_on_off_button(channel, 50, name=b'%d_Mute_Button' % channel) for channel in xrange(NUM_TRACKS)
         ]])
        self._crossfade_buttons = ButtonMatrixElement(rows=[
         [ make_button(channel, 66, name=b'%d_Crossfade_Button' % channel, skin=self._crossfade_button_skin) for channel in xrange(NUM_TRACKS)
         ]])
        ...
        self._device_controls = ButtonMatrixElement(rows=[
         [ make_ring_encoder(16 + index, 24 + index, name=b'Device_Control_%d' % index) for index in xrange(8)
         ]])
    ...
    def _create_mixer(self):
        ...
        self._encoder_mode.add_mode(b'pan', [AddLayerMode(self._mixer, Layer(pan_controls=self._mixer_encoders))])
        ...
        self._encoder_mode.add_mode(b'user', [AddLayerMode(self._mixer, Layer(user_controls=self._mixer_encoders))])
    ...
    def _product_model_id_byte(self):
        return 41
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/APC40_MkII/APC40_MkII.pyc
