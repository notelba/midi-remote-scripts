...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Roland_A_PRO\Roland_A_PRO.py
...
class Roland_A_PRO(ControlSurface):
    ...
    def _create_controls(self):
        self._encoders = ButtonMatrixElement(rows=[
         [ EncoderElement(MIDI_CC_TYPE, 0, identifier, Live.MidiMap.MapMode.absolute, name=b'Encoder_%d' % index) for index, identifier in enumerate(ENCODER_IDS)
         ]])
        self._master_encoder = EncoderElement(MIDI_CC_TYPE, 0, 10, Live.MidiMap.MapMode.absolute, name=b'Master_Encoder')
        self._sliders = ButtonMatrixElement(rows=[
         [ SliderElement(MIDI_CC_TYPE, 0, identifier, name=b'Slider_%d' % index) for index, identifier in enumerate(SLIDER_IDS)
         ]])
        self._master_slider = SliderElement(MIDI_CC_TYPE, 0, 7, name=b'Master_Slider')
    ...
    def _create_transport(self):
        self._transport = TransportComponent(name=b'Transport', is_enabled=False, layer=Layer(play_button=self._play_button, stop_button=self._stop_button, record_button=self._record_button))
        self._transport.set_enabled(True)

    def _create_mixer(self):
        mixer_size = self._sliders.width()
        self._mixer = MixerComponent(mixer_size, name=b'Mixer', is_enabled=False, layer=Layer(volume_controls=self._sliders, prehear_volume_control=self._master_encoder))
    ...
    def _create_drums(self):
        self._drums = DrumRackComponent(name=b'Drum_Rack', is_enabled=False, layer=Layer(pads=self._pads))
    ...
    def _create_modes(self):
        ...
        self._encoder_modes.add_mode(b'device_mode', [device_layer_mode, device_navigation_layer_mode])
        self._encoder_modes.add_mode(b'pan_mode', AddLayerMode(self._mixer, Layer(pan_controls=self._encoders, bank_up_button=self._forward_button, bank_down_button=self._backward_button, track_up_button=self._ff_button, track_down_button=self._rw_button)))
        ...
        self._encoder_modes.add_mode(b'send_a_mode', [send_layer_mode, partial(self._set_send_index, 0)])
        self._encoder_modes.add_mode(b'send_b_mode', [send_layer_mode, partial(self._set_send_index, 1)])
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Roland_A_PRO/Roland_A_PRO.pyc
