...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\APC_Key_25\APC_Key_25.py
...
class APC_Key_25(APC, OptimizedControlSurface):
    ...
    def _create_controls(self):
        ...
        self._shift_button = make_button(0, 98, resource_type=SharedResource, name=b'Shift_Button')
        self._parameter_knobs = [ make_knob(0, index + 48, name=b'Parameter_Knob_%d' % (index + 1)) for index in xrange(self.SESSION_WIDTH)
                                ]
        self._select_buttons = [ make_stop_button(0, 64 + index, name=b'Track_Select_%d' % (index + 1)) for index in xrange(self.SESSION_WIDTH)
                               ]
        ...
        self._matrix_buttons = [ [ make_color_button(0, matrix_note(track, scene), name=b'%d_Clip_%d_Button' % (track, scene)) for track in xrange(self.SESSION_WIDTH) ] for scene in xrange(self.SESSION_HEIGHT)
                               ]
        self._session_matrix = ButtonMatrixElement(name=b'Button_Matrix', rows=self._matrix_buttons)
        self._scene_launch_buttons = [ make_color_button(0, index + 82, name=b'Scene_Launch_%d' % (index + 1)) for index in xrange(self.SESSION_HEIGHT)
                                     ]
    ...
    def _make_stop_all_button(self):
        return make_button(0, 81, name=b'Stop_All_Clips_Button')
    ...
    def _create_device_component(self):
        return DeviceComponent(name=b'Device_Component', is_enabled=False, device_selection_follows_track_selection=True)
    ...
    def _create_transport(self):
        ...
        return TransportComponent(name=b'Transport', is_enabled=False, play_toggle_model_transform=play_toggle_model_transform, layer=Layer(play_button=self._play_button, record_button=self._record_button))

    def _create_encoder_modes(self):
        knob_modes = ModesComponent(name=b'Knob Modes', is_enabled=False)
        parameter_knobs_matrix = self.wrap_matrix(self._parameter_knobs)
        send_toggle_component = SendToggleComponent(self._mixer, name=b'Toggle Send', is_enabled=False, layer=Layer(toggle_button=self._send_button, priority=1))
        knob_modes.add_mode(b'volume', AddLayerMode(self._mixer, Layer(volume_controls=parameter_knobs_matrix)))
        knob_modes.add_mode(b'pan', AddLayerMode(self._mixer, Layer(pan_controls=parameter_knobs_matrix)))
        knob_modes.add_mode(b'send', [
         AddLayerMode(self._mixer, Layer(send_controls=parameter_knobs_matrix)),
         send_toggle_component])
        knob_modes.add_mode(b'device', AddLayerMode(self._device, Layer(parameter_controls=parameter_knobs_matrix)))
        knob_modes.selected_mode = b'volume'
    ...
    def _create_track_button_modes(self):
        track_button_modes = ModesComponent(name=b'Track Button Modes', is_enabled=False)
        select_button_matrix = self.wrap_matrix(self._select_buttons)
        track_button_modes.add_mode(b'clip_stop', AddLayerMode(self._session, Layer(stop_track_clip_buttons=select_button_matrix)))
        track_button_modes.add_mode(b'solo', AddLayerMode(self._mixer, layer=Layer(solo_buttons=select_button_matrix)))
        track_button_modes.add_mode(b'arm', AddLayerMode(self._mixer, layer=Layer(arm_buttons=select_button_matrix)))
        track_button_modes.add_mode(b'mute', AddLayerMode(self._mixer, layer=Layer(mute_buttons=select_button_matrix)))
        track_button_modes.add_mode(b'select', AddLayerMode(self._mixer, layer=Layer(track_select_buttons=select_button_matrix)))
        track_button_modes.selected_mode = b'clip_stop'
    ...
    def _on_handshake_successful(self):
        pass
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/APC_Key_25/APC_Key_25.pyc
