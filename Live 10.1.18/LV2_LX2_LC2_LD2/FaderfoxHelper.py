...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\LV2_LX2_LC2_LD2\FaderfoxHelper.py
...
class FaderfoxHelper:
    ...
    def switch_crossfader_ab(self, track):
        if hasattr(track.mixer_device, b'crossfade_assign'):
    ...
    def device_name(self, device):
        if hasattr(device, b'class_name'):
    ...
    def track_find_last_eq(self, track):
        ...
        def is_eq(device):
            return self.device_name(device) == b'Eq8' or self.device_name(device) == b'FilterEQ3'
    ...
    def eq_params(self, eq):
        if self.device_name(eq) == b'Eq8':
            return [ self.get_parameter_by_name(eq, name) for name in [
             b'3 Gain A', b'4 Gain A', b'5 Gain A', b'6 Gain A']
                   ]
        else:
            if self.device_name(eq) == b'FilterEQ3':
                return [ self.get_parameter_by_name(eq, name) for name in [
                 b'GainLo', b'GainMid', b'GainHi', b'']
                       ]
            else:
                return [
                 None, None, None, None]
    ...
    def get_parameter_by_name(self, device, name):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/LV2_LX2_LC2_LD2/FaderfoxHelper.pyc
