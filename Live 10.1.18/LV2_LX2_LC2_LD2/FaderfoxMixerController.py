...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\LV2_LX2_LC2_LD2\FaderfoxMixerController.py
...
class FaderfoxMixerController(FaderfoxComponent):
    ...
    def receive_midi_note(self, channel, status, note_no, note_vel):
        ...
        if channel == CHANNEL_SETUP2:
            self.log(b'received note %s' % note_no)
        ...
        if channel == CHANNEL_SETUP2 and note_no == MASTER_TRACK_SELECT_NOTE:
            self.lv1_track_idx = note_no
            self.log(b'select master track')
        ...
        if channel == TRACK_CHANNEL_SETUP2 and status == NOTEON_STATUS:
            ...
            self.handle_status_note(note_no, CROSS_AB_NOTES, b'cross_ab')
    ...
    def on_track_selected(self):
        ...
        if self.helper.is_master_track_selected():
            ...
            if idx < 16:
                ...
                if self.parent.is_live_5():
                    self.log(b'send track note %s' % TRACK_SELECT_NOTES[self.helper.selected_track_idx()])
                    self.parent.send_midi((
    ...
    def map_track_params(self, script_handle, midi_map_handle):
        ...
        if hasattr(track.mixer_device, b'cue_volume'):
            parameter = track.mixer_device.cue_volume
            cc = CUE_VOLUME_CC
            if self.parent.is_lv1:
                cc = LV1_CUE_VOLUME_CC
            ParamMap.map_with_feedback(midi_map_handle, CHANNEL_SETUP2, cc, parameter, Live.MidiMap.MapMode.absolute)
        if hasattr(track.mixer_device, b'crossfader'):
            ...
            if self.parent.is_lv1:
                ...
                ParamMap.map_with_feedback(midi_map_handle, CHANNEL_SETUP2, cc, parameter, Live.MidiMap.MapMode.absolute)
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/LV2_LX2_LC2_LD2/FaderfoxMixerController.pyc
