...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\LV2_LX2_LC2_LD2\FaderfoxTransportController.py
...
class FaderfoxTransportController(FaderfoxComponent):
        ...
        if status == NOTEON_STATUS:
            if channel == CHANNEL_SETUP2:
                if note_no == SCENE_LAUNCH_NOTE:
                    ...
                    if view.is_view_visible(b'Session'):
                        view.show_view(b'Arranger')
                    else:
                        view.show_view(b'Session')
                elif note_no == CLIP_TRACK_SWITCH_NOTE:
                    view = self.parent.application().view
                    if view.is_view_visible(b'Detail/Clip'):
    ...
    def disconnect(self):
        pass
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/LV2_LX2_LC2_LD2/FaderfoxTransportController.pyc
