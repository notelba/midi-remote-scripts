...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\LV2_LX2_LC2_LD2\FaderfoxDeviceController.py
...
class FaderfoxDeviceController(FaderfoxComponent):
    ...
    def realinit(self, parent):
        ...
        self.log(b'device controller %s' % parent)
        self.device = None
        if hasattr(self.parent.song(), b'appointed_device'):
            self.device = self.parent.song().appointed_device
        self.log(b'device %s' % self.device)
        ...
        def map_params_by_name(device, param_bank):
            ...
            for encoder in range(8):
                if len(params) >= encoder:
                    ...
                    if parameter:
                        mode2 = mode
                        fullname = self.helper.device_name(device) + b'.' + parameter.name
                        ...
                        self.logfmt(b'parameter %s %s to %s (quant %s)', parameter, parameter.name, ccs[encoder], parameter.is_quantized)
                        ParamMap.map_with_feedback(midi_map_handle, channel, ccs[encoder], parameter, mode2)
                    else:
                        self.log(b'Could not find parameter %s' % param_bank[encoder])

            return

        self.log(b'map device params %s' % self.device)
        if self.device:
            params = self.device.parameters
            device_bank = 0
            param_bank = 0
            device_name = self.helper.device_name(self.device)
            self.log(b"device name '%s'" % device_name)
            ...
            if device_name in DEVICE_BOB_DICT.keys():
                ...
                if device_name == b'Compressor2' and self.helper.get_parameter_by_name(self.device, b'S/C Gain'):
                    param_bank = CP2_BANK1_LIVE7
                self.log(b'class %s bank: %s' % (device_name, param_bank))
                self.show_bank_select(b'Best of parameters')
                map_params_by_name(self.device, param_bank)
            elif self.helper.device_is_plugin(self.device):
                self.show_bank_select(b'First eight parameters')
                map_params_by_number(self.device)
            else:
                self.log(b'Could not find %s in %s' % (device_name, DEVICE_BOB_DICT.keys()))
                return
    ...
    def show_bank_select(self, bank_name):
        if self.show_bank:
            self.show_bank = False
            if self.device:
                self.parent.show_message(str(self.device.name + b' Bank: ' + bank_name))
    ...
    def receive_midi_note(self, channel, status, note_no, note_vel):
        ...
        if not self.device:
            ...
            if channel == CHANNEL_SETUP2 and status == NOTEON_STATUS:
                ...
                self.log(b'device %s, index %s, parameter %s' % (self.device, idx, parameter.name))
            return
    ...
    def on_device_selected(self):
        self.log(b'selected device %s' % self.selected_track.view.selected_device)
        self.set_appointed_device(self.selected_track.view.selected_device)

    def on_track_selected(self):
        if self.parent.is_live_5():
            self.log(b'add a listener to selected device on track %s' % self.parent.song().view.selected_track)
            self.selected_track = self.parent.song().view.selected_track
        self.parent.request_rebuild_midi_map()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/LV2_LX2_LC2_LD2/FaderfoxDeviceController.pyc
