...
LIMITATION_ERROR_MESSAGE = b'Print to clip failed: No more scenes can be inserted for this version of Live.'
PACKET_ERROR_MESSAGE = b'Print to clip failed: Transmission error detected. Please try again.'
...
class PrintToClipComponent(Component):
    ...
    def _handle_data_packet(self, data_bytes):
        ...
        if len(payload) == BYTES_PER_GROUP_OFFSET:
            self._clip_data[b'length'] = to_absolute_beat_time(payload)
        else:
            ...
            if payload_length % BYTES_PER_NOTE == 0:
                self._clip_data[b'notes'].extend([ create_note(payload[i:i + BYTES_PER_NOTE], group_offset) for i in xrange(0, payload_length, BYTES_PER_NOTE)
                                                 ])
    ...
    def _print_data_to_clip(self):
        if b'length' in self._clip_data:
            clip = self._create_clip(self._clip_data[b'length'])
            if liveobj_valid(clip):
                self._wrap_trailing_notes()
                clip.set_notes(tuple(sorted(self._clip_data[b'notes'], key=itemgetter(1))))
    ...
    def _wrap_trailing_notes(self):
        for note in self._clip_data[b'notes'][:]:
            note_end_position = note[Note.start] + note[Note.length]
            if note_end_position > self._clip_data[b'length']:
                wrapped_note_length = note_end_position - self._clip_data[b'length'] + WRAPPED_NOTE_OFFSET
                self._clip_data[b'notes'].append((
                 note[Note.pitch],
                 -WRAPPED_NOTE_OFFSET,
                 wrapped_note_length,
                 note[Note.velocity],
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/novation/print_to_clip.pyc
