...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\KeyLab_Essential\keylab_essential.py
...
class KeyLabEssential(ControlSurface):
    ...
    def _create_hardware_settings(self):
        self._hardware_settings = self.hardware_settings_component_type(is_enabled=False, layer=Layer(hardware_live_mode_switch=self._hardware_live_mode_switch, memory_preset_switch=self._memory_preset_switch, memory_preset_select_mode_switch=self._memory_preset_select_mode_switch), name=b'Hardware_Settings')
    ...
    def _create_session(self):
        self._session_ring = SessionRingComponent(num_tracks=self._pads.width(), num_scenes=self._pads.height(), name=b'Session_Ring')
        self._session_ring.set_enabled(False)
        self._session = self.session_component_type(session_ring=self._session_ring, name=b'Session', is_enabled=False, layer=Layer(clip_launch_buttons=self._pads, clip_slot_leds=self._pad_leds))
        self._session.set_enabled(True)

    def _create_navigation(self):
        self._session_navigation = SessionNavigationComponent(session_ring=self._session_ring, is_enabled=False, layer=Layer(page_left_button=self._bank_left_button, page_right_button=self._bank_right_button, left_button=self._left_button, right_button=self._right_button), name=b'Session_Navigation')
    ...
    def _create_view_control(self):
        self._view_control = self.view_control_component_type(is_enabled=False, layer=Layer(prev_track_button=self._left_arrow_button, next_track_button=self._right_arrow_button, scene_scroll_encoder=self._jogwheel), name=b'View_Control')
        self._view_control.set_enabled(True)

    def _create_arrangement(self):
        self._arrangement = ArrangementComponent(is_enabled=False, layer=Layer(set_or_delete_cue_button=self._marker_button), name=b'Arrangement')
        self._arrangement.set_enabled(True)

    def _create_jogwheel_modes(self):
        self._jogwheel_modes = ModesComponent()
        self._jogwheel_modes.add_mode(b'Session', AddLayerMode(self._view_control, Layer(scene_scroll_encoder=self._jogwheel)))
        self._jogwheel_modes.add_mode(b'Arranger', AddLayerMode(self._arrangement, Layer(jump_encoder=self._jogwheel)))
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/KeyLab_Essential/keylab_essential.pyc
