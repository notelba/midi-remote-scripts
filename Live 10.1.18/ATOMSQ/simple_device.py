...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ATOMSQ\simple_device.py
...
class SimpleDeviceParameterComponent(Component):
    ...
    def __init__(self, device_provider=None, device_bank_registry=None, toggle_lock=None, *a, **k):
        ...
        self._device_name_slot = self.register_slot(None, self._update_device_name_display, b'name')
    ...
    def _on_off_parameter(self):
        if liveobj_valid(self._device):
            for p in self._device.parameters:
                if p.name.startswith(b'Device On') and liveobj_valid(p) and p.is_enabled:
                    return p
    ...
    def _update_device_name_display(self):
        self.device_name_display.message = self._device.name if liveobj_valid(self._device) else b' - '
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/ATOMSQ/simple_device.pyc
