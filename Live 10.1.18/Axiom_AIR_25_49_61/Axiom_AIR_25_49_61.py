...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Axiom_AIR_25_49_61\Axiom_AIR_25_49_61.py
...
class Axiom_AIR_25_49_61(ControlSurface):
    ...
    def __init__(self, c_instance):
        ...
        with self.component_guard():
            ...
            self._suggested_input_port = b'HyperControl'
            self._suggested_output_port = b'HyperControl'
    ...
    def handle_sysex(self, midi_bytes):
        if midi_bytes[0:10] == AXIOM_AIR_RESPONSE:
            if midi_bytes[12:15] < AXIOM_REV4_RESPONSE:
                ...
                self.schedule_message(3, self._name_display.display_message, b'Firmware')
                self.schedule_message(13, self._name_display.display_message, b'Update')
                self.schedule_message(23, self._name_display.display_message, b'Required')
    ...
    def _setup_controls(self):
        self._left_button = create_button(99, b'Left_Button')
        self._right_button = create_button(100, b'Right_Button')
        self._up_button = create_button(101, b'Up_Button')
        self._down_button = create_button(102, b'Down_Button')
        self._loop_button = create_button(113, b'Loop_Button')
        self._rwd_button = create_button(114, b'Rwd_Button')
        self._ffwd_button = create_button(115, b'FFwd_Button')
        self._stop_button = create_button(116, b'Stop_Button')
        self._play_button = create_button(117, b'Play_Button')
        self._rec_button = create_button(118, b'Record_Button')
        self._select_button = ConfigurableButtonElement(IS_MOMENTARY, MIDI_CC_TYPE, GLOBAL_CHANNEL, 98)
        self._select_button.name = b'Select_Button'
        self._select_button.add_value_listener(self._select_button_value)
        self._main_group_hyper_button = create_configurable_button(104, b'Fader_Group_HyperControl_Button', 2, 14)
        self._main_group_track_button = create_configurable_button(105, b'Main_Group_Track_Button', 2, 11)
        self._main_group_fx_button = create_configurable_button(106, b'Main_Group_Inst_FX_Button', 2, 11)
        self._identify_button = create_configurable_button(97, b'Identify_Button', 2, 16)
        self._identify_button.add_value_listener(self._identify_value)
        self._fader_buttons = []
        for index in range(8):
            self._fader_buttons.append(create_configurable_button(49 + index, b'Fader_Button_%d' % index))
            self._fader_buttons[(-1)].add_value_listener(self._fader_button_value, identify_sender=True)

        self._faders = []
        for index in range(8):
            self._faders.append(create_slider(33 + index, b'Fader_%d' % index))
            self._faders[(-1)].add_value_listener(self._fader_value, identify_sender=True)

        self._master_fader_button = create_configurable_button(57, b'Master_Fader_Button')
        self._master_fader_button.add_value_listener(self._fader_button_value, identify_sender=True)
        self._master_fader = create_slider(41, b'Master_Fader')
        self._master_fader.add_value_listener(self._fader_value, identify_sender=True)
        self._fader_group_mode_button = create_configurable_button(61, b'Fader_Group_Mode_Button')
        self._fader_group_midi_button = create_configurable_button(60, b'Fader_Group_MIDI_Button')
        self._fader_group_midi_button.add_value_listener(self._midi_button_value, identify_sender=True)
        self._fader_group_mix_button = create_configurable_button(58, b'Fader_Group_Mix_Button', 0, 1)
        self._fader_group_mix_button.add_value_listener(self._hyper_button_value, identify_sender=True)
        self._fader_group_fx_button = create_configurable_button(59, b'Fader_Group_Inst_FX_Button', 0, -1)
        self._fader_group_fx_button.add_value_listener(self._hyper_button_value, identify_sender=True)
        self._encoders = []
        for index in range(8):
            self._encoders.append(create_encoder(17 + index, b'Encoder_%d' % index))
            self._encoders[(-1)].add_value_listener(self._encoder_value, identify_sender=True)

        self._encoder_group_midi_button = create_configurable_button(27, b'Encoder_Group_MIDI_Button', 0, 72)
        self._encoder_group_midi_button.add_value_listener(self._midi_button_value, identify_sender=True)
        self._encoder_group_mix_button = create_configurable_button(25, b'Encoder_Group_Mix_Button', 0, 72)
        self._encoder_group_mix_button.add_value_listener(self._hyper_button_value, identify_sender=True)
        self._encoder_group_fx_button = create_configurable_button(26, b'Encoder_Group_Inst_FX_Button', 0, 72)
    ...
    def _setup_drum_pads(self):
        ...
        for index in range(8):
            self._drum_pads.append(create_configurable_button(81 + index, b'Pad_%d' % index, 0, 0, MIDI_CC_TYPE))
        ...
        for index in range(num_pads - 8):
            ...
            self._drum_pads[(-1)].name = b'Pad_' + str(index + 8)

        self._drum_group_midi_button = create_configurable_button(91, b'Drum_Group_MIDI_Button', 2, -2)
        self._drum_group_midi_button.add_value_listener(self._midi_button_value, identify_sender=True)
        self._drum_group_roll_button = create_configurable_button(90, b'Drum_Group_Roll_Button', -1)
        self._drum_group_hyper_button = create_configurable_button(89, b'Drum_Group_HyperControl_Button', 2, 2)
    ...
    def _setup_displays(self):
        ...
        self._name_display.name = b'Name_Display'
        ...
        self._value_display.name = b'Value_Display'
        ...
        self._bank_display.name = b'Bank_Display'
        ...
        self._pad_display.name = b'Pad_Display'
    ...
    def _setup_mixer(self):
        ...
        self._mixer_for_encoders.name = b'Mixer_for_encoders'
        self._mixer_for_faders = SpecialMixerComponent(self._name_display, self._value_display, 8)
        self._mixer_for_faders.name = b'Mixer_for_faders'
    ...
    def _setup_transport(self):
        ...
        self._transport.name = b'Transport'
    ...
    def _setup_modes(self):
        ...
        self._fader_modes.name = b'Fader_Modes'
        ...
        self._encoder_modes.name = b'Encoder_Modes'
    ...
    def _setup_single_fader_button_modes(self):
        ...
        self._single_fader_button_modes.name = b'Single_Fader_Button_Modes'
    ...
    def _show_startup_message(self):
        ...
        self._name_display.display_message(b'Ableton Live')
    ...
    def _fader_value(self, value, sender):
        ...
        if param != None:
            ...
            if param.name == b'Track Volume':
                if sender == self._master_fader:
                    if self._has_faders:
                        name_string = b'Master  Vol'
                    else:
                        name_string = self._mixer_for_faders.selected_strip().track_name_data_source().display_string() + b'   Vol'
                else:
                    name_string = self._mixer_for_faders.channel_strip(self._faders.index(sender)).track_name_data_source().display_string() + b'   Vol'
            else:
                name_string = param.name
                value = int((param.value - param.min) / param_range * 127)
            value_string = str(value)
        else:
            name_string = b'<unmapped>'
            value_string = None
    ...
    def _encoder_value(self, value, sender):
        ...
        if param != None:
            param_range = param.max - param.min
            if param.name == b'Track Volume':
                name_string = self._mixer_for_encoders.channel_strip(self._encoders.index(sender)).track_name_data_source().display_string() + b'   Vol'
                value = int((param.value - param.min) / param_range * 127)
            elif param.name == b'Track Panning':
                name_string = self._mixer_for_encoders.channel_strip(self._encoders.index(sender)).track_name_data_source().display_string() + b'   Pan'
    ...
    def _install_mapping(self, midi_map_handle, control, parameter, feedback_delay, feedback_map):
        ...
        return success
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Axiom_AIR_25_49_61/Axiom_AIR_25_49_61.pyc
