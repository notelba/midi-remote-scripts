...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\SL_MkIII\mixer.py
...
class MixerComponent(MixerComponentBase):
    send_up_button = ButtonControl(color=b'Mixer.Send')
    send_down_button = ButtonControl(color=b'Mixer.Send')
    ...
    def _update_send_index_display(self):
        send_index = self.send_index
        self.send_index_display[0] = b'Send ' + chr(send_index + 65) if send_index != None else b''
        return

    def _update_send_value_display(self):
        for index, send in enumerate(self.controlled_sends):
            self.send_value_display[index] = str(send) if send else b''

    def _update_send_encoder_color_fields(self):
        for index, send in enumerate(self.controlled_sends):
            self.send_encoder_color_fields[index].color = b'Mixer.Send' if send else b'DefaultButton.Disabled'
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/SL_MkIII/mixer.pyc
