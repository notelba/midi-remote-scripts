...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\SL_MkIII\device_navigation.py
...
class DisplayingDeviceNavigationComponent(DeviceNavigationComponent):
    ...
    def _update_device_names(self):
        for index, item in izip_longest(xrange(NUM_VISIBLE_ITEMS), self.items):
            display = getattr(self, (b'device_name_display_{}').format(index / NUM_DISPLAY_SEGMENTS + 1))
            display[index % NUM_DISPLAY_SEGMENTS] = item.item.name if item else b''

    def _update_device_bank_names(self):
        for index, item in izip_longest(xrange(NUM_VISIBLE_ITEMS), self.items):
            display = getattr(self, (b'device_bank_name_display_{}').format(index / NUM_DISPLAY_SEGMENTS + 1))
    ...
    def _update_selected_device_name_display(self):
        ...
        self.selected_device_name_display[0] = item.name if item else b'No Device'
    ...
    def _bank_name_for_item(self, item):
        ...
        return bank_name
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/SL_MkIII/device_navigation.pyc
