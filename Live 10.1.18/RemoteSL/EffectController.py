...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\RemoteSL\EffectController.py
...
class EffectController(RemoteSLComponent):
    ...
    def receive_midi_cc(self, cc_no, cc_value):
        if cc_no in fx_display_button_ccs:
            ...
            assert False, b'Lower Button CCS should be passed to Live!'
        elif cc_no in fx_poti_row_ccs:
            assert False, b'Poti CCS should be passed to Live!'
        else:
            assert False, b'unknown FX midi message'

    def receive_midi_note(self, note, velocity):
        if note in fx_drum_pad_row_notes:
            assert False, b'DrumPad CCS should be passed to Live!'
        else:
            assert False, b'unknown FX midi message'
    ...
    def __reassign_strips(self):
        ...
        if not self.__assigned_device == None:
            ...
            param_names = [
             b'Please select a Device in Live to edit it...']
        ...
        if self.__assigned_device != None:
            ...
            if cc_value == CC_VAL_BUTTON_PRESSED:
                if cc_no == FX_DISPLAY_PAGE_UP:
                    ...
                    assert False, b'unknown Display midi message'
                    if not self.__bank == new_bank:
                        self.__show_bank = True
                        self.__bank = self.__assigned_device_is_locked or new_bank
                        self.__reassign_strips()
                    else:
    ...
    def __handle_select_button_ccs(self, cc_no, cc_value):
        if cc_no == FX_SELECT_FIRST_BUTTON_ROW:
            ...
            assert False, b'unknown select row midi message'
        ...
        if self.__show_bank:
            ...
            if self.__assigned_device.class_name in DEVICE_DICT.keys():
                if self.__assigned_device.class_name in BANK_NAME_DICT.keys():
                    ...
                    self.__show_bank_select(b'Best of Parameters')
            else:
                self.__show_bank_select(b'Bank' + str(self.__bank + 1))

    def __show_bank_select(self, bank_name):
        if self.__assigned_device:
            self.__parent.show_message(str(self.__assigned_device.name + b' Bank: ' + bank_name))
...
class EffectChannelStrip():
    ...
    def on_encoder_moved(self, cc_value):
        assert self.__assigned_parameter == None, b'should only be reached when the encoder was not realtime mapped '
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/RemoteSL/EffectController.pyc
