...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\device_navigation.py
...
class DeviceNavigationComponent(DeviceNavigationComponentBase):
    ...
    def __init__(self, device_bank_registry=None, banking_info=None, delete_handler=None, track_list_component=None, *a, **k):
        ...
        self._modes.add_mode(b'default', [
         partial(self.chain_selection.set_parent, None),
         partial(self.bank_selection.set_device, None)])
        self._modes.add_mode(b'chain_selection', [self.chain_selection])
        self._modes.add_mode(b'bank_selection', [self.bank_selection])
        self._modes.selected_mode = b'default'
    ...
    def _in_device_enabling_mode(self):
        return self._track_list.selected_mode == b'mute'
    ...
    def _current_track(self):
        return self._track_decorator.decorate(self.song.view.selected_track, additional_properties={b'drum_pad_selected': False})
    ...
    def selected_object(self):
        ...
        return getattr(selected_item, b'proxied_object', selected_item)
    ...
    def _on_reselecting_object(self, device):
        if liveobj_valid(device) and device.can_have_chains:
            ...
            self._modes.selected_mode = b'bank_selection'
    ...
    def _show_chains(self, device):
        if device.can_have_chains:
            ...
            self._modes.selected_mode = b'chain_selection'
    ...
    def _update_item_provider(self, selection):
        ...
        if not is_drum_pad(selection):
            self._current_track().drum_pad_selected = False
        self.notify_drum_pad_selection()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/device_navigation.pyc
