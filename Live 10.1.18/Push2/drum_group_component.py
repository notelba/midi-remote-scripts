...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\drum_group_component.py
...
def find_simplers(chain):
    return ifilter(lambda i: hasattr(i, b'playback_mode'), find_instrument_devices(chain))
...
class DrumGroupComponent(DrumGroupComponentBase):
    ...
    def __init__(self, tracks_provider=None, device_decorator_factory=None, color_chooser=None, *a, **k):
        ...
        self.mute_button.color = b'DefaultButton.Transparent'
        self.solo_button.color = b'DefaultButton.Transparent'
    ...
    def _on_matrix_pressed(self, button):
        if self.select_color_button.is_pressed and self._color_chooser is not None:
            ...
            if liveobj_valid(pad) and pad.chains and liveobj_valid(pad.chains[0]):
                ...
                self.show_notification(b'Cannot color an empty drum pad')
        else:
    ...
    def _color_for_pad(self, pad):
        if self._is_hotswapping(pad):
            color = b'DrumGroup.PadHotswapping'
        else:
    ...
    def _chain_color_for_pad(self, pad, color):
        if color == b'DrumGroup.PadFilled':
            assert pad.chains and liveobj_valid(pad.chains[0]), b'Filled pads should have a chain'
            color = IndexedColor.from_live_index(pad.chains[0].color_index)
        elif color == b'DrumGroup.PadMuted':
            assert pad.chains and liveobj_valid(pad.chains[0]), b'Filled pads should have a chain'
            color = IndexedColor.from_live_index(pad.chains[0].color_index, shade_level=1)
        return color

    def _is_hotswapping(self, pad):
        if self._hotswap_indication_mode == b'current_pad':
            return pad == self._selected_drum_pad
        if self._hotswap_indication_mode == b'all_pads':
            return True
        return False
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/drum_group_component.pyc
