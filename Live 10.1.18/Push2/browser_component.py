...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\browser_component.py
...
NAVIGATION_COLORS = dict(color=b'Browser.Navigation', disabled_color=b'Browser.NavigationDisabled')
...
class PluginPresetBrowserItem(BrowserItem):
    ...
    def __init__(self, preset_name=None, preset_index=None, vst_device=None, *a, **k):
        ...
        super(PluginPresetBrowserItem, self).__init__(name=(preset_name if preset_name else b'<Empty Slot %i>' % (preset_index + 1)), is_loadable=True, *a, **k)
    ...
    @property
    def uri(self):
        return b'pluginpreset%i' % self.preset_index
...
class BrowserComponent(Component, Messenger):
    ...
    prehear_button = ToggleButtonControl(toggled_color=b'Browser.Option', untoggled_color=b'Browser.OptionDisabled')
    ...
    context_text = listenable_property.managed(b'')
    ...
    def __init__(self, preferences=dict(), commit_model_changes=None, selection=None, main_modes_ref=None, *a, **k):
        ...
        self.prehear_button.is_toggled = preferences.setdefault(b'browser_prehear', True)
        ...
        self.register_slot(self, self.notify_focused_item, b'focused_list_index')
    ...
    def prehear_button(self, toggled, button):
        ...
        self._preferences[b'browser_prehear'] = toggled
    ...
    @listens(b'selected_track.name')
    def _on_selected_track_name_changed(self):
        if self.is_enabled():
            self._update_context()

    @listens(b'detail_clip.name')
    def _on_detail_clip_name_changed(self):
        if self.is_enabled():
            self._update_context()

    @listens(b'hotswap_target')
    ...
    def browse_for_audio_clip(self):
        ...
        if main_modes is None:
            ...
            return not has_midi_support and b'clip' in main_modes.active_modes
    ...
    def _get_actual_item(self, item):
        contained_item = getattr(item, b'contained_item', None)
    ...
    def _make_notification_text(self, browser_item):
        return b'Loading %s' % browser_item.name
    ...
    def _update_navigation_buttons(self):
        ...
        context_button_color = IndexedColor.from_live_index(self.context_color_index, DISPLAY_BUTTON_SHADE_LEVEL) if self.context_color_index > -1 else b'Browser.Navigation'
    ...
    def _can_auto_expand(self):
        return len(self.focused_list.items) > self.NUM_ITEMS_PER_COLUMN * 2 and self.focused_list.selected_item.is_loadable and getattr(self.focused_list.selected_item, b'contained_item', None) == None
    ...
    def _replace_preview_list(self):
        ...
        if len(children_iterator) > 0:
            enable_wrapping = getattr(selected_item, b'enable_wrapping', True) and self.focused_list.items_wrapped
    ...
    def _wrap_device_item(self, item):
        ...
        return FolderBrowserItem(name=item.name, is_loadable=True, is_device=True, contained_item=item, wrapped_loadable=wrapped_loadable, icon=b'browser_arrowcontent.svg')
...
class MidiTrackBrowserItem(TrackBrowserItem):
    ...
    def __init__(self, *a, **k):
        super(MidiTrackBrowserItem, self).__init__(name=b'MIDI track', *a, **k)
...
class AudioTrackBrowserItem(TrackBrowserItem):
    ...
    def __init__(self, *a, **k):
        super(AudioTrackBrowserItem, self).__init__(name=b'Audio track', *a, **k)
...
class ReturnTrackBrowserItem(TrackBrowserItem):
    ...
    def __init__(self, *a, **k):
        super(ReturnTrackBrowserItem, self).__init__(name=b'Return track', *a, **k)
...
class DefaultTrackBrowserItem(BrowserItem):
    ...
    def __init__(self, *a, **k):
        super(DefaultTrackBrowserItem, self).__init__(name=b'Default track', is_loadable=True, *a, **k)
...
class NewTrackBrowserComponent(BrowserComponent):
    ...
    def _make_notification_text(self, browser_item):
        if isinstance(browser_item, DefaultTrackBrowserItem):
            return b'Default track created'
        new_track_position = self._selected_track_index() + 1
        return b'%s loaded in track %i' % (browser_item.name, new_track_position)
...
class UserFilesBrowserItem(BrowserItem):
    ...
    def children(self):
        res = [
         wrap_item(self._browser.user_library, b'browser_userlibrary.svg')] + wrap_items(list(self._browser.user_folders), b'browser_folder.svg')
        self._browser = None
        return res
...
class CollectionsBrowserItem(BrowserItem):
    ...
    @lazy_attribute
    def children(self):
        color_labels = wrap_items(list(self._browser.colors), b'browser_collection_icon.svg', enable_wrapping=False)
        self._browser = None
        return color_labels


def make_root_browser_items(browser, filter_type):
    collections = CollectionsBrowserItem(browser, name=b'Collections', icon=b'browser_collections.svg')
    sounds = wrap_item(browser.sounds, b'browser_sounds.svg')
    drums = wrap_item(browser.drums, b'browser_drums.svg', enable_wrapping=False)
    instruments = wrap_item(browser.instruments, b'browser_instruments.svg')
    audio_effects = wrap_item(browser.audio_effects, b'browser_audioeffect.svg')
    midi_effects = wrap_item(browser.midi_effects, b'browser_midieffect.svg')
    packs = wrap_item(browser.packs, b'browser_packs.svg')
    current_project = wrap_item(browser.current_project, b'browser_currentproject.svg')
    if filter_type == Live.Browser.FilterType.samples:
        categories = [
         packs, current_project]
    else:
        common_items = [
         wrap_item(browser.max_for_live, b'browser_max.svg'),
         wrap_item(browser.plugins, b'browser_plugins.svg'),
         packs,
         current_project]
        if filter_type == Live.Browser.FilterType.audio_effect_hotswap:
            ...
            categories = [
             ...
             audio_effects,
             midi_effects] + common_items
    user_files = UserFilesBrowserItem(browser, name=b'User Files', icon=b'browser_userfiles.svg')
    return [
     collections, user_files] + categories
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/browser_component.pyc
