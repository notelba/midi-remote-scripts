...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\model\declaration.py
...
class view_property(property_declaration):
    ...
    def __repr__(self):
        return b'<%s %r>' % (self.__class__.__name__, self.property_type)
...
class id_property(property_declaration):
    ...
    def id_attribute_getter(obj):
        if hasattr(obj, b'__id__'):
            return unicode(obj.__id__)
        if hasattr(obj, b'_live_ptr'):
...
class ModelVisitor(object):
    ...
    def visit_view_property(self, name, decl):
        if is_reference_property_decl(decl):
            ...
            raise Exception(b'Invalid property declaration')
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/model/declaration.pyc
