...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\model\generation.py
...
class NotifyingList(WrapperBase):
    ...
    def __repr__(self):
        return b'<%s %r>' % (self.__class__.__name__, self.data)
...
ClassInfo = namedtuple(b'ClassInfo', [b'class_', b'd', b'default_data', b'wrappers', b'children'])
    ...
    def visit_binding_class(self, class_):
        if class_ not in self._decl2class:
            with self(ClassInfo(class_=class_, d=None, default_data=None, wrappers={}, children=None)) as (ci):
                ...
                self._decl2class[class_] = partial(BoundObjectWrapper, wrappers=ci.wrappers, adapter=class_.__dict__.get(b'ADAPTER', ModelAdapter))
...
class ViewModelVisitor(ModelVisitor):
    ...
    def visit_viewmodel_class(self, class_):
        with self(ClassInfo(class_=class_, d={}, default_data={}, wrappers={}, children={})) as (ci):
            ...
            ci.d[b'default_data'] = ci.default_data
            ci.d[b'wrappers'] = ci.wrappers
            ci.d[b'children'] = ci.children
            generated_class = type(str(b'P2' + class_.__name__), (ModelMixin,), ci.d)
...
class ModelFingerprintVisitor(ModelVisitor):
    ...
    def fingerprint(self):
        if self._fingerprint is None:
            self._fingerprint = (b';').join(b'%s(%s)' % (classname, (b',').join(property_prints)) for classname, property_prints in sorted(self._class2proplist.iteritems(), key=lambda item: item[0]))
    ...
    def visit_id_property(self, *_a):
        self.property_prints.append(b'id')

    def visit_value_property(self, name, decl):
        super(ModelFingerprintVisitor, self).visit_value_property(name, decl)
        self.property_prints.append(b'%s:%s' % (name, decl.property_type.__name__))
    ...
    def visit_value_list_property(self, name, decl, property_type):
        ...
        self.property_prints.append(b'%s:listof(%s)' % (name, property_type.__name__))
    ...
    def visit_list_model_property(self, name, decl, property_type):
        ...
        self.property_prints.append(b'%s:listmodel(%s)' % (name, property_type.__name__))

    def visit_complex_list_property(self, name, decl, value_type):
        super(ModelFingerprintVisitor, self).visit_complex_list_property(name, decl, value_type)
        self.property_prints.append(b'%s:listof(%s)' % (name, value_type.__name__))
...
def generate_mrs_model(cls):
    ...
    return root_class
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/model/generation.pyc
