...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\wavetable.py
...
class WavetableDeviceDecorator(WavetableDeviceDecoratorBase):
    ...
    def _create_parameters(self):
        self.filter_switch_for_filter_switch_option = EnumWrappingParameter(name=b'Internal Filter', parent=self, values_host=self._filter_types_provider, index_property_host=self._filter_types_provider, values_property=b'available_values', index_property=b'index', value_type=WavetableFilterType)
        self.current_mod_target = InternalParameter(name=b'Current Mod Target', parent=self)
        return super(WavetableDeviceDecorator, self)._create_parameters() + (EnumWrappingParameter(name=b'Modulation Target Names', parent=self, values_host=self._live_object, index_property_host=self.current_mod_target_index, values_property=b'visible_modulation_target_names', index_property=b'index'), ModMatrixParameter(name=b'Amp Env Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.amp_envelope), ModMatrixParameter(name=b'Env 2 Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.envelope_2), ModMatrixParameter(name=b'Env 3 Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.envelope_3), ModMatrixParameter(name=b'Lfo 1 Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.lfo_1), ModMatrixParameter(name=b'Lfo 2 Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.lfo_2), ModMatrixParameter(name=b'MIDI Velocity Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.midi_velocity), ModMatrixParameter(name=b'MIDI Note Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.midi_note), ModMatrixParameter(name=b'MIDI Pitch Bend Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.midi_pitch_bend), ModMatrixParameter(name=b'MIDI Aftertouch Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.midi_channel_pressure), ModMatrixParameter(name=b'MIDI Mod Wheel Mod Amount', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.midi_mod_wheel), ModMatrixParameter(name=b'MIDI Random On Note On', parent=self, modulation_value_host=self._live_object, modulation_target_index_host=self.current_mod_target_index, modulation_source=ModulationSource.midi_random)) + (self.filter_switch_for_filter_switch_option, self.current_mod_target)
    ...
    def _create_options(self):
        ...
        def add_selected_parameter_to_mod_matrix():
            if is_selected_parameter_modulatable():
                ...
                self.notify_request_bank_view(b'Matrix')
        ...
        self.osc_on_option = DeviceOnOffOption(name=b'Osc', property_host=self._get_osc_on_property_host())
        self.filter_on_option = DeviceOnOffOption(name=b'Filter', property_host=self._get_filter_on_property_host())
        self.lfo_retrigger_option = DeviceOnOffOption(name=b'Retrigger', property_host=self._get_lfo_retrigger_property_host())
        self.add_to_mod_matrix_option = DeviceTriggerOption(name=b'Add to Matrix', callback=add_selected_parameter_to_mod_matrix, is_active=is_selected_parameter_modulatable)
        return (
         DeviceOnOffOption(name=b'Sub', property_host=get_parameter_by_name(self, b'Sub On')),
         DeviceSwitchOption(name=b'Filter 1 Slope', parameter=get_parameter_by_name(self, b'Filter 1 Slope'), labels=[
          b'12dB', b'24dB']),
         DeviceSwitchOption(name=b'Filter 2 Slope', parameter=get_parameter_by_name(self, b'Filter 2 Slope'), labels=[
          b'12dB', b'24dB']),
         DeviceSwitchOption(name=b'Filter Switch', parameter=self.filter_switch_for_filter_switch_option, labels=[
          b'Filter 1', b'Filter 2']),
         DeviceSwitchOption(name=b'LFO 1 Sync', parameter=get_parameter_by_name(self, b'LFO 1 Sync'), labels=[
          b'Hz', b'Sync']),
         DeviceSwitchOption(name=b'LFO 2 Sync', parameter=get_parameter_by_name(self, b'LFO 2 Sync'), labels=[
          b'Hz', b'Sync']),
         DeviceTriggerOption(name=b'Go to Amp Env', callback=lambda : (
          choose_envelope(WavetableEnvelopeType.amp),
          jump_to_bank(b'Envelopes'))),
         DeviceTriggerOption(name=b'Go to Env 2', callback=lambda : (
          choose_envelope(WavetableEnvelopeType.env2),
          jump_to_bank(b'Envelopes'))),
         DeviceTriggerOption(name=b'Go to Env 3', callback=lambda : (
          choose_envelope(WavetableEnvelopeType.env3),
          jump_to_bank(b'Envelopes'))),
         DeviceTriggerOption(name=b'Go to LFO 1', callback=lambda : (
          choose_lfo(WavetableLfoType.one), jump_to_bank(b'LFOs'))),
         DeviceTriggerOption(name=b'Go to LFO 2', callback=lambda : (
          choose_lfo(WavetableLfoType.two), jump_to_bank(b'LFOs'))),
         DeviceTriggerOption(name=b'Back', callback=self.notify_request_previous_bank_from_mod_matrix)) + (
         self.osc_on_option,
    ...
    def _get_osc_on_property_host(self):
        return get_parameter_by_name(self, (b'Osc {} On').format(2 if self.oscillator_switch.value else 1))

    def _get_filter_on_property_host(self):
        return get_parameter_by_name(self, (b'Filter {} On').format(self.filter_switch_for_filter_switch_option.value + 1))

    def _get_lfo_retrigger_property_host(self):
        return get_parameter_by_name(self, (b'LFO {} Retrigger').format(self._lfo_types_provider.index + 1))

    def _resolve_ambiguous_modulation_target_name(self, target_parameter_name):
        if re.match(b'^Osc (1|2) Transp$', target_parameter_name):
            return target_parameter_name.replace(b'Transp', b'Pitch')
        lfo_rate_re = re.match(b'^LFO (1|2) S\\. Rate$', target_parameter_name)
        if lfo_rate_re:
            lfo_number = lfo_rate_re.group(1)
            lfo_sync_param = get_parameter_by_name(self, (b'LFO {} Sync').format(lfo_number))
            if lfo_sync_param.value == 0:
                return (b'LFO {} Rate').format(lfo_number)
        ...
        if isinstance(parameter, PitchParameter) and has_automation(parameter):
            ...
            if playing_clip:
                ...
                if deleted_automation_names:
                    self.show_notification(MessageBoxText.DELETE_ENVELOPE % dict(automation=(b', ').join(deleted_automation_names)))
        else:
...
class WavetableDeviceComponent(DeviceComponentWithTrackColorViewData):
    OSCILLATOR_POSITION_PARAMETER_NAMES = re.compile(b'^(Osc (1|2) Pos)$|^Position$')
    FILTER_PARAMETER_NAMES = re.compile(b'^(Filter (1|2) (Type|Freq|Res))$|^Filter Type$|^Frequency$|^Resonance$')
    LFO_PARAMETER_NAMES = re.compile(b'^(LFO (1|2) (Shape|Shaping|S. Rate|Rate|Amount|Attack Time|Phase Offset))$|^LFO$|^LFO Type$|^Shape$|^Rate$|^Amount$|^Attack$|^Offset$')
    VISUALISATION_CONFIGURATION = {b'wavetable': {b'position_in_banks': {0: ButtonRange(0, 2), 1: ButtonRange(1, 3)}, b'visible_in_bank': lambda component, bank: component.selected_oscillator in [
                                         WavetableOscillatorType.one, WavetableOscillatorType.two]}, 
       b'filter': {b'position_in_banks': {0: ButtonRange(3, 5), 2: ButtonRange(2, 4)}}, b'lfo': {b'position_in_banks': {5: ButtonRange(0, 3)}}, b'envelope': {b'position_in_banks': {4: ButtonRange(2, 5)}}}
    ENVELOPE_PREFIXES = [
     b'Amp', b'Env 2', b'Env 3']

    def __init__(self, *a, **k):
        super(WavetableDeviceComponent, self).__init__(*a, **k)
        self._bank_before_mod_matrix = 0
        self._delete_default_component = WavetableDeleteComponent(parent=self, name=b'DeleteAndDefault')
    ...
    def _parameter_released(self, parameter):
        if liveobj_valid(self._decorated_device) and liveobj_valid(parameter):
            ...
            self._update_single_selected_parameter()
            if self.OSCILLATOR_POSITION_PARAMETER_NAMES.match(parameter.name):
                view_data[b'AdjustingPosition'] = False
            if not self._any_filter_parameter_touched():
                view_data[b'AdjustingFilter'] = False
            if not self._any_lfo_parameter_touched():
                view_data[b'AdjustingLfo'] = False
            view_data.update(self._envelope_visualisation_data())
            if view_data:
                self._update_visualisation_view_data(view_data)
    ...
    def _get_provided_parameters(self):
        ...
        for param, name in parameters:
            if param == self._decorated_device.current_mod_target:
                ...
                name = param.name if param is not None else b''
    ...
    def _visualisation_type_visible(self, visualisation_type):
        ...
        return bank in configuration.get(b'position_in_banks', {}) and configuration.get(b'visible_in_bank', const(True))(self, bank)
    ...
    def _get_visualisation_range(self, visualisation_type):
        configuration = self.VISUALISATION_CONFIGURATION[visualisation_type]
        positions = configuration.get(b'position_in_banks', {})
    ...
    def _get_current_view_data(self):
        view_data = {b'SelectedOscillator': self.selected_oscillator, 
           b'AdjustingPosition': False, 
           b'AdjustingFilter': False, 
           b'AdjustingLfo': False, 
           b'WavetableVisualisationStart': self._get_visualisation_start(b'wavetable'), 
           b'WavetableVisualisationWidth': self._get_visualisation_width(b'wavetable'), 
           b'FilterCurveVisualisationStart': self._get_visualisation_start(b'filter'), 
           b'FilterCurveVisualisationWidth': self._get_visualisation_width(b'filter'), 
           b'LfoVisualisationStart': self._get_visualisation_start(b'lfo'), 
           b'LfoVisualisationWidth': self._get_visualisation_width(b'lfo'), 
           b'EnvelopeVisualisationStart': self._get_visualisation_start(b'envelope'), 
           b'EnvelopeVisualisationWidth': self._get_visualisation_width(b'envelope'), 
           b'WavetableVisualisationVisible': self._visualisation_type_visible(b'wavetable'), 
           b'FilterVisualisationVisible': self._visualisation_type_visible(b'filter'), 
           b'LfoVisualisationVisible': self._visualisation_type_visible(b'lfo'), 
           b'EnvelopeVisualisationVisible': self._visualisation_type_visible(b'envelope'), 
           b'SelectedFilter': self.selected_filter, 
           b'SelectedLfo': self.selected_lfo}
        view_data.update(self._envelope_visualisation_data())
        return view_data
    ...
    def _envelope_visualisation_data(self):
        ...
        if self._visualisation_type_visible(b'envelope'):
    ...
    @listens(b'request_previous_bank_from_mod_matrix')
    ...
    def __on_selected_oscillator_changed(self):
        self._update_visualisation_view_data({b'SelectedOscillator': self.selected_oscillator, 
           b'WavetableVisualisationVisible': self._visualisation_type_visible(b'wavetable')})
        self.notify_visualisation_visible()
        self.notify_shrink_parameters()

    @listens(b'filter_index')
    def __on_selected_filter_changed(self):
        self._update_visualisation_view_data({b'SelectedFilter': self.selected_filter})

    @listens(b'lfo_index')
    def __on_selected_lfo_changed(self):
        self._update_visualisation_view_data({b'SelectedLfo': self.selected_lfo})

    @listens(b'envelope_index')
    def __on_selected_envelope_changed(self):
        self._update_visualisation_view_data(self._envelope_visualisation_data())

    @listens(b'envelope_view_index')
    def __on_selected_envelope_view_changed(self):
        self._update_visualisation_view_data(self._envelope_visualisation_data())

    @listens(b'current_mod_target_parameter')
    def __on_current_mod_target_parameter_changed(self):
        self._update_parameters()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/wavetable.pyc
