...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\routing.py
...
MASTER_OUTPUT_TARGET_ID = b'Master'
NO_INPUT_TARGET_ID = b'No Input'
AUDIO_CHANNEL_POSITION_POSTFIXES = [
 b'Pre FX', b'Post FX', b'Post Mixer']
...
class RoutingMeterRealTimeChannelAssigner(Component):
    ...
    def __init__(self, real_time_mapper=None, register_real_time_data=nop, sliding_window_size=None, *a, **k):
        ...
        self.real_time_channels = [ RealTimeDataComponent(parent=self, channel_type=b'meter', real_time_mapper=real_time_mapper, register_real_time_data=register_real_time_data) for _ in xrange(sliding_window_size)
                                  ]
        return
...
class TrackOrRoutingControlChooserComponent(ModesComponent):
    ...
    def __init__(self, tracks_provider=None, track_mixer_component=None, routing_control_component=None, *a, **k):
        ...
        self.add_mode(b'mix', track_mixer_component)
        self.add_mode(b'routing', routing_control_component)
        self.selected_mode = b'mix'
        for mode in [b'mix', b'routing']:
            button = self.get_mode_button(mode)
            button.mode_selected_color = b'MixOrRoutingChooser.ModeActive'
            button.mode_unselected_color = b'MixOrRoutingChooser.ModeInactive'
    ...
    def _update_routing_mode_availability(self):
        ...
        if is_available != self._routing_previously_available:
            ...
            if is_available and b'routing' in self.active_modes:
                self.pop_mode(b'mix')
            else:
                self.push_mode(b'mix')
...
class Router(EventObject):
    ...
    def __init__(self, routing_level=None, routing_direction=None, song=None, *a, **k):
        ...
        self._song = song
        self._current_target_property = b'%s_routing_%s' % (
         routing_direction,
         routing_level)
    ...
    def _register_listeners(self):
        self.register_slot(MultiSlot(subject=self._song.view, event_name_list=(
         b'selected_track', self._current_target_property), listener=self.__on_current_routing_changed))
        self.register_slot(MultiSlot(subject=self._song.view, event_name_list=(
         b'selected_track',
         b'available_%ss' % self._current_target_property), listener=self.__on_routings_changed))
...
class InputTypeRouter(Router):
    ...
    def __init__(self, *a, **k):
        super(InputTypeRouter, self).__init__(routing_direction=b'input', routing_level=b'type', *a, **k)
...
class OutputTypeRouter(Router):
    ...
    def __init__(self, *a, **k):
        super(OutputTypeRouter, self).__init__(routing_direction=b'output', routing_level=b'type', *a, **k)
...
class InputChannelRouter(Router):
    ...
    def __init__(self, *a, **k):
        super(InputChannelRouter, self).__init__(routing_direction=b'input', routing_level=b'channel', *a, **k)
...
class InputChannelAndPositionRouter(EventObject):
    ...
    def input_type_name(self):
        ...
        if current_target:
            return getattr(current_target.attached_object, b'name', b'')
        return b''

    def _update_channel_grouping(self):
        attached_object = getattr(self._input_type_router.current_target, b'attached_object', None)
...
class OutputChannelRouter(Router):
    ...
    def __init__(self, *a, **k):
        super(OutputChannelRouter, self).__init__(routing_direction=b'output', routing_level=b'channel', *a, **k)
...
class RoutingTarget(EventObject):
    ...
    def __eq__(self, other):
        return other is not None and self._live_target == getattr(other, b'_live_target', None)
    ...
    def __repr__(self):
        return b'<%s name=%s>' % (self.__class__.__name__, self._name)
...
class RoutingChannelList(RoutingTargetList):
    ...
    @listens(b'list_index_to_pool_index_mapping')
...
class RoutingChannelPositionList(EventObject):
    ...
    @listens(b'has_input_channel_position')
    def __on_has_input_channel_position_changed(self, _):
        self._update_targets()
        self.notify_selected_index()

    @listens(b'input_channel_positions')
    def __on_input_channel_positions_changed(self):
        self._update_targets()

    @listens(b'input_channel_position_index')
...
class RoutingControlComponent(ModesComponent):
    ...
    def __init__(self, real_time_mapper=None, register_real_time_data=None, *a, **k):
        ...
        self.add_mode(b'input', [
         SetAttributeMode(self, b'_can_route', can_set_input_routing),
         partial(self._set_active_routers, input_type_router, input_channel_and_position_router),
         self._real_time_channel_assigner])
        self.add_mode(b'output', [
         SetAttributeMode(self, b'_can_route', lambda *a: True),
         partial(self._set_active_routers, output_type_router, output_channel_router),
         self._real_time_channel_assigner])
        self.selected_mode = b'input'
        self.__on_selected_track_changed.subject = self.song.view
        self.__on_selected_track_changed()
        self._connect_monitoring_state_encoder()
        self.input_output_choice_encoder.connect_static_list(self, b'selected_mode', list_values=[b'input', b'output'])
    ...
    def can_monitor(self):
        ...
        return hasattr(track, b'current_monitoring_state') and not track.is_frozen and track.can_be_armed
    ...
    def is_choosing_output(self):
        return self.selected_mode == b'output'
    ...
    @listens(b'selected_track.current_monitoring_state')
    ...
    @listens_group(b'output_routing_type')
    ...
    def _connect_input_channel_position_encoder(self):
        if self._active_channel_router.has_input_channel_position:
            self.routing_channel_position_encoder.connect_list_property(self._active_channel_router, current_index_property_name=b'input_channel_position_index', max_index=len(self._active_channel_router.input_channel_positions) - 1)
    ...
    def _update_routing_type_list(self):
        ...
        self.routing_type_encoder.connect_list_property(self._routing_type_list, current_value_property_name=b'selected_target', list_property_name=b'targets')
    ...
    def _update_routing_channel_list(self):
        ...
        for encoder in self.routing_channel_encoders:
            encoder.connect_list_property(self._routing_channel_list, current_value_property_name=b'selected_target', list_property_name=b'targets')
    ...
    def _connect_monitoring_state_encoder(self):
        self.monitor_state_encoder.connect_static_list(self.song.view.selected_track, b'current_monitoring_state', list_values=[
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/routing.pyc
