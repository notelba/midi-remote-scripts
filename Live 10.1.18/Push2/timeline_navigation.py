...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\timeline_navigation.py
...
FocusMarker = namedtuple(b'FocusMarker', [b'name', b'position'])
...
class Region(namedtuple(b'Region', [b'start', b'end'])):
    """
...
class TimelineNavigation(EventObject):
    ...
    def regions_of_interest(self):
        ...
        """
        rois = {b'timeline': self.timeline_roi, b'focused_object': self.focused_object_roi}
    ...
    def zoom(self, value):
        """ Zooms in or out of the timeline start
        ...
        self.set_visible_region(region, force_animate=animate, source_action=b'zoom')
    ...
    def _add_margin_to_zoomed_region(self, zoom_region, focused_region, margin_type):
        ...
        if focused_region is not None and margin_type != MarginType.NONE:
            ...
            if zoom_region.start <= position <= zoom_region.end:
                ...
                logger.warn(b"Focused object not visible. Couldn't add margin to zoomed region. %d not in %r" % (
        ...
        if obj != self.get_zoom_object():
            ...
            if identifier in self.focusable_object_descriptions:
                if len(objects_to_show) > 1:
                    logger.debug(b'Focus all objects %r' % objects_to_show)
                    self._focused_identifier = identifier
                    self._show_all_objects(objects_to_show)
                else:
                    logger.debug(b'Focus object %r' % identifier)
    ...
    def _show_all_objects(self, identifiers):
        ...
        if new_region is not None:
            self.set_visible_region(self._add_margin_to_region(new_region), source_action=b'show_objects %r' % identifiers)
            self._request_select_region = True
            self._locked_roi = None
        self.focus_marker = FocusMarker(b'', 0.0)
        return

    def _focus_object_by_identifier(self, identifier, animate=False):
        """ Focuses the object in the timeline and brings it into the visible range.
    ...
    def _report_current_source_and_target_roi(self):
        ...
        logger.debug(b'Zooming between roi "%s" and "%s"' % (source_roi_name, target_roi_name))
...
class SimplerWaveformNavigation(TimelineNavigation, WaveformNavigation):
    """ Extends the TimelineNavigation class by the concept of focusing parameters
        and slices.
    """
    selected_slice_focus = b'selected_slice'
    ...
    @lazy_attribute
    def additional_regions_of_interest(self):
        return {b'start_end_marker': self.make_region_of_interest(start_identifier=b'Start', end_identifier=b'End', getter=lambda : (
                               self._simpler.positions.start_marker,
                               self._simpler.positions.end_marker)), 
           b'active_sample': self.make_region_of_interest(start_identifier=b'S Start', end_identifier=b'S Length', getter=lambda : (
                            self._simpler.positions.active_start,
                            self._simpler.positions.active_end)), 
           b'loop': self.make_region_of_interest(start_identifier=b'S Loop Length', end_identifier=b'S Length', getter=lambda : (
                   self._simpler.positions.loop_start,
                   self._simpler.positions.loop_end)), 
           b'selected_slice': self.make_region_of_interest(start_identifier=self.selected_slice_focus, end_identifier=None, getter=lambda : (
                             self._simpler.positions.selected_slice.time,
                             self.get_next_slice_position()))}

    @lazy_attribute
    def additional_focusable_object_descriptions(self):
        return {b'Start': ObjectDescription(('start_end_marker', ), b'start_marker'), 
           b'End': ObjectDescription(('start_end_marker', ), b'end_marker'), 
           b'S Start': ObjectDescription(('start_end_marker', 'active_sample'), b'position'), 
           b'S Length': ObjectDescription(('start_end_marker', 'active_sample'), b'position'), 
           b'S Loop Length': ObjectDescription(('start_end_marker', 'active_sample', 'loop'), b'position'), 
           self.selected_slice_focus: ObjectDescription(('start_end_marker', 'selected_slice'), b'')}

    def get_object_identifier(self, obj):
        if hasattr(obj, b'name'):
            return obj.name
        return obj
    ...
    def object_changed(self, identifier1, identifier2):
        if self.selected_slice_focus in (identifier1, identifier2) and self._get_selected_slice_index() == 0 and b'Start' in (identifier1, identifier2):
            return False
        return identifier1 != identifier2
    ...
    def __on_playback_mode_changed(self):
        start_end_region = self.regions_of_interest[b'start_end_marker'].region
        if start_end_region.inside(self.visible_region):
            self.focus_object(self._simpler.get_parameter_by_name(b'Start'))
        else:
    ...
    @listens(b'playing_position_enabled')
    ...
    def _focus_start_end_roi(self):
        self.focus_region_of_interest(b'start_end_marker', self._simpler.get_parameter_by_name(b'Start'))
...
class ClipTimelineNavigation(TimelineNavigation):
    ...
    MIN_VISIBLE_BEATS = 1
    zoom_focus = b'zoom'
    start_marker_focus = b'start_marker'
    loop_start_focus = b'loop_start'
    loop_end_focus = b'loop_end'

    def __init__(self, clip=None, *a, **k):
        super(ClipTimelineNavigation, self).__init__(*a, **k)
        self._clip = clip
        self._process_object_changes = True
        self._connect_positions_property(b'loop_start', self.loop_start_focus)
        self._connect_positions_property(b'loop_length', self.loop_end_focus)
        self._connect_positions_property(b'start_marker', self.start_marker_focus)
    ...
    @lazy_attribute
    def additional_regions_of_interest(self):
        return {b'start_end_marker': self.make_region_of_interest(start_identifier=self.start_marker_focus, end_identifier=self.loop_end_focus, getter=lambda : (
                               self._clip.positions.start_marker,
                               self._clip.positions.loop_end)), 
           b'loop': self.make_region_of_interest(start_identifier=self.loop_start_focus, end_identifier=self.loop_end_focus, getter=lambda : (
                   self._clip.positions.loop_start,
                   self._clip.positions.loop_end)), 
           b'start_end': self.make_region_of_interest(getter=self._get_start_end_region)}

    @lazy_attribute
    def additional_focusable_object_descriptions(self):
        return {self.start_marker_focus: ObjectDescription(('start_end', 'start_end_marker'), b'start_marker'), 
           self.loop_start_focus: ObjectDescription(('start_end', 'loop'), lambda : b'position' if self._clip.looping else b'start_marker'), 
           self.loop_end_focus: ObjectDescription(('start_end', ), b'end_marker')}
...
class AudioClipTimelineNavigation(ClipTimelineNavigation, WaveformNavigation):
    ...
    def _update_waveform_region_and_preserve_visible_region(self):
        ...
        self.set_visible_region(region)
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/timeline_navigation.pyc
