...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\device_view_component.py
...
class DeviceViewConnector(Component):
    ...
    def __init__(self, device_component=None, parameter_provider=None, device_type_provider=const(b'default'), view=None, *a, **k):
...
class SimplerDeviceViewConnector(DeviceViewConnector):
    ...
    def update(self):
        ...
        assert device == None or device.class_name == b'OriginalSimpler'
...
class CompressorDeviceViewConnector(DeviceViewConnector):
    ...
    def update(self):
        ...
        device = self._value_for_state(self._device_component.device(), None)
        assert not liveobj_valid(device) or device.class_name == b'Compressor2'
        self._view.bank_view_description = self._parameter_provider.device_component.bank_view_description
...
class DeviceViewComponent(ModesComponent):
    ...
    def __init__(self, device_component=None, view_model=None, *a, **k):
        ...
        for view, connector, name in (
         (
          view_model.deviceParameterView, DeviceViewConnector, b'default'),
         (
          view_model.simplerDeviceView, SimplerDeviceViewConnector, b'OriginalSimpler'),
         (
    ...
    def _on_parameters_changed(self):
        self.selected_mode = self._mode_to_select()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/device_view_component.pyc
