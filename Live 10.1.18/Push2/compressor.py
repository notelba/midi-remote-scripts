...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\compressor.py
...
class CompressorDeviceDecorator(LiveObjectDecorator, EventObject):
    ...
    def __init__(self, *a, **k):
        ...
        for event, func in (
         (
          b'input_routing_type', self.notify_input_routing_type_index),
         (
          b'input_routing_channel', self.notify_input_routing_channel_index)):
        ...
        self._position_parameter = make_parameter(b'Position', b'input_channel_positions', b'input_channel_position_index')
        self._additional_parameters = self.register_disconnectables((
         make_parameter(b'Input Type', b'available_input_routing_types', b'input_routing_type_index'),
         make_parameter(b'Input Channel', b'available_input_routing_channels', b'input_routing_channel_index'),
    ...
    def available_input_routing_channels(self):
        return tuple([ t.name for t in self._channel_list.targets ] or [b''])
...
class CompressorInputRouterMixin(object):
    ...
    def _register_listeners(self):
        self._registered_listeners = [
         self.register_slot(subject=self._get_routing_host(), event_name=b'available_%ss' % self._current_target_property, listener=self.notify_routing_targets),
...
class CompressorDeviceComponent(DeviceComponentWithTrackColorViewData):
    ...
    def _parameter_touched(self, parameter):
        if liveobj_valid(self._decorated_device) and liveobj_valid(parameter) and parameter.name == b'Threshold':
            self._update_visualisation_view_data({b'AdjustingThreshold': True})

    def _parameter_released(self, parameter):
        if liveobj_valid(self._decorated_device) and liveobj_valid(parameter) and parameter.name == b'Threshold':
            self._update_visualisation_view_data({b'AdjustingThreshold': False})
    ...
    def _initial_visualisation_view_data(self):
        ...
        view_data[b'AdjustingThreshold'] = False
        view_data[b'VisualisationLeft'] = VisualisationGuides.light_left_x(0)
        view_data[b'VisualisationWidth'] = VisualisationGuides.button_right_x(3) - VisualisationGuides.light_left_x(0)
        return view_data

    @property
    def _visualisation_visible(self):
        return self._bank.index == 0
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/compressor.pyc
