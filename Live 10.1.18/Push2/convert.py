...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\convert.py
...
class NullConvertCategory(ConvertCategory):
    ...
    def convert(self, song, action_index):
        assert False, b'Cannot call convert on NullConvertCategory'
...
class MidiTrackWithoutSimpler(TrackBasedConvertCategory):
    internal_name = b'midi_track_to_drum_pad'
    ...
    def __init__(self, device=None, decorator_factory=None, *a, **k):
        ...
        if hasattr(device, b'playback_mode'):
...
class AudioTrackWithSessionClip(ConvertCategory):
    internal_name = b'audio_clip_to_simpler'
...
class AudioTrackWithArrangementClip(ConvertCategory):
    internal_name = b'audio_arrangement_clip_to_simpler'
...
class MidiTrackWithSimpler(TrackBasedConvertCategory):
    internal_name = b'sliced_simpler_to_drum_rack'
...
class MidiTrackWithDrumRack(ConvertCategory):
    internal_name = b'drum_pad_to_midi_track'
...
class ConvertComponent(Component):
    ...
    action_buttons = control_list(ButtonControl, color=b'Option.Unselected', pressed_color=b'Option.Selected')
    cancel_button = ButtonControl(color=b'Option.Unselected', pressed_color=b'Option.Selected')
    source_color_index = listenable_property.managed(UNCOLORED_INDEX)
    source_name = listenable_property.managed(unicode(b''))
    ...
    @listens(b'action_invalidated')
    def __on_action_invalidated(self):
        self.notify_cancel()


class ConvertEnabler(Component):
    convert_toggle_button = ButtonControl(color=b'DefaultButton.On')
    ...
    def __init__(self, enter_dialog_mode=None, exit_dialog_mode=None, *a, **k):
        ...
        self._enter_dialog_mode = partial(enter_dialog_mode, b'convert')
        self._exit_dialog_mode = partial(exit_dialog_mode, b'convert')
    ...
    @listens(b'selected_track.devices')
    def __on_devices_changed(self):
        self._disable_and_check_enabled_state()
        self._update_drum_pad_listeners()
    ...
    @listens(b'selected_drum_pad.chains')
    ...
    def __on_clip_playing_status_changed(self):
        self._disable_and_check_enabled_state()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/convert.pyc
