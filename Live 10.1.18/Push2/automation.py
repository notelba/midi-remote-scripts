...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\automation.py
...
class AutomationComponent(AutomationComponentBase):
    ...
    def device(self):
        device = None
        if hasattr(self.parameter_provider, b'device'):
            device = self.parameter_provider.device()
        return device

    def _on_parameter_provider_changed(self, provider):
        self.notify_device()
        self.__on_device_changed.subject = provider if getattr(self.parameter_provider, b'device', None) is not None else None
        return
    ...
    def __on_device_changed(self):
        self.notify_device()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/automation.pyc
