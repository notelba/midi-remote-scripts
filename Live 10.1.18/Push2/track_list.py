...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\track_list.py
    ...
    if liveobj_valid(track) and not is_chain(track) and bool(track.clip_slots):
        if is_clip_stop_pending(track):
            ...
            if track.playing_slot_index >= 0:
                if track.solo:
                    return b'StopClips.SoloedTrack'
                if track.mute:
                    return b'StopClips.MutedTrack'
            ...
            return b'Session.StoppedClip'

    else:
        return b'Mixer.NoTrack'
...
def playing_clip(track):
    if hasattr(track, b'playing_slot_index'):
        try:
            if track.playing_slot_index >= 0:
...
class TrackListComponent(ModesComponent, Messenger):
    ...
    def __init__(self, tracks_provider=None, trigger_recording_on_release_callback=nop, color_chooser=None, clip_phase_enabler=None, *a, **k):
        ...
        self._playheads_real_time_data = [ RealTimeDataComponent(parent=self, channel_type=b'playhead', is_enabled=False) for _ in xrange(8)
                                         ]
        self.clip_phase_enabler = Component(parent=self)
        self.__on_clip_phase_enabler_changed.subject = self.clip_phase_enabler
        self._setup_action_mode(b'select', handler=self._select_mixable)
        self._setup_action_mode(b'lock_override', handler=self._select_mixable)
        self._setup_action_mode(b'delete', handler=self._delete_mixable)
        self._setup_action_mode(b'duplicate', handler=self._duplicate_mixable)
        self._setup_action_mode(b'arm', handler=self._arm_track)
        self._setup_action_mode(b'mute', handler=partial(toggle_mixable_mute, song=self.song))
        self._setup_action_mode(b'solo', handler=partial(toggle_mixable_solo, song=self.song))
        self._setup_action_mode(b'stop', handler=self._stop_track_clip, feedback_provider=stop_clip_button_color)
        self._setup_action_mode(b'select_color', handler=self._select_mixable_color, exit_handler=partial(self._select_mixable_color, None))
        self.selected_mode = b'select'
    ...
    def _setup_action_mode(self, name, handler, exit_handler=nop, feedback_provider=mixable_button_color):
        ...
        self.get_mode_button(name).mode_selected_color = b'DefaultButton.Transparent'
        self.get_mode_button(name).mode_unselected_color = b'DefaultButton.Transparent'
    ...
    @listens_group(b'muted_via_solo')
    ...
    def track_action_buttons(self, button):
        ...
        if self.selected_mode != b'select':
    ...
    def track_action_buttons(self, button):
        if self.selected_mode == b'select' and self._track_selected_when_pressed[button.index]:
            self._toggle_track_fold(self.tracks[button.index])

    def _toggle_track_fold(self, track):
        if hasattr(track, b'is_foldable') and track.is_foldable:
            track.fold_state = not track.fold_state
        elif hasattr(track, b'is_showing_chains') and track.can_show_chains:
            track.is_showing_chains = not track.is_showing_chains
        else:
            instruments = list(find_instrument_devices(track))
            if instruments:
                instrument = instruments[0]
                if hasattr(instrument, b'is_showing_chains') and instrument.can_show_chains:
    ...
    def on_enabled_changed(self):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/track_list.pyc
