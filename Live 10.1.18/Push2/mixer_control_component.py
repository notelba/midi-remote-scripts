...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\mixer_control_component.py
...
class MixerControlComponent(ModesComponent):
    ...
    cycle_sends_button = ButtonControl(color=b'DefaultButton.Off')
    ...
    def __init__(self, view_model=None, tracks_provider=None, real_time_mapper=None, register_real_time_data=None, *a, **k):
        ...
        self._send_offset = 0
        self.real_time_meter_handlers = [ RealTimeDataComponent(channel_type=b'meter', real_time_mapper=real_time_mapper, register_real_time_data=register_real_time_data, is_enabled=False) for _ in xrange(8)
                                        ]
        ...
        self.selected_mode = b'volume'
        self._selected_item = b''
        self._items = []
    ...
    def _setup_modes(self, view_model):
        self._add_mode(b'volume', view_model.volumeControlListView, lambda mixer: mixer.volume, additional_mode_contents=self.real_time_meter_handlers)
        self._add_mode(b'panning', view_model.panControlListView, lambda mixer: ConstantParameter(original_parameter=mixer.panning) if is_set_to_split_stereo(mixer) else mixer.panning)
    ...
    def _add_mode(self, mode, view, parameter_getter, additional_mode_contents=[]):
        ...
        mode_button.mode_selected_color = b'MixerControlView.SectionSelected'
        mode_button.mode_unselected_color = b'MixerControlView.SectionUnSelected'
    ...
    def set_mixer_section(self, mixer_section):
        ...
        if self._mixer_section_view:
            self._mixer_section_view.model.mode = b'Global'
            self._update_mixer_sections()
    ...
    def _update_mixer_sections(self):
        if self.is_enabled():
            ...
            if self.number_sends > SEND_LIST_LENGTH:
                ...
                self._mixer_sections[7] = IconItemSlot(icon=b'page_right.svg')
            self.notify_items()
            if self.selected_mode in SEND_MODE_NAMES:
                index = SEND_MODE_NAMES.index(self.selected_mode)
    ...
    def cycle_sends_button(self, button):
        button.color = b'MixerControlView.SectionSelected'

    @cycle_sends_button.released
    def cycle_sends_button(self, button):
        button.color = b'MixerControlView.SectionUnSelected'
    ...
    def _updating_send_offset_mode_selection(self):
        ...
        self._update_buttons(self.selected_mode)
        self._update_controls(self._parameter_getter, self._selected_view)
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/mixer_control_component.pyc
