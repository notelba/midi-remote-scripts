...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\drum_pad_parameter_component.py
...
class ChokeParameter(InternalParameterBase):
    ...
    def __init__(self, drum_pad=None, *a, **k):
        super(ChokeParameter, self).__init__(name=b'Choke', *a, **k)
...
class DrumPadTransposeParameter(EnumWrappingParameter):
    ...
    def __init__(self, drum_pad=None, *a, **k):
        super(DrumPadTransposeParameter, self).__init__(name=b'Transpose', values_host=self, values_property=b'available_transpose_steps', index_property_host=get_first_chain(drum_pad), index_property=b'out_note', *a, **k)
    ...
    def display_value(self):
        ...
        sign = b'-' if difference < 0 else b'+' if difference > 0 else b''
        return sign + unicode(abs(difference)) + b' st'
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/drum_pad_parameter_component.pyc
