...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\bank_selection_component.py
...
class BankProvider(ItemProvider):
    ...
    def set_device(self, device):
        if self._device != device:
            ...
            self._on_bank_names_changed.subject = self._device if has_event(self._device, b'bank_parameters_changed') else None
...
class EditModeOptionsComponent(Component):
    color_class_name = b'EditModeOptions'
    option_buttons = control_list(ButtonControl, color=color_class_name + b'.ItemSelected', control_count=8)
    ...
    def _update_button_feedback(self):
        for button in self.option_buttons:
            if button.index > 0:
                ...
                button.color = self.color_class_name + b'.' + (b'ItemNotSelected' if has_active_option else b'NoItem')
...
class BankSelectionComponent(ItemListerComponent):
    color_class_name = b'BankSelection'
    __events__ = ('back', )
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/bank_selection_component.pyc
