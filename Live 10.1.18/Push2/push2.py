...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\push2.py
...
def make_dialog_layer(priority=consts.DIALOG_PRIORITY, *a, **k):
    return (
     BackgroundLayer(b'global_param_controls', b'select_buttons', b'track_state_buttons', priority=priority),
...
class Push2(IdentifiableControlSurface, PushBase):
    drum_group_note_editor_skin = b'DrumGroupNoteEditor'
    slicing_note_editor_skin = b'SlicingNoteEditor'
    drum_group_velocity_levels_skin = b'DrumGroupVelocityLevels'
    slicing_velocity_levels_skin = b'VelocityLevels'
    note_layout_button = b'layout_button'
    input_target_name_for_auto_arm = b'Push2 Input'
    ...
    def __init__(self, c_instance=None, model=None, bank_definitions=None, decoupled_parameter_list_change_notifications=False, *a, **k):
        ...
        logger.info(b'Push 2 script loaded')
        return
    ...
    def on_process_state_changed(self, state):
        logger.debug(b'Process state changed %r' % state)
    ...
    def on_user_data_arrived(self, message):
        if self._initialized:
            logger.debug(b'User data arrived %r' % message)
            data = json.loads(message)
            self._process_qml_errors(data)
            self._firmware_update.process_firmware_response(data)

    def _process_qml_errors(self, data):
        qmlerrors = [ entry for entry in data if entry[b'type'] == b'qmlerror' ]
        if qmlerrors:
            first_error = qmlerrors[0]
            line = first_error[b'line']
            url = first_error[b'url']
            description = first_error[b'description'].replace(b'"', b'\\"')
            code = b'\n' * (line - 1) + b'raise QmlError("%s")' % description
            exec compile(code, url, b'exec')

    def disconnect(self):
        super(Push2, self).disconnect()
        self.__dict__.clear()
        logger.info(b'Push 2 script unloaded')
    ...
    def _create_background_layer(self):
        return super(Push2, self)._create_background_layer() + Layer(mix_button=b'mix_button', page_left_button=b'page_left_button', page_right_button=b'page_right_button', mute_button=b'global_mute_button', solo_button=b'global_solo_button', track_stop_button=b'global_track_stop_button', convert_button=b'convert_button', layout_button=b'layout_button', setup_button=b'setup_button')

    def _create_message_box_background_layer(self):
        return super(Push2, self)._create_message_box_background_layer() + BackgroundLayer(b'mix_button', b'page_left_button', b'page_right_button', b'convert_button', b'layout_button', b'setup_button')

    def _create_message_box_layer(self):
        return Layer(cancel_button=b'track_state_buttons_raw[0]', priority=consts.MESSAGE_BOX_PRIORITY)

    def _create_note_settings_component_layer(self):
        return Layer(full_velocity_button=b'accent_button', priority=consts.MOMENTARY_DIALOG_PRIORITY)
    ...
    def _create_convert(self):
        convert = ConvertComponent(decorator_factory=self._device_decorator_factory, name=b'Convert', tracks_provider=self._session_ring, is_enabled=False, layer=make_dialog_layer(action_buttons=b'select_buttons', cancel_button=b'track_state_buttons_raw[0]'))
    ...
    def _select_note_mode(self):
        ...
        self._note_editor_settings_component.settings.set_color_mode(b'drum_pad' if self._note_modes.selected_mode == b'drums' else b'clip')
    ...
    def _init_convert_enabler(self):
        ...
        self._convert_enabler.layer = Layer(convert_toggle_button=b'convert_button')
    ...
    def __on_convert_suceeded(self, action_name):
        if action_name == b'audio_clip_to_simpler':
    ...
    def _init_main_modes(self):
        ...
        self._main_modes.add_mode(b'user', [
         ...
         SetAttributeMode(obj=self._user, attribute=b'mode', value=sysex.USER_MODE)], behaviour=UserButtonBehavior(user_component=self._user))
    ...
    def _exit_modal_modes(self):
        self._dialog_modes.selected_mode = None
        self._setup_enabler.selected_mode = b'disabled'
        return

    def _create_capture_and_insert_scene_component(self):
        return CaptureAndInsertSceneComponent(name=b'Capture_And_Insert_Scene', decorator_factory=self._clip_decorator_factory)
    ...
    def _init_mute_solo_stop(self):
        self._mute_solo_stop = MuteSoloStopClipComponent(item_provider=self._session_ring, track_list_component=self._track_list_component, cancellation_action_performers=[
         self._device_navigation, self._drum_component] + self._note_editor_settings_component.editors, solo_track_button=b'global_solo_button', mute_track_button=b'global_mute_button', stop_clips_button=b'global_track_stop_button')
        self._mute_solo_stop.layer = Layer(stop_all_clips_button=self._with_shift(b'global_track_stop_button'))
        self._master_selector = MasterTrackComponent(tracks_provider=self._session_ring, is_enabled=False, layer=Layer(toggle_button=b'master_select_button'))
    ...
    def _create_loop_navigation_layer(self):
        return Layer(prev_loop_page_button=b'page_left_button', next_loop_page_button=b'page_right_button')

    def _create_color_chooser(self):
        color_chooser = ColorChooserComponent()
        color_chooser.layer = Layer(matrix=b'matrix', priority=consts.MOMENTARY_DIALOG_PRIORITY)
    ...
    def _create_session(self):
        ...
        for scene_ix in xrange(8):
            ...
            for track_ix in xrange(8):
                ...
                clip_slot.layer += Layer(select_color_button=b'shift_button')
                clip_slot.set_decorator_factory(self._clip_decorator_factory)

        return session

    def _create_session_navigation_layer(self):
        return Layer(left_button=b'nav_left_button', right_button=b'nav_right_button', up_button=b'nav_up_button', down_button=b'nav_down_button', page_left_button=b'page_left_button', page_right_button=b'page_right_button', page_up_button=b'octave_up_button', page_down_button=b'octave_down_button')

    def on_select_clip_slot(self, clip_slot):
        self.show_notification(b'Clip Selected: ' + select_clip_and_get_name_from_slot(clip_slot, self.song))

    def on_select_scene(self, scene):
        self.show_notification(b'Scene Selected: ' + select_scene_and_get_name(scene, self.song))

    def _create_session_mode(self):
        session_modes = MessengerModesComponent(muted=True, is_enabled=False)
        session_modes.add_mode(b'session', [
         self._session_mode], message=consts.MessageBoxText.LAYOUT_SESSION_VIEW)
        session_modes.add_mode(b'overview', [
         self._session_overview_mode], message=consts.MessageBoxText.LAYOUT_SESSION_OVERVIEW)
        session_modes.selected_mode = b'session'
        simple_mode_switcher = SimpleModeSwitcher(session_modes, is_enabled=False, layer=Layer(cycle_button=self.note_layout_button, lock_button=ComboElement(self.note_layout_button, modifier=b'shift_button')))
        return [
         session_modes, self._session_navigation, simple_mode_switcher]

    def _create_session_overview_layer(self):
        return Layer(button_matrix=b'matrix')
    ...
    def _create_drum_component(self):
        return DrumGroupComponent(name=b'Drum_Group', is_enabled=False, tracks_provider=self._session_ring, device_decorator_factory=self._device_decorator_factory, quantizer=self._quantize, color_chooser=self._create_color_chooser())

    def _init_drum_component(self):
        super(Push2, self)._init_drum_component()
        self._drum_component.layer += Layer(select_color_button=b'shift_button')

    def _create_device_mode(self):
        self._model.deviceVisualisation = self._device_component
        self._drum_pad_parameter_component = DrumPadParameterComponent(device_component=self._device_component, view_model=self._model, is_enabled=False, layer=Layer(choke_encoder=b'parameter_controls_raw[0]', transpose_encoder=b'parameter_controls_raw[1]'))
        self._device_or_pad_parameter_chooser = ModesComponent()
        self._device_or_pad_parameter_chooser.add_mode(b'device', [
         make_freeze_aware(self._device_parameter_component, self._device_parameter_component.layer),
         self._device_view])
        self._device_or_pad_parameter_chooser.add_mode(b'drum_pad', [
         make_freeze_aware(self._drum_pad_parameter_component, self._drum_pad_parameter_component.layer)])
        self._device_or_pad_parameter_chooser.selected_mode = b'device'
        return [
         partial(self._view_control.show_view, b'Detail/DeviceChain'),
         ...
         SetAttributeMode(obj=self._note_editor_settings_component, attribute=b'parameter_provider', value=self._device_component),
    ...
    def _setup_freeze_aware_device_navigation(self):
        ...
        return make_freeze_aware(self._device_navigation, self._device_navigation.layer, default_mode_extras=[
         create_layer_setter(b'scroll_right_layer', Layer(button=self.elements.track_state_buttons_raw[(-1)])),
         create_layer_setter(b'scroll_left_layer', Layer(button=self.elements.track_state_buttons_raw[0]))], frozen_mode_extras=[
         lambda : setattr(self._device_navigation.modes, b'selected_mode', b'default'),
         create_layer_setter(b'scroll_right_layer', Layer()),
         create_layer_setter(b'scroll_left_layer', Layer())])

    @listens(b'drum_pad_selection')
    def __on_drum_pad_selection_changed(self):
        ...
        new_mode = b'drum_pad' if show_pad_parameters else b'device'
    ...
    def _init_browse_mode(self):
        ...
        self._main_modes.add_mode(b'browse', [
        ...
        self._main_modes.add_mode(b'add_device', [
        ...
        self._main_modes.add_mode(b'add_track', [
    ...
    def _create_browser_layer(self):
        return (
         BackgroundLayer(b'select_buttons', b'track_state_buttons', priority=consts.DIALOG_PRIORITY),
         Layer(up_button=b'nav_up_button', down_button=b'nav_down_button', right_button=b'nav_right_button', left_button=b'nav_left_button', back_button=b'track_state_buttons_raw[-2]', open_button=b'track_state_buttons_raw[-1]', load_button=b'select_buttons_raw[-1]', scroll_encoders=self.elements.global_param_controls.submatrix[:-1, :], scroll_focused_encoder=b'parameter_controls_raw[-1]', close_button=b'track_state_buttons_raw[0]', prehear_button=b'track_state_buttons_raw[1]', priority=consts.DIALOG_PRIORITY))

    def _create_browser(self):
        browser = BrowserComponent(name=b'Browser', is_enabled=False, preferences=self.preferences, main_modes_ref=weakref.ref(self._main_modes), layer=self._create_browser_layer())
        self._on_browser_loaded.add_subject(browser)
        self._on_browser_closed.add_subject(browser)
        browser.load_neighbour_overlay.layer = Layer(load_previous_button=b'track_state_buttons_raw[7]', load_next_button=b'select_buttons_raw[7]', priority=consts.DIALOG_PRIORITY)
        return browser

    def _create_new_track_browser(self):
        browser = NewTrackBrowserComponent(name=b'NewTrackBrowser', is_enabled=False, preferences=self.preferences, layer=self._create_browser_layer())
        self._on_browser_loaded.add_subject(browser)
        self._on_browser_closed.add_subject(browser)
        return browser

    @listens_group(b'loaded')
    def _on_browser_loaded(self, sender):
        if sender.browse_for_audio_clip:
            self._main_modes.selected_mode = b'clip'
        else:
    ...
    @listens_group(b'close')
    def _on_browser_closed(self, sender):
        if sender.browse_for_audio_clip:
            self._main_modes.selected_mode = b'clip'
        elif self._main_modes.selected_mode == b'add_track':
    ...
    def _determine_mix_mode(self):
        ...
        if selected_mode == b'mix':
            if self._is_on_master():
                if mix_mode == b'global':
                    self._mix_modes.push_mode(b'track')
            elif mix_mode == b'track' and b'global' in self._mix_modes.active_modes:
                self._mix_modes.pop_mode(b'track')
    ...
    def _create_device_component(self):
        device_component_layer = Layer(parameter_touch_buttons=ButtonMatrixElement(rows=[
         self.elements.global_param_touch_buttons_raw]), shift_button=b'shift_button')
        return DeviceComponentProvider(device_component_layer=device_component_layer, device_decorator_factory=self._device_decorator_factory, device_bank_registry=self._device_bank_registry, banking_info=self._banking_info, name=b'DeviceComponent', is_enabled=False, delete_button=self.elements.delete_button, decoupled_parameter_list_change_notifications=self._decoupled_parameter_list_change_notifications)

    def _create_device_parameter_component(self):
        return DeviceParameterComponent(parameter_provider=self._device_component, is_enabled=False, layer=Layer(parameter_controls=b'fine_grain_param_controls'))

    def _create_device_navigation(self):
        device_navigation = DeviceNavigationComponent(name=b'DeviceNavigation', device_bank_registry=self._device_bank_registry, banking_info=self._banking_info, device_component=self._device_component, delete_handler=self._delete_component, track_list_component=self._track_list_component, is_enabled=False, layer=Layer(select_buttons=b'track_state_buttons'))
        device_navigation.scroll_left_layer = Layer(button=b'track_state_buttons_raw[0]')
        device_navigation.scroll_right_layer = Layer(button=b'track_state_buttons_raw[-1]')
        device_navigation.move_device.layer = Layer(move_encoders=b'global_param_controls')
        device_navigation.chain_selection.layer = Layer(select_buttons=b'select_buttons', priority=consts.DIALOG_PRIORITY)
        device_navigation.chain_selection.scroll_left_layer = Layer(button=b'select_buttons_raw[0]', priority=consts.DIALOG_PRIORITY)
        device_navigation.chain_selection.scroll_right_layer = Layer(button=b'select_buttons_raw[-1]', priority=consts.DIALOG_PRIORITY)
        device_navigation.bank_selection.layer = Layer(option_buttons=b'track_state_buttons', select_buttons=b'select_buttons', priority=consts.DIALOG_PRIORITY)
        device_navigation.bank_selection.scroll_left_layer = Layer(button=b'select_buttons_raw[0]', priority=consts.DIALOG_PRIORITY)
        device_navigation.bank_selection.scroll_right_layer = Layer(button=b'select_buttons_raw[-1]', priority=consts.DIALOG_PRIORITY)
    ...
    def _init_device(self):
        ...
        self._device_view = DeviceViewComponent(name=b'DeviceView', device_component=self._device_component, view_model=self._model, is_enabled=False)
    ...
    def _create_view_control_component(self):
        return ViewControlComponent(name=b'View_Control', tracks_provider=self._session_ring)

    def _init_session_ring(self):
        self._session_ring = SessionRingTrackProvider(name=b'Session_Ring', num_tracks=NUM_TRACKS, num_scenes=NUM_SCENES, is_enabled=True)
    ...
    def _create_main_mixer_modes(self):
        self._mixer_control = MixerControlComponent(name=b'Global_Mix_Component', view_model=self._model.mixerView, tracks_provider=self._session_ring, is_enabled=False, layer=Layer(controls=b'fine_grain_param_controls', volume_button=b'track_state_buttons_raw[0]', panning_button=b'track_state_buttons_raw[1]', send_slot_one_button=b'track_state_buttons_raw[2]', send_slot_two_button=b'track_state_buttons_raw[3]', send_slot_three_button=b'track_state_buttons_raw[4]', send_slot_four_button=b'track_state_buttons_raw[5]', send_slot_five_button=b'track_state_buttons_raw[6]', cycle_sends_button=b'track_state_buttons_raw[7]'))
        self._model.mixerView.realtimeMeterData = self._mixer_control.real_time_meter_handlers
        track_mixer_control = TrackMixerControlComponent(name=b'Track_Mix_Component', is_enabled=False, tracks_provider=self._session_ring, layer=Layer(controls=b'fine_grain_param_controls', scroll_left_button=b'track_state_buttons_raw[6]', scroll_right_button=b'track_state_buttons_raw[7]'))
        routing_control = RoutingControlComponent(is_enabled=False, layer=Layer(monitor_state_encoder=b'parameter_controls_raw[0]', input_output_choice_encoder=b'parameter_controls_raw[1]', routing_type_encoder=b'parameter_controls_raw[2]', routing_channel_encoders=self.elements.global_param_controls.submatrix[
         3:7, :], routing_channel_position_encoder=b'parameter_controls_raw[7]'))
        track_mix_or_routing_chooser = TrackOrRoutingControlChooserComponent(tracks_provider=self._session_ring, track_mixer_component=track_mixer_control, routing_control_component=routing_control, is_enabled=False, layer=Layer(mix_button=b'track_state_buttons_raw[0]', routing_button=b'track_state_buttons_raw[1]'))
        self._model.mixerView.trackControlView = track_mix_or_routing_chooser
        self._mix_modes = ModesComponent(is_enabled=False)
        self._mix_modes.add_mode(b'global', self._mixer_control)
        self._mix_modes.add_mode(b'track', track_mix_or_routing_chooser)
        self._mix_modes.selected_mode = b'global'
        ...
        class MixModeBehaviour(ReenterBehaviour):
            ...
            def press_immediate(behaviour_self, component, mode):
                if self._is_on_master() and self._mix_modes.selected_mode != b'track':
                    self._mix_modes.selected_mode = b'track'
        ...
        self._main_modes.add_mode(b'mix', [
         self._mix_modes,
         SetAttributeMode(obj=self._note_editor_settings_component, attribute=b'parameter_provider', value=self._track_parameter_provider),
         self._clip_phase_enabler], behaviour=MixModeBehaviour())

    def _init_dialog_modes(self):
        self._dialog_modes = ModesComponent()
        self._dialog_modes.add_mode(b'convert', LazyEnablingMode(self._create_convert))
    ...
    def _create_scales(self):
        ...
        scales = ScalesComponent(note_layout=self._note_layout, is_enabled=False, layer=make_dialog_layer(root_note_buttons=root_note_buttons, in_key_toggle_button=b'select_buttons_raw[0]', fixed_toggle_button=b'select_buttons_raw[-1]', scale_encoders=self.elements.global_param_controls.submatrix[1:-1, :], layout_encoder=b'parameter_controls_raw[0]', direction_encoder=b'parameter_controls_raw[-1]', up_button=b'nav_up_button', down_button=b'nav_down_button', right_button=b'nav_right_button', left_button=b'nav_left_button'))
        self._model.scalesView = scales
        return scales

    def _init_scales(self):
        self._dialog_modes.add_mode(b'scales', self._create_scales())
        super(Push2, self)._init_scales()

    def _create_scales_enabler(self):
        return ScalesEnabler(enter_dialog_mode=self._enter_dialog_mode, exit_dialog_mode=self._exit_dialog_mode, is_enabled=False, layer=Layer(toggle_button=b'scale_presets_button'))
    ...
    def _create_clip_mode(self):
        base_loop_layer = Layer(shift_button=b'shift_button', loop_button=b'track_state_buttons_raw[1]', crop_button=b'track_state_buttons_raw[2]')
        audio_clip_layer = Layer(warp_mode_encoder=b'parameter_controls_raw[5]', transpose_encoder=b'parameter_controls_raw[6]', detune_encoder=self._with_shift(b'parameter_controls_raw[6]'), gain_encoder=b'parameter_controls_raw[7]', shift_button=b'shift_button')
        self._clip_control.mode_selector.add_mode(b'midi', [
         make_freeze_aware(self._clip_control.midi_loop_controller, base_loop_layer + Layer(encoders=self.elements.global_param_controls.submatrix[1:4, :], zoom_encoder=b'fine_grain_param_controls_raw[0]')),
         self._clip_control.midi_clip_controller])
        self._clip_control.mode_selector.add_mode(b'audio', [
         make_freeze_aware(self._clip_control.audio_loop_controller, base_loop_layer + Layer(encoders=self.elements.global_param_controls.submatrix[1:4, :], zoom_encoder=b'fine_grain_param_controls_raw[0]')),
         make_freeze_aware(self._clip_control.audio_clip_controller, audio_clip_layer)])
        self._clip_control.mode_selector.add_mode(b'no_clip', [])
        self._clip_control.mode_selector.selected_mode = b'no_clip'
        ...
        return [
         partial(self._view_control.show_view, b'Detail/Clip'),
         self._clip_control,
         self._clip_phase_enabler]

    def _init_quantize_actions(self):
        self._quantize = self._for_non_frozen_tracks(QuantizationComponent(name=b'Selected_Clip_Quantize', quantization_names=QUANTIZATION_NAMES_UNICODE, settings_class=self.quantization_settings_class, is_enabled=False, layer=Layer(action_button=b'quantize_button')))
        self._quantize.settings.layer = make_dialog_layer(swing_amount_encoder=b'parameter_controls_raw[0]', quantize_to_encoder=b'parameter_controls_raw[1]', quantize_amount_encoder=b'parameter_controls_raw[2]', record_quantization_encoder=b'parameter_controls_raw[4]', record_quantization_toggle_button=b'track_state_buttons_raw[4]', priority=consts.MOMENTARY_DIALOG_PRIORITY)
        self._model.quantizeSettingsView = self._quantize.settings

    def _init_fixed_length(self):
        super(Push2, self)._init_fixed_length()
        self._fixed_length.settings_component.layer = make_dialog_layer(length_option_buttons=b'select_buttons', fixed_length_toggle_button=b'track_state_buttons_raw[0]', legato_launch_toggle_button=b'track_state_buttons_raw[7]', priority=consts.MOMENTARY_DIALOG_PRIORITY)
    ...
    def _create_main_modes_layer(self):
        return Layer(mix_button=b'mix_button', clip_button=b'clip_mode_button', device_button=b'device_mode_button', browse_button=b'browse_mode_button', add_device_button=b'create_device_button', add_track_button=b'create_track_button') + Layer(user_button=b'user_button', priority=consts.USER_BUTTON_PRIORITY)
    ...
    def _init_setup_component(self):
        self._setup_settings.general.workflow = b'scene' if self._settings[b'workflow'].value else b'clip'
        self.__on_workflow_setting_changed.subject = self._setup_settings.general
        setup = SetupComponent(name=b'Setup', settings=self._setup_settings, pad_curve_sender=self._pad_curve_sender, firmware_switcher=self._firmware_switcher, is_enabled=False, layer=make_dialog_layer(category_radio_buttons=b'select_buttons', priority=consts.SETUP_DIALOG_PRIORITY, make_it_go_boom_button=b'track_state_buttons_raw[7]'))
        setup.general.layer = Layer(workflow_encoder=b'parameter_controls_raw[0]', display_brightness_encoder=b'parameter_controls_raw[1]', led_brightness_encoder=b'parameter_controls_raw[2]', priority=consts.SETUP_DIALOG_PRIORITY)
        setup.info.layer = Layer(install_firmware_button=b'track_state_buttons_raw[6]', priority=consts.SETUP_DIALOG_PRIORITY)
        setup.pad_settings.layer = Layer(sensitivity_encoder=b'parameter_controls_raw[4]', gain_encoder=b'parameter_controls_raw[5]', dynamics_encoder=b'parameter_controls_raw[6]', priority=consts.SETUP_DIALOG_PRIORITY)
        setup.display_debug.layer = Layer(show_row_spaces_button=b'track_state_buttons_raw[0]', show_row_margins_button=b'track_state_buttons_raw[1]', show_row_middle_button=b'track_state_buttons_raw[2]', show_button_spaces_button=b'track_state_buttons_raw[3]', show_unlit_button_button=b'track_state_buttons_raw[4]', show_lit_button_button=b'track_state_buttons_raw[5]', priority=consts.SETUP_DIALOG_PRIORITY)
        self._model.setupView = setup
        self._setup_enabler = EnablingModesComponent(component=setup, enabled_color=b'DefaultButton.On', disabled_color=b'DefaultButton.On')
        self._setup_enabler.layer = Layer(cycle_mode_button=b'setup_button')
    ...
    def __on_workflow_setting_changed(self, value):
        self._settings[b'workflow'].value = value == b'scene'
    ...
    def _create_pad_sensitivity_update(self):
        ...
        self._pad_sensitivity_update.set_profile(b'drums', PadSettings(sensitivity=playing_profile, aftertouch_enabled=MONO_AFTERTOUCH_ENABLED))
        self._pad_sensitivity_update.set_profile(b'instrument', PadSettings(sensitivity=playing_profile, aftertouch_enabled=MONO_AFTERTOUCH_ENABLED))
        self._pad_sensitivity_update.set_profile(b'loop', PadSettings(sensitivity=loop_selector_profile, aftertouch_enabled=MONO_AFTERTOUCH_DISABLED))
    ...
    def _update_encoder_model(self):
        self._model.controls.encoders = [ NamedTuple(__id__=b'encoder_%i' % i, touched=e.is_pressed()) for i, e in enumerate(self.elements.global_param_touch_buttons_raw)
                                        ]
    ...
    def on_identified(self, response_bytes):
        try:
            ...
            logger.info(b'Push 2 identified')
            logger.info(b'Firmware %i.%i Build %i' % (major, minor, build))
            logger.info(b'Serial number %i' % sn)
            logger.info(b'Board Revision %i' % board_revision)
            self._board_revision = board_revision
            self._identified = True
            self._try_initialize()
        except IndexError:
            logger.warning(b"Couldn't identify Push 2 unit")
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/push2.pyc
