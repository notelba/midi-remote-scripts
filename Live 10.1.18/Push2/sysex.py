...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\sysex.py
...
def make_aftertouch_mode_message(mode_id):
    ...
    mode_byte = 0 if mode_id == b'mono' else 1
...
def make_touch_strip_mode_message(mode):
    ...
    if mode == TouchStripModes.CUSTOM_PITCHBEND:
        mode_bytes = int(b'1111001', 2)
    elif mode == TouchStripModes.CUSTOM_VOLUME:
        mode_bytes = int(b'0000001', 2)
    elif mode == TouchStripModes.CUSTOM_PAN:
        mode_bytes = int(b'0010001', 2)
    elif mode == TouchStripModes.CUSTOM_DISCRETE:
        mode_bytes = int(b'0011001', 2)
    elif mode == TouchStripModes.CUSTOM_FREE:
        mode_bytes = int(b'0001011', 2)
    elif mode == TouchStripModes.MODWHEEL:
        mode_bytes = int(b'0000100', 2)
    elif mode == TouchStripModes.PITCHBEND:
        mode_bytes = int(b'1111000', 2)
    else:
        raise RuntimeError(b'Touch strip mode %i not supported' % mode)
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Push2/sysex.pyc
