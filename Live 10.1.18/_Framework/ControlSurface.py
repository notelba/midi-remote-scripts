...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\ControlSurface.py
...
class ControlSurface(Subject, SlotManager):
    ...
    def __init__(self, c_instance=None, publish_self=True, *a, **k):
        ...
        self.log_message(b'Initializing...')
        self._pad_translations = None
        self._suggested_input_port = str(b'')
        self._suggested_output_port = str(b'')
        self._components = []
        self._displays = []
        self.controls = []
        ...
        self.register_slot(self.song(), self._on_track_list_changed, b'visible_tracks')
        self.register_slot(self.song(), self._on_scene_list_changed, b'scenes')
        self.register_slot(self.song().view, self._on_selected_track_changed, b'selected_track')
        self.register_slot(self.song().view, self._on_selected_scene_changed, b'selected_scene')
        return
    ...
    def log_message(self, *message):
        ...
        message = b'(%s) %s' % (self.__class__.__name__, (b' ').join(map(str, message)))
        console_message = b'LOG: ' + message
    ...
    def handle_nonsysex(self, midi_bytes):
        ...
        if recipient is not None:
            ...
            self.log_message(b'Got unknown message: ' + str(midi_bytes))
        return
    ...
    def handle_sysex(self, midi_bytes):
        ...
        if result != None:
            ...
            self.log_message(b'Got unknown sysex message: ' + str(midi_bytes))
        return
    ...
    def _register_control(self, control):
        ...
        assert control != None
        assert control not in self.controls, b'Control registered twice'
    ...
    def _register_component(self, component):
        ...
        assert component not in self._components, b'Component registered twice'
    ...
    def _do_send_midi(self, midi_event_bytes):
        try:
            ...
            self.log_message(b'Error while sending midi message', midi_event_bytes)
            traceback.print_exc()
            return False
    ...
    def _install_forwarding(self, midi_map_handle, control):
        ...
        if success:
            ...
            for key in forwarding_keys:
                ...
                assert key not in registry.keys(), b'Registry key %s registered twice. Check Midi messages!' % str(key)
...
class OptimizedControlSurface(ControlSurface):
    ...
    def _register_control(self, control):
        ...
        if hasattr(control, b'_is_resource_based'):
            control._is_resource_based = True
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/ControlSurface.pyc
