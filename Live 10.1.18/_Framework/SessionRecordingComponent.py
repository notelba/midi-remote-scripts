...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\SessionRecordingComponent.py
...
class SessionRecordingComponent(CompoundComponent):
    ...
    def __init__(self, clip_creator=None, view_controller=None, *a, **k):
        ...
        song = self.song()
        self._automation_toggle, self._re_enable_automation_toggle, self._delete_automation = self.register_components(ToggleComponent(b'session_automation_record', song), ToggleComponent(b're_enable_automation_enabled', song, read_only=True), ToggleComponent(b'has_envelopes', None, read_only=True))
    ...
    def _update_generic_new_button(self, new_button):
        if new_button and self.is_enabled():
            ...
            new_button.set_light(can_new or b'DefaultButton.Disabled')
        return
    ...
    def _update_new_scene_button(self):
        if self._new_scene_button and self.is_enabled():
            ...
            self._new_scene_button.set_light(can_new or b'DefaultButton.Disabled')
    ...
    def _update_record_button(self):
        if self._record_button and self.is_enabled():
            ...
            if status == Live.Song.SessionRecordStatus.transition:
                self._record_button.set_light(b'Recording.Transition')
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/SessionRecordingComponent.pyc
