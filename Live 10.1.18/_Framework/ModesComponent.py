...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\ModesComponent.py
...
class ExcludingBehaviourMixin(ModeButtonBehaviour):
    ...
    def update_button(self, component, mode, selected_mode):
        if not self.is_excluded(component, selected_mode):
            ...
            component.get_mode_button(mode).set_light(b'DefaultButton.Disabled')
...
class ModesComponent(CompoundComponent):
    ...
    def add_mode(self, name, mode_or_component, toggle_value=False, groups=set(), behaviour=None):
        ...
        if not name not in self._mode_map.keys():
            ...
            task.kill()
            slot = self.register_slot(listener=partial(self._on_mode_button_value, name), event=b'value', extra_kws=dict(identify_sender=True))
            ...
            button_setter = b'set_' + name + b'_button'
...
class EnablingModesComponent(ModesComponent):
    ...
    def __init__(self, component=None, toggle_value=False, disabled_value=False, *a, **k):
        ...
        self.add_mode(b'disabled', None, disabled_value)
        self.add_mode(b'enabled', component, toggle_value)
        self.selected_mode = b'disabled'
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/ModesComponent.pyc
