...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\SessionComponent.py
...
class SessionComponent(CompoundComponent):
    ...
    def _enable_skinning(self):
        self.set_stop_clip_triggered_value(b'Session.StopClipTriggered')
        self.set_stop_clip_value(b'Session.StopClip')
        for scene_index in xrange(self._num_scenes):
            scene = self.scene(scene_index)
            scene.set_scene_value(b'Session.Scene')
            scene.set_no_scene_value(b'Session.NoScene')
            scene.set_triggered_value(b'Session.SceneTriggered')
            for track_index in xrange(self._num_tracks):
                clip_slot = scene.clip_slot(track_index)
                clip_slot.set_triggered_to_play_value(b'Session.ClipTriggeredPlay')
                clip_slot.set_triggered_to_record_value(b'Session.ClipTriggeredRecord')
                clip_slot.set_record_button_value(b'Session.RecordButton')
                clip_slot.set_stopped_value(b'Session.ClipStopped')
                clip_slot.set_started_value(b'Session.ClipStarted')
                clip_slot.set_recording_value(b'Session.ClipRecording')
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/SessionComponent.pyc
