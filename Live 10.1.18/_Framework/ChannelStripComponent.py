...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\ChannelStripComponent.py
...
class ChannelStripComponent(ControlSurfaceComponent):
    ...
    def __init__(self):
        ...
        self._select_button_slot = make_button_slot(b'select')
    ...
    def _update_track_name_data_source(self):
        self._track_name_data_source.set_display_string(self._track.name if self._track != None else b' - ')
        return
    ...
    def _mute_value(self, value):
        if not self._mute_button != None:
            ...
            if not isinstance(value, int):
                raise AssertionError
                if self.is_enabled() and self._track != None and self._track != self.song().master_track and (not self._mute_button.is_momentary() or value != 0):
    ...
    def _on_input_routing_changed(self):
        ...
        if self.is_enabled():
            self._on_arm_changed()
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/ChannelStripComponent.pyc
