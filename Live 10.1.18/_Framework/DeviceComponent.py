...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\DeviceComponent.py
...
class DeviceComponent(ControlSurfaceComponent, Subject):
    ...
    def __init__(self, device_bank_registry=None, device_selection_follows_track_selection=False, *a, **k):
        ...
        self._on_off_property_slot = make_property_slot(b'value', alias=b'device_on_off')
        self._name_property_slot = make_property_slot(b'name', alias=b'device_name')
        self._parameters_property_slot = make_property_slot(b'parameters')
        self._device_bank_property_slot = make_property_slot(b'device_bank')
        ...
        self._lock_button_slot = make_button_slot(b'lock')
        self._on_off_button_slot = make_button_slot(b'on_off')
        song = self.song()
        view = song.view
    ...
    @subject_slot(b'has_macro_mappings')
        ...
        if self.is_enabled() and self._device != None:
            ...
            if self._parameter_controls != None:
                ...
                if self._bank_name != old_bank_name:
                    self._show_msg_callback(self._device.name + b' Bank: ' + self._bank_name)
    ...
    def _bank_down_value(self, value):
        if not self._bank_down_button != None:
            ...
            if not isinstance(value, int):
                raise AssertionError
                if self.is_enabled() and (not self._bank_down_button.is_momentary() or value is not 0) and self._device != None and (self._bank_index == None or self._bank_index > 0):
                    self._bank_name = b''
    ...
    def _on_device_name_changed(self):
        if self._device_name_data_source != None:
            self._device_name_data_source.set_display_string(self._device.name if self.is_enabled() and self._device != None else b'No Device')
        return
    ...
    def _on_off_parameter(self):
        ...
        if self._device != None:
            for parameter in self._device.parameters:
                if str(parameter.name).startswith(b'Device On'):
                    result = parameter
                    break
    ...
    def _current_bank_details(self):
        ...
        if banks:
            if self._bank_index != None and self._is_banking_enabled() or not best_of:
                ...
                bank = best_of
                bank_name = b'Best of Parameters'
        return (
         bank_name, bank)
    ...
    def _release_parameters(self, controls):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/DeviceComponent.pyc
