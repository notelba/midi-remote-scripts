...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\ClipSlotComponent.py
...
class ClipSlotComponent(ControlSurfaceComponent):
    ...
    def _feedback_value(self):
        if self._clip_slot != None:
            ...
            if getattr(slot_or_clip, b'controls_other_clips', True) and self._stopped_value != None:
                return self._stopped_value
    ...
    @subject_slot(b'has_clip')
    def _on_clip_state_changed(self):
        self._update_clip_property_slots()
        self.update()

    @subject_slot(b'controls_other_clips')
    def _on_controls_other_clips_changed(self):
        self._update_clip_property_slots()
        self.update()

    @subject_slot(b'color')
    ...
    @subject_slot(b'has_stop_button')
    def _on_has_stop_button_changed(self):
        self.update()

    @subject_slot(b'is_triggered')
    ...
    def _do_launch_clip(self, value):
        ...
        if launch_pressed and self.has_clip() and self.song().select_on_launch:
            self.song().view.highlighted_clip_slot = self._clip_slot
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/ClipSlotComponent.pyc
