...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\SubjectSlot.py
...
def subject_add_event(cls, event_name_or_event):
    ...
    kwargs = dict({b'doc': event.doc, b'override': event.override})
    ...
    @monkeypatch(cls, (b'clear_' + event.name + b'_listeners'), **kwargs)
...
class SubjectMeta(type):
    ...
    def __new__(cls, name, bases, dct):
        events = dct.get(b'__subject_events__', [])
        if events and b'disconnect' not in dct:
            dct[b'disconnect'] = lambda self: super(cls, self).disconnect()
        cls = super(SubjectMeta, cls).__new__(cls, name, bases, dct)
        assert not events or hasattr(cls, b'disconnect')
...
class SubjectSlot(Disconnectable):
    ...
    def _check_subject_interface(self, subject):
        if not callable(getattr(subject, b'add_' + self._event + b'_listener', None)):
            raise SubjectSlotError(b'Subject %s missing "add" method for event: %s' % (subject, self._event))
        if not callable(getattr(subject, b'remove_' + self._event + b'_listener', None)):
            raise SubjectSlotError(b'Subject %s missing "remove" method for event: %s' % (
             subject, self._event))
        if not callable(getattr(subject, self._event + b'_has_listener', None)):
            raise SubjectSlotError(b'Subject %s missing "has" method for event: %s' % (subject, self._event))
        return

    def connect(self):
        if not self.is_connected and self._subject != None and self._listener != None:
            add_method = getattr(self._subject, b'add_' + self._event + b'_listener')
    ...
    def soft_disconnect(self):
        ...
        if self.is_connected and self._subject != None and self._listener != None:
            all_args = tuple(self._extra_args) + (self._listener,)
            remove_method = getattr(self._subject, b'remove_' + self._event + b'_listener')
            try:
    ...
    def is_connected(self):
        ...
        connected = False
        try:
            connected = bool(self._subject != None and self._listener != None and getattr(self._subject, self._event + b'_has_listener')(*all_args))
        except RuntimeError:
            pass
...
def subject_slot_group(event, *a, **k):
    ...
    return decorator
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/SubjectSlot.pyc
