...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\InputControlElement.py
...
class InputControlElement(NotifyingControlElement):
    ...
    __subject_events__ = (
     SubjectEvent(name=b'value', signal=InputSignal, override=True),)
    ...
    def receive_value(self, value):
        value = getattr(value, b'midi_value', value)
        self._verify_value(value)
        self._last_sent_message = None
    ...
    def _report_value(self, value, is_input):
        self._verify_value(value)
        message = str(self.__class__.__name__) + b' ('
        if self._msg_type == MIDI_NOTE_TYPE:
            message += b'Note ' + str(self._msg_identifier) + b', '
        elif self._msg_type == MIDI_CC_TYPE:
            message += b'CC ' + str(self._msg_identifier) + b', '
        else:
            message += b'PB '
        message += b'Chan. ' + str(self._msg_channel)
        message += b') '
        message += b'received value ' if is_input else b'sent value '
        message += str(value)
    ...
    def _last_sent_value(self):
        ...
        return -1
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/InputControlElement.pyc
