...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\Control.py
...
class Control(object):
    ...
    class State(SlotManager):
        ...
        def __init__(self, control=None, manager=None, channel=None, identifier=None, *a, **k):
            ...
            self._value_listener = control._event_listeners.get(b'value', None)
...
class ButtonControl(Control):
    ...
    REPEAT_RATE = 0.1
    pressed = control_event(b'pressed')
    released = control_event(b'released')
    pressed_delayed = control_event(b'pressed_delayed')
    released_delayed = control_event(b'released_delayed')
    released_immediately = control_event(b'released_immediately')
    ...
    class State(Control.State):
        ...
        def __init__(self, control=None, manager=None, color=None, pressed_color=None, disabled_color=None, repeat=False, enabled=True, *a, **k):
            ...
            self._pressed_listener = control._event_listeners.get(b'pressed', None)
            self._released_listener = control._event_listeners.get(b'released', None)
            self._pressed_delayed_listener = control._event_listeners.get(b'pressed_delayed', None)
            self._released_delayed_listener = control._event_listeners.get(b'released_delayed', None)
            self._released_immediately_listener = control._event_listeners.get(b'released_immediately', None)
            self._repeat = repeat
            self._is_pressed = False
            self._enabled = enabled
            self._color = color if color is not None else b'DefaultButton.On'
            self._disabled_color = disabled_color if disabled_color is not None else b'DefaultButton.Disabled'
...
class ToggleButtonControl(Control):
    ...
    class State(Control.State):
        ...
        def __init__(self, control=None, manager=None, untoggled_color=None, toggled_color=None, *a, **k):
            ...
            self._untoggled_color = untoggled_color or b'DefaultButton.Off'
            self._toggled_color = toggled_color or b'DefaultButton.On'
            self._is_toggled = False
            self._toggled_listener = control._event_listeners.get(b'toggled', None)
            return
...
class RadioButtonControl(Control):
    ...
    class State(Control.State):
        ...
        def __init__(self, control, manager=None, unchecked_color=None, checked_color=None, *a, **k):
            ...
            self._unchecked_color = unchecked_color or b'DefaultButton.Off'
            self._checked_color = checked_color or b'DefaultButton.On'
            self._checked = False
            self._checked_listener = control._event_listeners.get(b'checked', None)
...
class EncoderControl(Control):
    TOUCH_TIME = 0.5
    touched = control_event(b'touched')
    released = control_event(b'released')
    ...
    class State(Control.State):
        ...
        def __init__(self, control=None, manager=None, *a, **k):
            ...
            self._touched_listener = control._event_listeners.get(b'touched', None)
            self._released_listener = control._event_listeners.get(b'released', None)
            self._touch_value_slot = None
            self._timer_based = False
            if self._touched_listener or self._released_listener:
                self._touch_value_slot = self.register_slot(None, self._on_touch_value, b'touch_value')
            return
        ...
        def set_control_element(self, control_element):
            ...
            if control_element and hasattr(control_element, b'is_pressed') and control_element.is_pressed():
                self._touch_encoder()
...
class ControlList(Control):
    ...
    class State(Control.State):
        ...
        def __init__(self, control=None, manager=None, extra_args=None, extra_kws=None, unavailable_color=None, *a, **k):
            ...
            self._unavailable_color = unavailable_color if unavailable_color is not None else b'DefaultButton.Disabled'
            if extra_args is not None:
                self._extra_args = extra_args
            if extra_kws is not None:
                self._extra_kws = extra_kws
        ...
        def _make_control(self, index):
            ...
            if not hasattr(control_state, b'index'):
                control_state.index = index
            else:
                raise RuntimeError(b"Cannot set 'index' attribute. Attribute already set.")
            return control
        ...
        def _send_unavailable_color(self, element):
            if hasattr(element, b'set_light'):
...
class MatrixControl(ControlList):
    ...
    class State(ControlList.State):
        ...
        def _make_control(self, index):
            ...
            control_state = control._get_state(self._manager)
            if not hasattr(control_state, b'coordinate'):
                control_state.coordinate = (
                 int(index / self.width), index % self.width)
            else:
                raise RuntimeError(b"Cannot set 'coordinate' attribute. Attribute already set.")
            return control

        def set_control_element(self, control_elements):
            dimensions = (None, None)
            if hasattr(control_elements, b'width') and hasattr(control_elements, b'height'):
                dimensions = (
                ...
                raise RuntimeError(b'Control Elements must be a matrix')
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_Framework/Control.pyc
