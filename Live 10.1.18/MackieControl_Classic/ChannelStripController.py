...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\MackieControl_Classic\ChannelStripController.py
...
class ChannelStripController(MackieControlComponent):
    ...
    def destroy(self):
        ...
        self.send_midi((CC_STATUS, 75, g7_seg_led_conv_table[b' ']))
        self.send_midi((CC_STATUS, 74, g7_seg_led_conv_table[b' ']))
    ...
    def handle_vpot_rotation(self, strip_index, stack_offset, cc_value):
        ...
        if self.__assignment_mode == CSM_IO:
            ...
            channel_strip = self.__channel_strips[(stack_offset + strip_index)]
            assert not channel_strip.assigned_track() or not channel_strip.assigned_track().has_audio_output, b'in every other mode, the midimap should handle the messages'
        ...
        for s in self.__channel_strips:
            ...
            if self.__assignment_mode == CSM_VOLPAN:
                if s.assigned_track() and s.assigned_track().has_audio_output:
                    vpot_param = (
                     s.assigned_track().mixer_device.panning, b'Pan')
                    vpot_display_mode = VPOT_DISPLAY_BOOST_CUT
                    slider_param = (
                     s.assigned_track().mixer_device.volume, b'Volume')
        ...
        if self.__assignment_mode == CSM_VOLPAN:
            ...
            for t in self.song().return_tracks:
                if t == self.__last_attached_selected_track:
                    ass_string = [b'R',
                     chr(ord(b'A') + list(self.song().return_tracks).index(t))]
                    break

            for t in self.song().visible_tracks:
                if t == self.__last_attached_selected_track:
                    ass_string = list(b'%.2d' % min(99, list(self.song().visible_tracks).index(t) + 1))
                    break
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/MackieControl_Classic/ChannelStripController.pyc
