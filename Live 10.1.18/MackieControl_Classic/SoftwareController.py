...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\MackieControl_Classic\SoftwareController.py
...
class SoftwareController(MackieControlComponent):
    ...
    def __init__(self, main_script):
        ...
        av.add_is_view_visible_listener(b'Session', self.__update_session_arranger_button_led)
        av.add_is_view_visible_listener(b'Detail/Clip', self.__update_detail_sub_view_button_led)
        av.add_is_view_visible_listener(b'Browser', self.__update_browser_button_led)
        av.add_is_view_visible_listener(b'Detail', self.__update_detail_button_led)
    ...
    def destroy(self):
        av = self.application().view
        av.remove_is_view_visible_listener(b'Session', self.__update_session_arranger_button_led)
        av.remove_is_view_visible_listener(b'Detail/Clip', self.__update_detail_sub_view_button_led)
        av.remove_is_view_visible_listener(b'Browser', self.__update_browser_button_led)
        av.remove_is_view_visible_listener(b'Detail', self.__update_detail_button_led)
    ...
    def __toggle_session_arranger_is_visible(self):
        if self.application().view.is_view_visible(b'Session'):
            if self.shift_is_pressed():
                self.application().view.focus_view(b'Session')
            else:
                self.application().view.hide_view(b'Session')
        else:
            assert self.application().view.is_view_visible(b'Arranger')
            if self.shift_is_pressed():
                self.application().view.focus_view(b'Arranger')
            else:
                self.application().view.hide_view(b'Arranger')

    def __toggle_detail_sub_view(self):
        if self.application().view.is_view_visible(b'Detail/Clip'):
            if self.shift_is_pressed():
                self.application().view.focus_view(b'Detail/Clip')
            else:
                self.application().view.show_view(b'Detail/DeviceChain')
        elif self.shift_is_pressed():
            self.application().view.focus_view(b'Detail/DeviceChain')
        else:
            self.application().view.show_view(b'Detail/Clip')

    def __toggle_browser_is_visible(self):
        if self.application().view.is_view_visible(b'Browser'):
            if self.shift_is_pressed():
                self.application().view.focus_view(b'Browser')
            else:
                self.application().view.hide_view(b'Browser')
        else:
            self.application().view.show_view(b'Browser')

    def __toggle_detail_is_visible(self):
        if self.application().view.is_view_visible(b'Detail'):
            if self.shift_is_pressed():
                self.application().view.focus_view(b'Detail')
            else:
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/MackieControl_Classic/SoftwareController.pyc
