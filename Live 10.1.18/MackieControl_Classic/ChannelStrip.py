...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\MackieControl_Classic\ChannelStrip.py
...
class ChannelStrip(MackieControlComponent):
    ...
    def show_full_enlighted_poti(self):
        self.send_midi((
         CC_STATUS + 0, 48 + self.__strip_index, VPOT_DISPLAY_WRAP * 16 + 11))
    ...
    def __assigned_track_index(self):
        ...
        assert self.__assigned_track and 0
        return
    ...
    def __update_mute_led(self):
        if self.__assigned_track and self.__assigned_track.mute:
            self.send_midi((
             NOTE_ON_STATUS, SID_MUTE_BASE + self.__strip_index, BUTTON_STATE_ON))
        else:
            self.send_midi((
             NOTE_ON_STATUS, SID_MUTE_BASE + self.__strip_index, BUTTON_STATE_OFF))

    def __update_solo_led(self):
        if self.__assigned_track and self.__assigned_track.solo:
            self.send_midi((
             NOTE_ON_STATUS, SID_SOLO_BASE + self.__strip_index, BUTTON_STATE_ON))
        else:
            self.send_midi((
             NOTE_ON_STATUS, SID_SOLO_BASE + self.__strip_index, BUTTON_STATE_OFF))

    def __update_track_is_selected_led(self):
        if self.song().view.selected_track == self.__assigned_track:
            self.send_midi((
             NOTE_ON_STATUS, SID_SELECT_BASE + self.__strip_index, BUTTON_STATE_ON))
        else:
            self.send_midi((
             NOTE_ON_STATUS, SID_SELECT_BASE + self.__strip_index, BUTTON_STATE_OFF))
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/MackieControl_Classic/ChannelStrip.pyc
