...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\MackieControl_Classic\MainDisplayController.py
...
class MainDisplayController(MackieControlComponent):
    ...
    def __init__(self, main_script, display):
        ...
        self.__channel_strip_strings = [ b'' for x in range(NUM_CHANNEL_STRIPS) ]
    ...
    def set_controller_extensions(self, left_extensions, right_extensions):
        ...
        self.__channel_strip_strings = [ b'' for x in range(len(self.__displays) * NUM_CHANNEL_STRIPS) ]
        self.refresh_state()
    ...
    def on_update_display_timer(self):
        ...
        for display in self.__displays:
            if self.__channel_strip_mode:
                ...
                for t in track_index_range:
                    if self.__parameters and self.__show_parameter_names:
                        if self.__parameters[strip_index]:
                            ...
                            upper_string += self.__generate_6_char_string(b'')
                    elif t < len(tracks):
                    ...
                    if self.__parameters and self.__parameters[strip_index]:
                        if self.__parameters[strip_index][0]:
                            ...
                            lower_string += self.__generate_6_char_string(b'')
                ...
                ascii_message = b'< _1234 guck ma #!?:;_ >'
                if not self.__test:
                    self.__test = 0
    ...
    def __generate_6_char_string(self, display_string):
        if not display_string:
            return b'      '
        if len(display_string.strip()) > 6 and display_string.endswith(b'dB') and display_string.find(b'.') != -1:
            display_string = display_string[:-2]
        if len(display_string) > 6:
            for um in [b' ', b'i', b'o', b'u', b'e', b'a']:
        ...
        assert len(ret) == 6
        return ret
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/MackieControl_Classic/MainDisplayController.pyc
