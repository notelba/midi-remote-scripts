...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\MiniLab_mkII\MiniLabMk2.py
...
class MiniLabMk2(MiniLab):
    ...
    def _create_controls(self):
        ...
        self._pad_leds = ButtonMatrixElement(rows=[ [ SysexValueControl(message_prefix=SETUP_MSG_PREFIX + (WRITE_COMMAND, WORKING_MEMORY_ID, COLOR_PROPERTY, column + 112 + row * 8), default_value=(0, ), name=b'Pad_LED_%d' % (column,)) for column in xrange(8) ] for row in xrange(2)
                                                  ], name=b'Pad_LED_Matrix')
        self._memory_slot_selection = SysexValueControl(message_prefix=SETUP_MSG_PREFIX + (MEMORY_SLOT_PROPERTY,), name=b'Memory_Slot_Selection')
        self._hardware_live_mode_switch = SysexValueControl(message_prefix=LIVE_MODE_MSG_HEAD, default_value=(
         OFF_VALUE,), name=b'Hardware_Live_Mode_Switch')

    def _create_hardware_settings(self):
        self._hardware_settings = HardwareSettingsComponent(name=b'Hardware_Settings', is_enabled=False, layer=Layer(memory_slot_selection=self._memory_slot_selection, hardware_live_mode_switch=self._hardware_live_mode_switch))
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/MiniLab_mkII/MiniLabMk2.pyc
