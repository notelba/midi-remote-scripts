...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchkey_MK2\Launchkey_MK2.py
...
class Launchkey_MK2(OptimizedControlSurface):
    ...
    def _create_controls(self):
        self._encoders = ButtonMatrixElement(rows=[
         [ make_encoder(identifier, name=b'Encoder_%d' % (index,)) for index, identifier in enumerate(xrange(21, 29))
         ]])
        self._top_pad_row = ButtonMatrixElement(rows=[
         [ make_button(identifier, name=b'Pad_0_%d' % (index,)) for index, identifier in enumerate(xrange(96, 104))
         ]])
        self._bottom_pad_row_raw = [ make_button(identifier, name=b'Pad_1_%d' % (index,)) for index, identifier in enumerate(xrange(112, 120))
                                   ]
        self._bottom_pad_row = ButtonMatrixElement(rows=[self._bottom_pad_row_raw])
        self._top_launch_button = make_button(104, name=b'Scene_Launch_Button')
        self._bottom_launch_button = make_button(120, name=b'Stop_All_Clips_Button')
        self._scene_up_button = make_button(112, MIDI_CC_TYPE, name=b'Scene_Up_Button')
        self._scene_down_button = make_button(113, MIDI_CC_TYPE, name=b'Scene_Down_Button')
        self._stop_button = make_button(114, MIDI_CC_TYPE, name=b'Stop_Button')
        self._play_button = make_button(115, MIDI_CC_TYPE, name=b'Play_Button')
        self._loop_button = make_button(116, MIDI_CC_TYPE, name=b'Loop_Button')
        self._record_button = make_button(117, MIDI_CC_TYPE, name=b'Record_Button')
        self._sliders = ButtonMatrixElement(rows=[
         [ make_slider(identifier, name=b'Slider_%d' % (index,)) for index, identifier in enumerate(xrange(41, 49))
         ]])
        self._master_slider = make_slider(7, name=b'Master_Slider')
        self._25_key_slider = make_slider(7, name=b'Slider', channel=0)
        self._mute_buttons_raw = [ make_button(identifier, MIDI_CC_TYPE, name=b'Mute_Button_%d' % (index,)) for index, identifier in enumerate(xrange(51, 59))
                                 ]
        self._mute_buttons = ButtonMatrixElement(rows=[self._mute_buttons_raw])
        self._master_button = make_button(59, MIDI_CC_TYPE, name=b'Master_Button')
        self._track_left_button = make_button(102, MIDI_CC_TYPE, name=b'Track_Left_Button')
        self._track_right_button = make_button(103, MIDI_CC_TYPE, name=b'Track_Right_Button')
        ...
        for index in xrange(consts.MAX_SENDS):
            setattr(self, b'_send_%d_button' % (index,), self._bottom_pad_row_raw[(index + 2)])
            self._send_mode_buttons[b'send_%d_mode_button' % (index,)] = getattr(self, b'_send_%d_button' % (index,))

        self._extended_mode_button = make_button(12, name=b'Dummy_Extended_Mode_Button')
        self._extended_mode_button.add_value_listener(nop)
        self._encoder_incontrol_button = make_button(13, is_momentary=False, name=b'Encoder_InControl_Button')
        self._encoder_incontrol_button.add_value_listener(nop)
        self._slider_incontrol_button = make_button(14, is_momentary=False, name=b'Fader_InControl_Button')
        self._slider_incontrol_button.add_value_listener(nop)
        self._pad_incontrol_button = make_button(15, is_momentary=False, name=b'Pad_InControl_Button')
        self._pad_incontrol_button.add_value_listener(self._update_pads)
        self._encoder_incontrol_button2 = make_button(16, name=b'Encoder_InControl_Button')
        self._pad_in_control_status_button = make_button(11, name=b'Dummy_InControl_Button')

    def _create_session(self):
        self._session = SessionComponent(name=b'Session', is_enabled=False, num_tracks=self._top_pad_row.width(), num_scenes=self._top_pad_row.height(), enable_skinning=True, layer=Layer(clip_launch_buttons=self._top_pad_row, scene_launch_buttons=ButtonMatrixElement(rows=[
         [
    ...
    def _setup_navigation(self):
        self._session_navigation = SessionNavigationComponent(is_enabled=False, name=b'Session_Navigation', layer=Layer(next_track_button=self._track_right_button, prev_track_button=self._track_left_button, next_scene_button=self._scene_down_button, prev_scene_button=self._scene_up_button))
        self._session_navigation.set_enabled(True)

    def _create_transport(self):
        self._transport = TransportComponent(is_enabled=False, name=b'Transport', layer=Layer(play_button=self._play_button, stop_button=self._stop_button, loop_button=self._loop_button, record_button=self._record_button))
    ...
    def _create_mixer(self):
        ...
        self._mixer = MixerComponent(is_enabled=False, name=b'Mixer', num_tracks=self._sliders.width(), layer=mixer_volume_layer)
        ...
        self._mute_button_modes.add_mode(b'mute_mode', mute_mode)
        self._mute_button_modes.add_mode(b'solo_mode', solo_mode, behaviour=CancellableBehaviour())
        self._mute_button_modes.layer = Layer(solo_mode_button=self._master_button)
        self._mute_button_modes.selected_mode = b'mute_mode'
        self._mute_button_modes.set_enabled(True)
        return

    def _create_device(self):
        self._device = DeviceComponent(name=b'Device', is_enabled=False, device_selection_follows_track_selection=True)
        self.set_device_component(self._device)
        self._device.set_enabled(True)

    def _create_background(self):
        self._background = BackgroundComponent(name=b'BackgroundComponent')
    ...
    def _create_encoder_modes(self):
        ...
        self._encoder_modes.add_mode(b'device_mode', device_mode, is_enabled=True)
        self._encoder_modes.add_mode(b'pan_mode', [pan_mode, background_mode], is_enabled=True)
        for index in xrange(6):
            self._encoder_modes.add_mode(b'send_%d_mode' % (index,), [
             sends_mode, partial(self._set_send_index, index), background_mode], is_enabled=False)
    ...
    def _create_mode_selector(self):
        ...
        self._mode_selector.add_mode(b'mode_selection', [
         partial(self._toggle_in_control, True), mode_selection, device_navigation], behaviour=MomentaryBehaviour())
        session_control = AddLayerMode(self._session, Layer(clip_launch_buttons=self._top_pad_row))
        self._mode_selector.add_mode(b'session_mode', [partial(self._toggle_in_control, False), session_control])
    ...
    def _on_return_tracks_changed(self):
        ...
        for index in xrange(6):
            self._encoder_modes.set_mode_enabled(b'send_%d_mode' % (index,), True if index < num_sends else False)
    ...
    def handle_sysex(self, midi_bytes):
        if self._is_identity_response(midi_bytes):
            ...
            if self._is_identity_response_valid(product_id_bytes):
                ...
                self.log_message(b'MIDI device responded with wrong product id (%s).' % (
    ...
    def on_identified(self):
        ...
        self._mode_selector.selected_mode = b'session_mode'
        self.update()

    def disconnect(self):
        self._extended_mode_button.turn_off()
        super(Launchkey_MK2, self).disconnect()
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Launchkey_MK2/Launchkey_MK2.pyc
