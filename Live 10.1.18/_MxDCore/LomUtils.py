...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_MxDCore\LomUtils.py
...
def create_lom_doc_string(lom_object):
    description = b''
    if hasattr(lom_object, b'__doc__') and isinstance(lom_object.__doc__, basestring) and len(lom_object.__doc__) > 0:
        description = b'description %s' % lom_object.__doc__.replace(b'\n', b' ').replace(b',', b'\\,')
    return description
...
class LomInformation(object):
    ...
    def _generate_object_info(self, lom_object, epii_version):
        if isinstance(lom_object, cs_base_classes()):
            property_names = ifilter(lambda prop: not prop.startswith(b'_'), dir(lom_object))
        ...
        try:
            ...
            if not is_class(real_prop):
                ...
                if prop_info and prop_info.format == MFLPropertyFormats.JSON:
                    self._properties.append((prop_name, b'dict'))
                ...
                elif prop_name == b'canonical_parent':
                    if real_prop != None:
                        self._children.append((prop_name, prop_type))
                elif callable(real_prop):
                    if not prop_name.endswith(b'_listener'):
                        self._functions.append((prop_name,))
                elif prop_type == b'unicode':
                    self._properties.append((prop_name, b'str'))
                elif prop_type not in ('type', 'Enum'):
                    info_type = b'int' if isinstance(real_prop, ENUM_TYPES) else prop_type
...
class LomIntrospection(object):
    ...
    def _is_relevant_class(self, class_object):
        return is_class(class_object) and hasattr(class_object, b'__module__') and sys.modules.get(class_object.__module__) in self._lom_modules and class_object not in self._lom_classes and class_object not in self._excluded
...
def wrap_control_surfaces_list(parent):
    ...
    return TupleWrapper.get_tuple_wrapper(parent, b'control_surfaces', element_filter=lambda e: isinstance(e, cs_base_classes()))
...
class LomPathResolver(object):
    ...
    def _property_object_from_path(self, path_components):
        ...
        for component in path_components[1:]:
            try:
                ...
                raise LomAttributeError(b"invalid index of component '%s'" % prev_component)
            except AttributeError:
                raise LomAttributeError(b"invalid path component '%s'" % component)
            else:
                prev_component = component

        if not is_lom_object(lom_object, self._lom_classes):
            raise LomObjectError(b"component '%s' is not an object" % prev_component)
        return lom_object
    ...
    def _calculate_object_from_path(self, path_components):
        ...
        return lom_object
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_MxDCore/LomUtils.pyc
