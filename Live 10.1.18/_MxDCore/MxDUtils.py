...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_MxDCore\MxDUtils.py
...
STATE_NEUTRAL = b'neutral'
STATE_QUOTED_STR = b'quoted'
STATE_UNQUOTED_STR = b'unquoted'
STATE_PENDING_NR = b'number'
STATE_PENDING_FLOAT = b'float'
QUOTE_ENTITY = b'&quot;'
QUOTE_SIMPLE = b'"'
...
class StringHandler(object):
    ...
    def parse_string(self, string):
        ...
        for index in range(len(string)):
            char = string[index]
            handle_selector = b'_' + str(self._state) + b'_handle_char'
            if hasattr(self, handle_selector):
                getattr(self, handle_selector)(char, index)
            else:
                logger.info(b'Unknown state ' + str(self._state))
                assert False

        finalize_selector = b'_finalize_' + str(self._state)
    ...
    def _finalize_quoted(self):
        raise RuntimeError(b'no match for quote at index %d found' % self._open_quote_index)
    ...
    def _finalize_number(self):
        ...
        if cmp(unicode(self._arguments[(-1)]), b'id') == 0:
            self._arguments.pop()
            try:
                argument = self._id_callback(argument)
            except KeyError:
                raise RuntimeError(b'Invalid id')
    ...
    def _add_argument(self, argument):
        ...
        self._sub_string = b''
        self._state = STATE_NEUTRAL
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_MxDCore/MxDUtils.pyc
