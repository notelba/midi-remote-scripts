...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_MxDCore\MxDCore.py
...
PATH_KEY = b'CURRENT_PATH'
ID_KEY = b'CURRENT_LOM_ID'
TYPE_KEY = b'CURRENT_TYPE'
PROP_KEY = b'CURRENT_PROPERTY'
PROP_LISTENER_KEY = b'PROPERTY_LISTENER'
PATH_LISTENER_KEY = b'PATH_LISTENERS'
OPEN_OPERATIONS_KEY = b'OPEN_OPERATION'
NOTE_BUFFER_KEY = b'NOTE_BUFFER'
NOTE_OPERATION_KEY = b'NOTE_OPERATION'
NOTE_COUNT_KEY = b'NOTE_COUNT'
NOTE_REPLACE_KEY = b'NOTE_REPLACE'
NOTE_SET_KEY = b'NOTE_SET'
CONTAINS_CS_ID_KEY = b'CONTAINS_CS_ID_KEY'
LAST_SENT_ID_KEY = b'LAST_SENT_ID'
PRIVATE_PROP_WARNING = b'Warning: Calling private property. This property might change or be removed in the future.'
HIDDEN_PROP_WARNING = b'Warning: Calling hidden property. This property might change or be removed in the future.'

def concatenate_strings(string_list, string_format=b'%s %s'):
    return unicode(reduce(lambda s1, s2: string_format % (s1, s2), string_list) if len(string_list) > 0 else b'')
...
def parameter_to_bool(parameter):
    ...
    elif unicode(parameter) in ('True', 'False'):
        bool_value = unicode(parameter) == b'True'
    return bool_value
...
class MxDCore(object):
    ...
    def __init__(self, *a, **k):
        ...
        self._call_handler = {b'get_notes': self._object_get_notes_handler, 
           b'set_notes': self._object_set_notes_handler, 
           b'get_selected_notes': self._object_selected_notes_handler, 
           b'replace_selected_notes': self._object_replace_selected_notes_handler, 
           b'notes': self._object_notes_handler, 
           b'note': self._object_note_handler, 
           b'done': self._object_done_handler, 
           b'get_control_names': self._cs_api.object_get_control_names, 
           b'get_control': self._cs_api.object_get_control, 
           b'grab_control': self._cs_api.object_grab_control, 
           b'release_control': self._cs_api.object_release_control, 
           b'send_midi': self._cs_api.object_send_midi, 
           b'send_receive_sysex': self._cs_api.object_send_receive_sysex, 
           b'grab_midi': self._cs_api.object_grab_midi, 
           b'release_midi': self._cs_api.object_release_midi}
    ...
    def disconnect(self):
        for dev_id in self.device_contexts.keys():
            ...
            if device_context[CONTAINS_CS_ID_KEY]:
                self.release_device_context(dev_id, -1, b'')
    ...
    def _get_current_type(self, device_id, object_id):
        ...
        if current_type != -1:
            return {0: b'obs', 1: None, 2: b'obj', 3: b'obs', 4: b'rmt'}[current_type]
        else:
    ...
    def _set_current_property(self, device_id, object_id, property_name):
        ...
        if not self.manager.set_current_property(device_id, object_id, property_name):
            ...
            self._set_current_type(device_id, object_id, b'obs')
            self._observer_update_listener(device_id, object_id)
    ...
    def update_device_context(self, device_id, object_id):
        ...
        if object_id not in self.device_contexts[device_id].keys():
            self.device_contexts[device_id][object_id] = {PATH_KEY: [], ID_KEY: 0, 
               TYPE_KEY: None, 
               PROP_KEY: b'', 
    ...
    def path_set_path(self, device_id, object_id, parameters):
        ...
        pure_path = parameters.strip().strip(b'"')
        path_components = pure_path.split(b' ')
        if len(pure_path) > 0 and path_components[0] not in ROOT_KEYS:
            self._raise(device_id, object_id, b'set path: invalid path')
        else:
    ...
    def path_goto(self, device_id, object_id, parameters):
        ...
        for msg_type, value in (
         (
          b'path_curr_path', result_path),
         (
          b'path_orig_id', result_id),
         (
          b'path_curr_id', result_id)):
    ...
    def path_get_id(self, device_id, object_id, parameters):
        ...
        for msg_type, value in ((b'path_orig_id', result_id), (b'path_curr_id', result_id)):
            self.manager.send_message(device_id, object_id, msg_type, value)
    ...
    def _get_lom_object_properties(self, device_id, object_id, looking_for):
        ...
        if len(current_path) == 0:
            ...
            if is_object_iterable(current_object):
                result = b'%d list elements, no %s' % (len(current_object), looking_for)
            else:
    ...
    def path_get_props(self, device_id, object_id, parameters):
        result = self._get_lom_object_properties(device_id, object_id, b'properties') or b'No path properties'
        self.manager.send_message(device_id, object_id, b'path_props', result)

    def path_get_children(self, device_id, object_id, parameters):
        result = self._get_lom_object_properties(device_id, object_id, b'children') or b'No children'
        self.manager.send_message(device_id, object_id, b'path_children', result)
    ...
    def path_get_count(self, device_id, object_id, parameters):
        ...
        if property != None:
            ...
            self.manager.send_message(device_id, object_id, b'path_count', concatenate_strings((parameters, count)))
        else:
            self._raise(device_id, object_id, b'getcount: invalid property name')
        return
    ...
    def obj_set_id(self, device_id, object_id, parameter):
        if self._is_integer(parameter) and self._lom_id_exists(device_id, int(parameter)):
            self._set_current_lom_id(device_id, object_id, int(parameter), b'obj')
        else:
            self._raise(device_id, object_id, b'set id: invalid id')

    def obj_get_id(self, device_id, object_id, parameter):
        self.manager.send_message(device_id, object_id, b'obj_id', unicode(self._get_current_lom_id(device_id, object_id)))
        ...
        if current_object != None:
            ...
            if not is_object_iterable(current_object):
                ...
                def accumulate_info(info_list, label):
                    result = b''
                    if len(info_list) > 0:
                        str_format = b'%s %s %s\n' if len(info_list[0]) > 1 else b'%s %s\n'
                        formatter = lambda info: str_format % ((label,) + info)
                        result = concatenate_strings(map(formatter, info_list), string_format=b'%s%s')
                    return result

                object_info += accumulate_info(lom_info.lists_of_children, b'children') + accumulate_info(lom_info.children, b'child') + accumulate_info(lom_info.properties, b'property') + accumulate_info(lom_info.functions, b'function')
            object_info += b'done'
        self.manager.send_message(device_id, object_id, b'obj_info', unicode(object_info))
        return
    ...
    def _set_property_value(self, lom_object, property_name, value):
        ...
        if prop_info and prop_info.from_json:
            ...
            if value is None:
                raise LomAttributeError(b'set: invalid value')
        elif property_name in PROPERTY_TYPES.keys():
            if not is_lom_object(value, self.lom_classes):
                raise LomAttributeError(b'set: no valid object id')
            if not isinstance(value, PROPERTY_TYPES[property_name]):
                raise LomAttributeError(b'set: type mismatch')
        elif isinstance(prop, (int, bool)):
            if unicode(value) in ('True', 'False'):
                value = int(unicode(value) == b'True')
            ...
            if prop_info and prop_info.format == MFLPropertyFormats.JSON:
                ...
                raise LomAttributeError(b'set: unsupported property type')
        else:
            raise LomAttributeError(b'set: unsupported property type')
    ...
    def _warn_if_using_private_property(self, device_id, object_id, property_name):
        ...
        if property_name.startswith(b'_'):
    ...
    def obj_get(self, device_id, object_id, parameters):
        ...
        if current_object != None:
            try:
                ...
                self.manager.send_message(device_id, object_id, b'obj_prop_val', result)
            except LomAttributeError as e:
                self._warn(device_id, object_id, unicode(e))

        else:
            self._warn(device_id, object_id, b'get: no valid object set')
        return
    ...
    def obj_call(self, device_id, object_id, parameters):
        ...
        if current_object != None:
            try:
                ...
                self._raise(device_id, object_id, b"%s: '%s'" % (unicode(e), parameters))
            except Exception as e:
                reason = b'Invalid ' + (b'arguments' if isinstance(e, TypeError) else b'syntax')
                self._raise(device_id, object_id, b"%s: '%s'" % (reason, parameters))

        else:
            self._warn(device_id, object_id, b'call %s: no valid object set' % parameters)
        return

    def obs_set_id(self, device_id, object_id, parameter):
        if self._is_integer(parameter) and self._lom_id_exists(device_id, int(parameter)):
            self.device_contexts[device_id][object_id][LAST_SENT_ID_KEY] = None
            self._set_current_lom_id(device_id, object_id, int(parameter), b'obs')
        else:
            self._raise(device_id, object_id, b'set id: invalid id')
        return

    def obs_get_id(self, device_id, object_id, parameter):
        self.manager.send_message(device_id, object_id, b'obs_id', unicode(self._get_current_lom_id(device_id, object_id)))
    ...
    def obs_get_prop(self, device_id, object_id, parameter):
        self.manager.send_message(device_id, object_id, b'obs_prop', unicode(self._get_current_property(device_id, object_id)))

    def obs_get_type(self, device_id, object_id, parameter):
        current_object = self._get_current_lom_object(device_id, object_id)
        property_name = self._get_current_property(device_id, object_id)
        result = b'unknown'
        if current_object != None and property_name != b'':
            if hasattr(current_object, property_name):
                result = getattr(current_object, property_name).__class__.__name__
            else:
                self._warn(device_id, object_id, b'gettype: no property with the name ' + property_name)
        self.manager.send_message(device_id, object_id, b'obs_type', unicode(result))
        return
    ...
    def rmt_set_id(self, device_id, object_id, parameter):
        if self._is_integer(parameter) and self._lom_id_exists(device_id, int(parameter)):
            ...
            if isinstance(lom_object, (Live.DeviceParameter.DeviceParameter, type(None))):
                self._set_current_lom_id(device_id, object_id, new_id, b'rmt')
            else:
                self._raise(device_id, object_id, b'set id: only accepts objects of type DeviceParameter')
        else:
            self._raise(device_id, object_id, b'set id: invalid id')
        return
    ...
    def str_representation_for_object(self, lom_object, mark_ids=True):
        ...
        if is_object_iterable(lom_object):
            ...
            result = (b'id ' if mark_ids else b'') + unicode(self._get_lom_id_by_lom_object(lom_object))
    ...
    def _install_path_listeners(self, device_id, object_id, listener_callback):
        ...
        for lom_object, attribute in obj_attr_iter:
            ...
            if lom_object != None and hasattr(lom_object, attribute + b'_has_listener'):
                assert not getattr(lom_object, attribute + b'_has_listener')(listener)
                getattr(lom_object, b'add_%s_listener' % attribute)(listener)
    ...
    def _uninstall_path_listeners(self, device_id, object_id):
        ...
        for (lom_object, attribute), listener in old_listeners.iteritems():
            if lom_object != None and getattr(lom_object, attribute + b'_has_listener')(listener):
                getattr(lom_object, b'remove_%s_listener' % attribute)(listener)
    ...
    def _path_listener_callback(self, device_id, object_id):
        ...
        self.manager.send_message(device_id, object_id, b'path_curr_id', unicode(resulting_id))
        self._install_path_listeners(device_id, object_id, self._path_listener_callback)
    ...
    def _goto_path(self, device_id, object_id, parameters):
        ...
        try:
            ...
            path_components = []
            if len(pure_path) > 0:
                path_components = pure_path.strip().split(b' ')
            ...
            self._raise(device_id, object_id, b'goto: invalid path')
            return
    ...
    def _object_default_call_handler(self, device_id, object_id, lom_object, parameters):
        ...
        self.manager.send_message(device_id, object_id, b'obj_call_result', result_str)
    ...
    def _object_get_notes_handler(self, device_id, object_id, lom_object, parameters):
        ...
        assert parameters[0] == b'get_notes'
        notes = getattr(lom_object, b'get_notes')(parameters[1], parameters[2], parameters[3], parameters[4])
        self.manager.send_message(device_id, object_id, b'obj_call_result', self._create_notes_output(notes))

    def _object_selected_notes_handler(self, device_id, object_id, lom_object, parameters):
        assert isinstance(lom_object, Live.Clip.Clip) and lom_object.is_midi_clip
        notes = getattr(lom_object, b'get_selected_notes')()
        self.manager.send_message(device_id, object_id, b'obj_call_result', self._create_notes_output(notes))
    ...
    def _object_notes_handler(self, device_id, object_id, lom_object, parameters):
        ...
        if NOTE_OPERATION_KEY in device_context[OPEN_OPERATIONS_KEY].keys():
            ...
            self._raise(device_id, object_id, b'no operation in progress')
    ...
    def _object_note_handler(self, device_id, object_id, lom_object, parameters):
        ...
        try:
            if NOTE_OPERATION_KEY not in operations:
                raise LomNoteOperationError(b'no operation in progress')
            if NOTE_COUNT_KEY not in operations:
                raise LomNoteOperationError(b'no note count given')
            assert NOTE_BUFFER_KEY in operations
            if len(operations[NOTE_BUFFER_KEY]) >= operations[NOTE_COUNT_KEY]:
                raise LomNoteOperationError(b'too many notes')
    ...
    def _selector_for_note_operation(self, note_operation):
        if note_operation not in (NOTE_REPLACE_KEY, NOTE_SET_KEY):
            raise LomNoteOperationWarning(b'invalid note operation')
        if note_operation == NOTE_SET_KEY:
            return b'set_notes'
        return b'replace_selected_notes'
        ...
        if NOTE_OPERATION_KEY in open_operations and NOTE_COUNT_KEY in open_operations:
            ...
            try:
                ...
                if len(notes) != open_operations[NOTE_COUNT_KEY]:
                    raise LomNoteOperationWarning(b'wrong note count')
    ...
    def _create_notes_output(self, notes):
        ...
        note_format = lambda note: b'note %s\n' % concatenate_strings(map(element_format, note))
        result = b'notes %d\n%sdone' % (
         len(notes),
         concatenate_strings(map(note_format, notes), string_format=b'%s%s'))
        return result
    ...
    def _start_note_operation(self, device_id, object_id, lom_object, parameters, operation):
        ...
        if NOTE_OPERATION_KEY not in device_context[OPEN_OPERATIONS_KEY].keys():
            ...
            self._raise(device_id, object_id, b'an operation is already in progress')
    ...
    def _observer_install_listener(self, device_id, object_id):
        ...
        property_name = self._get_current_property(device_id, object_id)
        self._warn_if_using_private_property(device_id, object_id, property_name)
        if property_name == b'id':
            self._observer_id_callback(device_id, object_id)
        elif current_object != None and property_name != b'':
            ...
            if hasattr(current_object, transl_prop_name + b'_has_listener'):
                assert not getattr(current_object, transl_prop_name + b'_has_listener')(listener_callback)
                getattr(current_object, b'add_%s_listener' % transl_prop_name)(listener_callback)
                ...
                self._warn(device_id, object_id, b'property cannot be listened to')
        return
    ...
    def _observer_uninstall_listener(self, device_id, object_id):
        ...
        if current_object != None and listener_callback != None:
            assert property_name != b''
            assert not isinstance(current_object, TupleWrapper)
            transl_prop_name = self._listenable_property_for(property_name)
            assert hasattr(current_object, transl_prop_name + b'_has_listener')
            if getattr(current_object, transl_prop_name + b'_has_listener')(listener_callback):
                getattr(current_object, b'remove_%s_listener' % transl_prop_name)(listener_callback)
    ...
    def _observer_property_message_type(self, prop, prop_info):
        prop_type = None
        if prop_info and prop_info.format == MFLPropertyFormats.JSON:
            prop_type = b'obs_dict_val'
        elif isinstance(prop, (str, unicode)):
            prop_type = b'obs_string_val'
        elif isinstance(prop, (int, bool)):
            prop_type = b'obs_int_val'
        elif isinstance(prop, float):
            prop_type = b'obs_float_val'
        elif is_object_iterable(prop):
            prop_type = b'obs_list_val'
        elif is_lom_object(prop, self.lom_classes):
            prop_type = b'obs_id_val'
        return prop_type
    ...
    def _observer_property_callback(self, device_id, object_id, *args):
        ...
        if len(args) > 0:
            ...
            if hasattr(current_object, property_name):
                ...
                if prop_type == None:
                    self._warn(device_id, object_id, b'unsupported property type')
                else:
            ...
            elif hasattr(current_object, property_name + b'_has_listener'):
                self.manager.send_message(device_id, object_id, b'obs_string_val', b'bang')
            else:
                self._warn(device_id, object_id, b'property should be listenable')
        return

    def _observer_id_callback(self, device_id, object_id):
        assert self._get_current_property(device_id, object_id) == b'id'
        object_context = self.device_contexts[device_id][object_id]
        current_object = self._get_current_lom_object(device_id, object_id)
        ...
        if current_id != object_context[LAST_SENT_ID_KEY]:
            ...
            self.manager.send_message(device_id, object_id, b'obs_id_val', unicode(current_id))
        return
    ...
    def _raise(self, device_id, object_id, message):
        logger.error(b'Error: ' + unicode(message))
        self.manager.print_message(device_id, object_id, b'error', unicode(message))

    def _warn(self, device_id, object_id, message):
        logger.warning(b'Warning: ' + unicode(message))
        self.manager.print_message(device_id, object_id, b'warning', unicode(message))
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/_MxDCore/MxDCore.pyc
