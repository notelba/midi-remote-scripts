...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Akai_Force_MPC\device_navigation.py
...
class ScrollingDeviceNavigationComponent(DeviceNavigationComponent):
    prev_device_button = ButtonControl(color=b'Action.Off', pressed_color=b'Action.On')
    next_device_button = ButtonControl(color=b'Action.Off', pressed_color=b'Action.On')
    ...
    def _update_device_name_display(self):
        device = self._device_component.device()
        self.device_name_display[0] = device.name if liveobj_valid(device) else b'No Device'
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Akai_Force_MPC/device_navigation.pyc
