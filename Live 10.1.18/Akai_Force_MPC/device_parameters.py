...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Akai_Force_MPC\device_parameters.py
...
class DeviceParameterComponent(DeviceParameterComponentBase):
    ...
    def __init__(self, *a, **k):
        self._physical_display_parameter_name_data_sources = map(DisplayDataSource, ('', ) * NUM_PARAM_CONTROLS)
        self._physical_display_parameter_value_data_sources = map(DisplayDataSource, ('', ) * NUM_PARAM_CONTROLS)
    ...
    def _update_device_enable_button(self):
        on_off = self.device_on_off_parameter
        self.device_enable_button.color = b'DefaultButton.On' if liveobj_valid(on_off) and on_off.value else b'DefaultButton.Off'
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Akai_Force_MPC/device_parameters.pyc
