...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Akai_Force_MPC\channel_strip.py
...
class ChannelStripComponent(ChannelStripComponentBase):
    ...
    def __on_volume_changed(self):
        track = self.track
        value_string = format_volume_value_string(str(track.mixer_device.volume) if liveobj_valid(track) and track.has_audio_output else b'')
    ...
    def __on_pan_changed(self):
        track = self.track
        self.pan_value_display[0] = str(track.mixer_device.panning) if liveobj_valid(track) and track.has_audio_output else b''
    ...
    def __on_muted_via_solo_changed(self):
        self.solo_mute_button.color = b'DefaultButton.On' if liveobj_valid(self.track) and self.track != self.song.master_track and self.track.muted_via_solo else b'DefaultButton.Off'
    ...
    def _update_crossfade_assign_color_controls(self):
        ...
        if self._track_has_visible_crossfade_assignment_buttons():
            ...
            assign_a_control_color = b'Mixer.CrossfadeAssignA' if mixer_device.crossfade_assign == force_to_live_crossfade_assign_value(CROSSFADE_ASSIGN_A) else off_color
            assign_b_control_color = b'Mixer.CrossfadeAssignB' if mixer_device.crossfade_assign == force_to_live_crossfade_assign_value(CROSSFADE_ASSIGN_B) else off_color
    ...
    def _update_track_name_data_source(self):
        ...
        self._oled_display_track_name_data_source.set_display_string(self._track.name if liveobj_valid(self._track) else b' - ')

    def _update_arm_color_control(self):
        color = b'Mixer.ArmOff'
        track = self.track
    ...
    def _update_mute_color_controls(self):
        mute_color_control_color = b'Mixer.MuteOff'
        mute_button_color = b'Mixer.MuteOn'
        track = self.track
        if liveobj_valid(track) and (track == self.song.master_track or not track.mute):
            mute_color_control_color = b'Mixer.MuteOn'
            mute_button_color = b'Mixer.MuteOff'
    ...
    def _update_track_color_control(self):
        color_to_send = b'DefaultButton.Off'
        ...
        if liveobj_valid(track) and track.color_index != None:
            ...
            if track == self.song.view.selected_track:
                selected_color_to_send = b'DefaultButton.On'
...
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Akai_Force_MPC/channel_strip.pyc
