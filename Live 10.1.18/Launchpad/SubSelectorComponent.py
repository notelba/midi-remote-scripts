...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad\SubSelectorComponent.py
...
class SubSelectorComponent(ModeSelectorComponent):
    ...
    def __init__(self, matrix, side_buttons, session):
        ...
        for column in range(matrix.width()):
            self._mixer.channel_strip(column).name = b'Channel_Strip_' + str(column)
            self._sliders.append(PreciseButtonSliderElement(tuple([ matrix.get_button(column, 7 - row) for row in range(8) ])))
            self._sliders[(-1)].name = b'Button_Slider_' + str(column)
    ...
    def set_update_callback(self, callback):
        assert dir(callback).count(b'im_func') is 1
    ...
    def _setup_send2_mode(self):
        ...
        self._mixer.set_global_buttons(None, None, None)
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Launchpad/SubSelectorComponent.pyc
