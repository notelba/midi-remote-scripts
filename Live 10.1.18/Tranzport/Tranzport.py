...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Tranzport\Tranzport.py
...
class Tranzport:
    ...
    def __init__(self, c_instance):
        ...
        self.application().view.add_is_view_visible_listener(b'Session', self.__on_view_changed)
        ...
        self.__display_line_one = self.__translate_string(b'    Ableton Live    ')
        self.__display_line_two = self.__translate_string(b'                    ')
    ...
    def disconnect(self):
        ...
        if Live:
            self.application().view.remove_is_view_visible_listener(b'Session', self.__on_view_changed)
        for i in chain(self.song().tracks, self.song().return_tracks):
    ...
    def __on_loop_button_pressed(self, button, status):
        ...
        if status > 0:
            if button == TRANZ_LOOP:
                ...
                elif self.application().view.is_view_visible(b'Session'):
                    self.application().view.show_view(b'Arranger')
                elif self.application().view.is_view_visible(b'Arranger'):
                    self.application().view.show_view(b'Session')
    ...
    def __show_track_and_scene(self):
        ...
        if self.application().view.is_view_visible(b'Session'):
            ...
            line = line + self.__translate_string(b'  Scene%2d' % (list(self.song().scenes).index(self.song().view.selected_scene) + 1))
        elif self.application().view.is_view_visible(b'Arranger'):
    ...
    def __show_pos_and_tempo(self):
        ...
        if self.application().view.is_view_visible(b'Session'):
            current_slot = self.song().view.highlighted_clip_slot
            position_str = self.__translate_string(b'[No Clip]  ')
            if current_slot:
                if current_slot.clip:
                    if current_slot.clip.name == b'':
                        position_str = self.__translate_string(b'[No Name]  ')
                    else:
                        position_str = self.__translate_string(self.__bring_string_to_length(current_slot.clip.name, 9)) + self.__translate_string(b'  ')
        else:
            position_str = self.__translate_string(str(b'%3d.' % beat_time.bars)) + self.__translate_string(str(b'%02d.' % beat_time.beats)) + self.__translate_string(str(b'%02d  ' % beat_time.ticks))
        tempo_str = self.__translate_string(b'%3.2fbpm' % self.song().tempo)
        if len(tempo_str) == 8:
    ...
    def __show_vol_and_pan(self):
        ...
        if self.__current_track.has_audio_output:
            self.__display_line_two = self.__translate_string(self.__string_from_number_with_length(volume, 9)) + self.__translate_string(b'       ') + self.__translate_string(self.__string_from_number_with_length(panning, 3))
        else:
            self.__display_line_two = self.__translate_string(b'  No Audio Output   ')

    def __show_loop_settings(self):
        start_time = self.song().get_beats_loop_start()
        length_time = self.song().get_beats_loop_length()
        self.__display_line_two = self.__translate_string(b'S%3d.' % start_time.bars) + self.__translate_string(b'%02d.' % start_time.beats) + self.__translate_string(b'%02d ' % start_time.ticks) + self.__translate_string(b'L%2d.' % length_time.bars) + self.__translate_string(b'%02d.' % length_time.beats) + self.__translate_string(b'%02d' % length_time.ticks)
    ...
    def __show_send_settings(self):
        ...
        if len(self.__current_track.mixer_device.sends) > 0 and self.__current_track.has_audio_output:
            ...
            result = self.__translate_string(self.__string_from_number_with_length(current_send, 8)) + self.__translate_string(b'  in  ') + self.__translate_string(current_send.name)
        else:
            result = self.__translate_string(b' No Sends Available ')
        self.__display_line_two = result

    def __show_page_select(self):
        index = self.__selected_page
        if self.application().view.is_view_visible(b'Session') or index > 0:
            index = index + 1
    ...
    def __string_from_number_with_length(self, number, length):
        ...
        return result
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Tranzport/Tranzport.pyc
