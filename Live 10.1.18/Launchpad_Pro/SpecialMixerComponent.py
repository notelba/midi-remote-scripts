...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_Pro\SpecialMixerComponent.py
...
class SpecialMixerComponent(MixerComponent):
    ...
    def set_volume_controls(self, controls):
        for strip, control in izip_longest(self._channel_strips, controls or []):
            if control:
                ...
                control.set_light_and_type(b'Mixer.Volume', FADER_TYPE_STANDARD)
    ...
    def set_pan_controls(self, controls):
        for strip, control in izip_longest(self._channel_strips, controls or []):
            if control:
                ...
                control.set_light_and_type(b'Mixer.Pan', FADER_TYPE_BIPOLAR)
    ...
    def set_arm_buttons(self, buttons):
        for strip, button in izip_longest(self._channel_strips, buttons or []):
            if button:
                button.reset_state()
                button.set_on_off_values(b'Mixer.ArmOn', b'Mixer.ArmOff')
    ...
    def set_mute_buttons(self, buttons):
        for strip, button in izip_longest(self._channel_strips, buttons or []):
            if button:
                button.reset_state()
                button.set_on_off_values(b'Mixer.MuteOff', b'Mixer.MuteOn')
    ...
    def set_track_select_buttons(self, buttons):
        for strip, button in izip_longest(self._channel_strips, buttons or []):
            if button:
                button.reset_state()
                button.set_on_off_values(b'Mixer.Selected', b'Mixer.Unselected')
...
class SpecialChanStripComponent(ChannelStripComponent):
    ...
    def __init__(self, *a, **k):
        ...
        self.empty_color = b'DefaultButton.Disabled'
    ...
    def _select_value(self, value):
        ...
        if self.is_enabled() and value and self._track:
            ...
            if view.is_view_visible(b'Detail') and not view.is_view_visible(b'Detail/DeviceChain'):
                view.show_view(b'Detail/DeviceChain')
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/Launchpad_Pro/SpecialMixerComponent.pyc
