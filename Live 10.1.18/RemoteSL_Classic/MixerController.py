...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\RemoteSL_Classic\MixerController.py
...
class MixerController(RemoteSLComponent):
    ...
    def __handle_transport_ccs(self, cc_no, cc_value):
        if cc_no == TS_REWIND_CC:
            ...
            assert False, b'unknown Transport CC ' + str(cc_no)
...
class MixerChannelStrip():
    ...
    def slider_moved(self, cc_value):
        assert self.__assigned_track == None or self.slider_parameter() == None, b'should only be reached when the slider was not realtime mapped '
        return
    ...
    def _on_arm_changed(self):
        if self.__control_second_button and self.__mixer_controller.support_mkII():
            ...
            self.__mixer_controller.remote_sl_parent().send_midi((
             ...
             self.__index + MX_SECOND_BUTTON_ROW_BASE_CC,
             value))
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/RemoteSL_Classic/MixerController.pyc
