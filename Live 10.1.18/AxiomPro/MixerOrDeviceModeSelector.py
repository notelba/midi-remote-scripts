...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\AxiomPro\MixerOrDeviceModeSelector.py
...
class MixerOrDeviceModeSelector(ModeSelectorComponent):
    ...
    def __init__(self, mixer_modes, device, encoders, page_buttons):
        ...
        self._device_dummy_source.set_display_string(b'Mixer')
    ...
    def _parameter_value(self, value, control):
        ...
        if self.is_enabled():
            ...
            if parameter != None:
                self._parameter_source.set_display_string(parameter.name + b': ' + parameter.__str__())
            else:
                self._parameter_source.set_display_string(b'<unmapped>')
    ...
    def _on_timer(self):
        if self._clean_value_display_in > 0:
            self._clean_value_display_in -= 1
            if self._clean_value_display_in == 0:
                self._parameter_source.set_display_string(b'')
                self._clean_value_display_in = -1
    ...
    def _peek_value(self, value):
        ...
        return
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/AxiomPro/MixerOrDeviceModeSelector.pyc
