...
# Embedded file name: c:\Jenkins\live\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\AxiomPro\PageableDeviceComponent.py
...
SPECIAL_NAME_DICT = {b'InstrumentImpulse': (
                        ('Pad1', 'Pad2'),
                        ('Pad3', 'Pad4'),
                        ('Pad5', 'Pad6'),
                        ('Pad7', 'Pad8')), 
   ...
   b'MultiSampler': (
                   ...
                   ('Osc', )), 
   b'UltraAnalog': (
                  ('Osc', ), ('Fltr', 'FEnv', 'FMod'), ('VEnv', 'Mix'), ('Out', )), 
   b'LoungeLizard': (
                   ('Ma&Ti', ), ('To&Da', ), ('Pick', 'Mod'), ('Glob.', )), 
   b'StringStudio': (
                   ('Ex&St', 'Damp'),
                   ('Te&Pi', 'Body'),
                   ('Fltr', 'LFO'),
                   ('Glob.', 'Vibr')), 
   b'Eq8': (
          ('Bands', 'EQ1-3'), ('Freq', 'Gain'), ('Reso', 'Fltr'), ('Glob.', ))}
SPECIAL_DEVICE_DICT = {b'InstrumentImpulse': [
                        (
                         IMP_BANK1, IMP_BANK2),
                        (
                         IMP_BANK3, IMP_BANK4),
...
class PageableDeviceComponent(DeviceComponent):
    ...
    def __init__(self, *a, **k):
        ...
        for new_index in range(8):
            ...
            self._parameter_name_data_sources[(-1)].set_display_string(b' - ')
            self._page_name_data_sources[(-1)].set_display_string(b' - ')
    ...
    def _assign_parameters(self):
        ...
        if self._device.class_name in SPECIAL_DEVICE_DICT.keys():
            ...
            self._parameter_value_data_source.set_display_string(b'')
            for index in range(len(self._parameter_controls)):
                if self._parameter_controls[index].mapped_parameter() != None:
                    ...
                    self._parameter_name_data_sources[index].set_display_string(b' - ')
    ...
    def __assign_parameters_special(self):
        ...
        for index in range(len(self._page_name_data_sources)):
            if index < len(bank_names):
                ...
                self._page_name_data_sources[index].set_display_string(b' - ')
    ...
    def __assign_parameters_normal(self):
        ...
        if len(banks) > self._bank_index:
            ...
            if self._bank_index in range(len(bank_names)):
                ...
                self._bank_name = b'Bank ' + str(self._bank_index + 1)
            ...
            for index in range(4):
                ...
                if index < num_banks:
                    if index < num_double_pages:
                        ...
                        display_string = str(parameter_offset + 1).rjust(2) + b'-' + str(parameter_offset + num_controls).rjust(2)
                ...
                parameter_offset += num_controls
# okay decompiling /home/deniz/data/projects/midiremote/Live 10.1.18/AxiomPro/PageableDeviceComponent.pyc
